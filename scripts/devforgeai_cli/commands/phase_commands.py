...
def phase_init_command(
    ...
    """
    try:
        # Validate workflow parameter (STORY-517)
        from ..phase_state import VALID_WORKFLOWS
        ...
        if workflow == "qa":
            ...
            if state_path.exists():
                if format == "json":
                    print(json.dumps({
                        "success": False,
                        "error": "QA state file already exists",
                        "story_id": story_id,
                        "path": str(state_path)
                    }))
                else:
                    print(f"QA state file already exists for {story_id}")
                    print(f"  Path: {state_path}")
                return 1

            state = ps.create_qa(story_id)
            ...
            if format == "json":
                ...
                print(f"Created QA phase state for {story_id}")
                print(f"  Path: {state_path}")
                print(f"  Current phase: {state['current_phase']}")

            return 0
        else:
            # Dev workflow (existing behavior)
...
def phase_complete_command(
    ...
    try:
        ...
        if workflow == "qa":
            # QA workflow: validate steps before completing (STORY-517)
            result = ps.complete_qa_phase(story_id, phase, checkpoint_passed)
            state = ps.read_qa(story_id)
