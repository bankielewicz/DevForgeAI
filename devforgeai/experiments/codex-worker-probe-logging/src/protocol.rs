...
impl<'a> Session<'a> {
    ...
    fn profile_rpc(&mut self, method: &str, params: Value) -> Result<Value> {
        self.rpc(method, params).map_err(|reason| {
            if reason == "rpc_error" {
    ...
    fn profile_observation(
        ...
        params: Value,
        stage: &str,
        check: impl FnOnce(&Value) -> Result<Value>,
    ) -> Result<()> {
    ...
    fn profile_pages(&mut self, method: &str, mut params: Value) -> Result<Vec<Value>> {
        ...
        for _ in 0..10 {
            let response = self.profile_rpc(method, params.clone())?;
            if response["data"].as_array().ok_or("protocol_error")?.len() > 100 {
                return Err("protocol_error".into());
            }
            // Both pinned inventory response schemas omit this field on a final page.
            let cursor = response.get("nextCursor").unwrap_or(&Value::Null).clone();
            pages.push(response);
            if cursor.is_null() {
                return Ok(pages);
            }
    ...
    /// Observes the fixed profile without creating a thread or a turn.
    pub fn preflight(&mut self, r: &Request, covered_sources: &[std::path::PathBuf]) -> Result<()> {
        use crate::effective_profile as check;
    ...
    fn check_account(&mut self, r: &Request) -> Result<()> {
        let account = self
        ...
        match self.rpc("account/rateLimits/read", json!({})) {
            Ok(limits) => {
                ...
                if ["primary", "secondary"].iter().any(|key| {
                    limits["rateLimits"][key]["usedPercent"]
                        .as_f64()
