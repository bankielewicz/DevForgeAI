...
fn main() {
    ...
    if number == 4 && (70..=73).contains(&sub) {
        let bytes = match sub {
            70 => {
            ...
            71 => vec![],
            72 => {
                b"Bearer sk-CANARY_SECRET_29417 arbitrary account/path/prompt CANARY_SECRET_29417"
                    .to_vec()
            }
            _ => vec![b'x'; 600_000],
        };
            ...
            "initialize" => {
                ...
                if number == 4 {
                    match sub {
                        ...
                        74 => {
                            send(
                                json!({"id":900,"method":"CANARY_SECRET_29417","params":{"prompt":"CANARY_SECRET_29417"}}),
                            );
                            continue;
                        }
                        75 => {
                            send(
                                json!({"method":"CANARY_SECRET_29417","params":{"token":"Bearer sk-CANARY_SECRET_29417"}}),
                            );
