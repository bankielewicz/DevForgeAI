...
fn main() {
    ...
    for line in io::stdin().lock().lines() {
        ...
        if case == "initialize-server-request" && method == "initialize" {
            send(
                json!({"id":"request-1","method":"server/unsupported","params":{"unknown":"PRIVATE_PAYLOAD"}}),
            );
            // The harness sends a rejection then stops; no next read is needed.
