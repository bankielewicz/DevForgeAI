...
1. **Hypothesis A**: Pattern not in guidance file
   - Solution: Verify `src/claude/skills/designing-systems/references/user-input-guidance.md` contains explicit architecture pattern section
...
grep -A 10 "Explicit Classification" src/claude/skills/designing-systems/references/user-input-guidance.md | grep -E "^\s*-\s*"
...
**Recommended Fix**:
```markdown
# In src/claude/skills/designing-systems/SKILL.md, Phase 1, Step 0:
...
def simulate_skill_with_missing_guidance(skill_name):
    ...
    fallback_messages = {
        "designing-systems": (
            ...
            "designing-systems/references/user-input-guidance.md, "
...
def test_06_fallback_behavior_identical(temp_project_with_skills):
    ...
    skills_data = {}
    for skill in ["designing-systems", "devforgeai-ui-generator", "devforgeai-orchestration"]:
