...
jobs:
  develop:
    ...
    steps:
      ...
      - name: Develop Story
        run: |
          claude -p "Load story @devforgeai/specs/Stories/${{ matrix.story }}.story.md and implement using /dev workflow" \
            --dangerously-skip-permissions \
            --max-turns 20 \
  ...
  develop-stories:
    ...
    steps:
      ...
      - name: Load Story Context
        ...
        run: |
          cd "../worktree-${{ matrix.story }}"
          story_file="devforgeai/specs/Stories/${{ matrix.story }}.story.md"
      ...
      - name: Develop Story (TDD Workflow)
        run: |
          cd "../worktree-${{ matrix.story }}"
          claude -p "Load story @devforgeai/specs/Stories/${{ matrix.story }}.story.md and implement following DevForgeAI /dev workflow: Red (tests) → Green (implementation) → Refactor" \
            --dangerously-skip-permissions \
