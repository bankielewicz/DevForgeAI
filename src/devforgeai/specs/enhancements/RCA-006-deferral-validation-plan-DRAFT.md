...
2. **Scope Change with ADR (Requirements changed)**
   ...
   - Validation: Verify ADR-XXX exists in devforgeai/specs/adrs/
...
FOR each incomplete DoD item:
    ...
    ELSE IF reason matches "Out of scope: ADR-XXX":
        Glob(pattern="devforgeai/specs/adrs/ADR-{XXX}*.md")
        IF not found:
...
```
Grep(pattern="## QA Validation History", path="devforgeai/specs/Stories/{story-id}.story.md")
...
```
Glob(pattern="devforgeai/specs/Stories/{referenced_story_id}*.md")
