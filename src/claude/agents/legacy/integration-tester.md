...
tools: Read, Write, Edit, Bash(docker:*), Bash(devforgeai-validate:*)
model: sonnet
color: green
...
hooks:
  Stop:
    - hooks:
        - type: command
          command: ".claude/hooks/observation-verify.sh integration-tester"
---
...
When the Task() prompt includes `Handoff: tmp/<WORK_ID>/handoffs/phase-<NN>-integration-tester-handoff.md`, this is a registry-protected dispatch.
...
3. If the handoff is missing, stale, references the wrong workflow/phase/subagent, or lacks required integration-test context, return `H-CONTEXT-MISS` and stop.
...
Do not read broad context files or write integration tests for that dispatch WITHOUT a validated handoff and coverage attestation.
...
**IF invoked from Phase 05** (orchestrator already ran anti-gaming in Phase 05 Step 1):
SKIP — anti-gaming validation already completed by the orchestrator. Proceed to Step 1.

**IF invoked standalone** (proactive trigger, direct invocation):
```bash
devforgeai-validate anti-gaming --test-dir=tests/${STORY_ID}/ --project-root=.
```
Exit 0 = PASS. Exit 1 = BLOCKED (violations found, HALT). Exit 2 = error.
