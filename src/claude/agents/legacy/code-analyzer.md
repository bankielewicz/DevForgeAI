...
When the Task() prompt includes `Handoff: tmp/<WORK_ID>/handoffs/phase-<NN>-code-analyzer-handoff.md`, this is a registry-protected dispatch.
...
3. If the handoff is missing, stale, references the wrong workflow/phase/subagent, or lacks required code-analysis context, return `H-CONTEXT-MISS` and stop.
4. Include a `subagent-result-v1` `coverage_attestation` object in the structured JSON result:
...
Do not read broad context files or return code-analysis results for that dispatch WITHOUT a validated handoff and coverage attestation.
...
- spec-driven-system-architecture skill (brownfield integration)
...
- **Context files** (optional): `devforgeai/specs/context/tech-stack.md` (for accurate terminology), `source-tree/` (documented organization), `architecture-constraints.md` (pattern validation)
...
- **Context Files**: `devforgeai/specs/context/tech-stack.md`, `source-tree/`, `architecture-constraints.md`
