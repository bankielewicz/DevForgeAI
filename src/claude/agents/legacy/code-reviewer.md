...
hooks:
  Stop:
    - hooks:
        - type: command
          command: ".claude/hooks/observation-verify.sh code-reviewer"
---
...
- Read-only analysis. Write tool retained ONLY for non-worklog QA artifacts (e.g., review report Markdown). Conviction worklogs MUST be emitted via the `devforgeai-validate emit-conviction-worklog` CLI subprocess (per Conviction Worklog Contract). Never use Write/Edit/Bash heredoc/redirect for worklog files.
...
When `tmp/<WORK_ID>/handoffs/phase-<NN>-code-reviewer-handoff.md` appears in the Task() prompt:
...
4. Embed a `subagent-result-v1` coverage attestation in the existing review report and conviction worklog payload:
   `{ "coverage_attestation": { "context_pack_path": "<handoff path>", "context_pack_consumed": true, "context_miss": [] } }`

The standards-loading workflow below applies only for dispatches WITHOUT a validated handoff.
...
### Persisted Worklog (`phase-04-code-reviewer.json`) — Rule-16 Required Shape

The conviction gate (`_validate_subagent_worklog` in `conviction.py`) enforces the
full Rule-16 field set on every entry in `findings[]`. Each entry MUST include
all 5 fields below. The `conviction-postrecord.sh` hook fails phase-record with
exit 2 if `line`, `severity`, or `remediation` is missing or invalid (ADR-083).
  ...
  "findings": [
    {
      ...
      "file": "src/example.py",
      "line": 42,
      "description": "Concrete, non-aspirational description with at least ten words grounding the issue in observable evidence from the file.",
      "remediation": "Concrete, non-aspirational remediation with at least ten words describing the exact corrective action the implementer should take."
    }
  ],
  "evidence": [...],
  "output": {...}
...
| `file` | yes (if present, must resolve under project root) | file-realness check |
| `line` | yes | positive integer (bool rejected) |
| `description` | yes | no forbidden phrases, ≥10 words |
| `remediation` | yes | non-empty string, no forbidden phrases, ≥10 words |
...
When a `Task(subagent_type="code-reviewer")` invocation returns "Prompt is too long", the orchestrator MUST follow the 3-step fallback protocol documented in `.claude/rules/workflow/subagent-prompt-overflow.md` (STORY-633). Do NOT attempt inline execution without first exhausting Step 1 (trim context) and Step 2 (split delegation).

This agent has documented overflow history per STORY-631:
- Phase: Phase 04 Refactor
- Tool uses before overflow: 19
