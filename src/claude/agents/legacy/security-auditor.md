...
tools: [Read, Grep, Glob, "Bash(devforgeai-validate:*)", "Bash(npm:audit)", "Bash(pip:check)", "Bash(dotnet:list package --vulnerable)"]
model: sonnet
color: green
...
When `tmp/<WORK_ID>/handoffs/phase-<NN>-security-auditor-handoff.md` appears in the Task() prompt:
...
4. Embed a `subagent-result-v1` coverage attestation in the existing security report and conviction worklog payload:
   `{ "coverage_attestation": { "context_pack_path": "<handoff path>", "context_pack_consumed": true, "context_miss": [] } }`

The context/reference loading workflow below applies only for dispatches WITHOUT a validated handoff.
...
### Persisted Worklog (`phase-04-security-auditor.json`) — Rule-16 Required Shape
  ...
  "findings": [
    {
      ...
      "line": 87,
      "description": "Password compared via plain equality; vulnerable to timing-side-channel attack disclosing credential length.",
      "remediation": "Replace equality check with constant-time comparator (hmac.compare_digest) and ensure password handling routes through the bcrypt verifier."
    }
  ],
  "evidence": [...],
  "output": {...}
