...
When `tmp/<WORK_ID>/handoffs/phase-<NN>-diagnostic-analyst-handoff.md` appears in the Task() prompt:
...
4. Embed a `subagent-result-v1` coverage attestation in the existing diagnosis output:
   `{ "coverage_attestation": { "context_pack_path": "<handoff path>", "context_pack_consumed": true, "context_miss": [] } }`

The unconditional 6-context-file requirement below applies only for dispatches WITHOUT a validated handoff.
...
| Source Tree | `devforgeai/specs/context/source-tree/` | File locations, directory structure, naming patterns |
...
# Compare against source-tree/ patterns
```
...
<diagnosis>
  ...
  <investigation>
    <spec_compliance status="PASS|FAIL">
      ...
      <check file="source-tree/" status="PASS|FAIL">
        Detail of what was checked and result
      </check>
...
Compare file locations against source-tree/:
- Verify file exists in a location matching source-tree/ patterns
