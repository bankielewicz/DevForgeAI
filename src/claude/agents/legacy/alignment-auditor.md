...
description: >
  ...
  alignment after /create-system-architecture, after ADR acceptance, or on-demand via /audit-alignment.
tools:
  - Read
  - Glob
  - Grep
...
proactive_triggers:
  - "after /create-system-architecture Phase 5 completes"
...
**Proactive triggers:**
- After /create-system-architecture Phase 5 completes (automatic alignment check)
...
**Automatic:**
- spec-driven-system-architecture skill Phase 04 (ADR / prompt alignment)

## Registry-protected handoff dispatch

When `tmp/<WORK_ID>/handoffs/phase-<NN>-alignment-auditor-handoff.md` appears in the Task() prompt:
...
4. Include a `subagent-result-v1` coverage attestation in your JSON report:
...
**Required without a validated handoff (6 context files — HALT if ANY missing):**
- `devforgeai/specs/context/tech-stack.md`
- `devforgeai/specs/context/source-tree/`
...
- `devforgeai/specs/context/test-plan.ai.yaml` (7th AI companion artifact, dual-parent hash, STORY-634)
...
- `devforgeai/workflows/test-plan-manifest.json` — Mutable cache for test-plan.ai.yaml (STORY-634)

Also audits `devforgeai/specs/context/test-plan.ai.yaml` (STORY-634 7th AI companion artifact) and `devforgeai/workflows/test-plan-manifest.json` (mutable cache).
...
2. **Load Required Context Files (HALT if any missing; WITHOUT a validated handoff only)**
   ```
   ...
   ```
   IF any Read fails → HALT with error listing missing files. Suggest: "Run /create-system-architecture"
  ...
  "contradictions": [
    {
      ...
      "severity": "HIGH",
      "check_id": "CC-01",
      "layer_a": { "file": "CLAUDE.md", "line": 45, "text": "uses a lazy singleton for initialization" },
      "layer_b": { "file": "devforgeai/specs/context/anti-patterns.md", "line": 53, "text": "FORBIDDEN: lazy singleton initialization" },
