...
hooks:
  Stop:
    - hooks:
        - type: command
          command: ".claude/hooks/observation-verify.sh agent-generator"
---
...
- **Framework context**: CLAUDE.md, tech-stack.md, source-tree/, coding-standards.md
...
7. **Pattern A marker bypass (ADR-081)** - Immediately before the Step 8 operational Write, Write an empty file at `tmp/_operational-edit-bypass.marker`. This satisfies the `pre-dual-path-write-guard.sh` PreToolUse hook for the operational `.claude/agents/{name}.md` Write. The marker MUST be fresh (< 30 seconds) and is consumed (deleted) by the hook on bypass — single-shot per marker. If Step 8 also generates a reference file at `.claude/agents/{name}/references/*.md`, write a SEPARATE fresh marker immediately before that Write too.
8. **Write subagent file** - Write() operation proceeds ONLY after template compliance validation passes AND the Step 7 marker has been written
...
**External References:**
- `devforgeai/specs/context/source-tree/` - File location constraints
