...
**Step 6.5: Capture Codebase Context Snapshot (NEW)**

```
# For each file path referenced in ANY recommendation:
FOR each unique file_path in all_recommendations.implementation.file:
  Read(file_path="{file_path}")

  Extract and store:
    - Module path (e.g., crate::state::phase_state)
    - All struct/enum definitions referenced by recommendations (full definitions with fields)
    - All function signatures being modified (with /// doc comments)
    - Relevant use/import statements

# Build MODULE_HIERARCHY_SUBSET:
  Show only the modules involved in recommendations
  Show dependency direction (→ means "depends on")
  Example: commands/phase.rs → state/phase_state.rs → crypto/state_file.rs

# Build TYPE_DEFINITIONS:
  For each struct/enum referenced:
    Include full definition with all pub fields
    Include derive macros
    Include doc comments

# Build FUNCTION_SIGNATURES:
  For each function being modified:
    Include full signature (pub fn name(args) -> ReturnType)
    Include /// doc comments
    Do NOT include function body (that goes in recommendation's Current Code Context)

# Capture Architecture Constraints:
FOR each recommendation that modifies .rs files:
  ...
  Extract constraints that apply:
    - Layer dependency rules for modified module
    - Anti-patterns that the change must avoid
    - Coding standards the change must follow

  Cite with line numbers: (Source: {file}, lines {N}-{M})
...
FOR each recommendation (sorted by priority, then dependency order):
  ...
  # Use structured header format (MUST match consumer parser pattern)
  # Consumer expects: ^### REC-[0-9]+: PRIORITY - Title
  Write: "### REC-{rec.id}: {rec.priority} - {rec.title}"
  Write: ""
  Write: "**Addresses:** Why #{rec.why_number} — \"{rec.why_quote}\""
  Write: "**Conditional:** {rec.conditional_type}: {rec.condition}"

  # Include ALL mandatory subsections from recommendation-template.md:
  Write: "#### Evidence Traceability"
  Write: "#### Conditional vs Unconditional"
  Write: "#### Problem Addressed"
  Write: "#### Current Code Context"  ← [MANDATORY for code changes]
  Write: "#### Proposed Change"
  Write: "#### Architecture Constraints"  ← [MANDATORY for .rs files]
  Write: "#### Rationale"
  Write: "#### Test Specification"  ← [MANDATORY — table format]
  Write: "#### Effort Estimate"  ← MUST include **Time:** N hours
  Write: "#### Success Criteria"  ← MUST include - [ ] checkboxes
  Write: "#### Impact"

  # Per-recommendation validation:
  IF rec modifies code AND rec.current_code_context is empty:
    HALT: "REC-{rec.id} modifies code but has no Current Code Context"
  IF rec.why_number is empty:
    HALT: "REC-{rec.id} has no Evidence Traceability (missing Why # linkage)"
  IF rec.test_specification has no table rows:
    HALT: "REC-{rec.id} has no Test Specification table"
  IF rec.effort_hours is empty:
    HALT: "REC-{rec.id} has no Effort Estimate hours"
  IF rec.success_criteria has no checkboxes:
    HALT: "REC-{rec.id} has no Success Criteria checkboxes"
```
...
**Step 3.5: Validate Self-Containedness (NEW)**

```
SELF_CONTAINEDNESS_CHECKLIST:

Document-level:
- [ ] Codebase Context Snapshot section present and non-empty
      (IF recommendations touch code files)
- [ ] At least one type definition or function signature quoted
      (IF recommendations touch .rs files)
- [ ] Applicable Architecture Constraints section present
      (IF recommendations touch .rs files)
- [ ] 5 Whys each have inline **Evidence:** citation
...
FOR each REC-N:
  ...
  - [ ] Has "#### Test Specification" with markdown table (not prose)
  - [ ] Has "**Time:** {N} hours" in Effort Estimate (parseable by consumer)
  - [ ] Has "#### Success Criteria" with at least 1 "- [ ]" checkbox
  - [ ] Has "#### Architecture Constraints" with at least 1 citation
        (IF modifies .rs files, OR "N/A — no Rust code changes")

FRESH_SESSION_TEST (mental model):
  Imagine a fresh Claude session opens ONLY this RCA document.
  Can it:
  ...
  IF any answer is NO:
    ...
    Self-heal: Add the missing context from Phase 3/4/5 data
...
- [ ] All have "**Addresses:** Why #{N}" traceability
- [ ] All have Current Code Context (5-30 lines, or N/A for docs-only)
- [ ] All have Conditional/Unconditional marker
- [ ] All have Test Specification as markdown table
- [ ] All have Success Criteria with - [ ] checkboxes
- [ ] All code-modifying recs have Architecture Constraint citations
...
**Objective:** Convert actionable RCA recommendations into tracked work items (stories or technical debt entries), ensuring CRITICAL and HIGH findings are not lost after RCA completion.

**Failure Isolation:** Phase 7.5 failure does NOT prevent RCA completion. The RCA document is already written in Phase 5. Wrap all Phase 7.5 logic in error handling so that failures produce a warning but do not invalidate the RCA.

```
TRY:
    Execute Phase 7.5 steps below
CATCH any error:
    Display: "WARNING: Phase 7.5 (Recommendation-to-Story Pipeline) encountered an error. RCA document remains valid."
    Display: "Error: {error_message}"
    Continue to Phase 8 / Error Handling (non-blocking)
```

**Step 1: Filter Recommendations by Priority**

Collect all recommendations from the RCA document (Phase 5 output). Separate into actionable (CRITICAL, HIGH) and informational (MEDIUM, LOW).
...
IF len(actionable_recs) == 0:
    Display: "No CRITICAL/HIGH recommendations to process. Phase 7.5 complete."
    RETURN
```

**Step 2: Prompt User for Each Actionable Recommendation**

For each CRITICAL or HIGH recommendation, present three action options via AskUserQuestion:

```
FOR each rec in actionable_recs:
    Display:
        Priority: {rec.priority}
        ID: {rec.id}  (e.g., REC-1, REC-2)
        Title: {rec.title}
        Description: {rec.description}

    AskUserQuestion:
        "What action for {rec.id} ({rec.priority}): {rec.title}?"
        Options:
          1. "Create story now" — Will reference /create-stories-from-rca command
          2. "Add to technical debt register" — Appends to devforgeai/technical-debt-register.md
          3. "Skip" — Acknowledged, no action taken
```

**Step 3: Execute Selected Action**

```
IF user_choice == "Create story now":
    Display:
        "To create stories from this RCA, run:"
        "  /create-stories-from-rca RCA-{NNN}"
        ""
        "This command will generate story files from CRITICAL/HIGH recommendations."
    Note: The /create-stories-from-rca command already exists. Phase 7.5 reminds users to invoke it.
    Alternative: Use /create-story to create individual stories manually from recommendation details.

ELIF user_choice == "Add to technical debt register":
    # Append entry to devforgeai/technical-debt-register.md
    entry = """
    | {rec.id} | {rec.priority} | {rec.title} | Source: RCA-{NNN} REC-{N} | Open |
    """
    ...
    Display: "Added to technical debt register: {rec.title} (Source: RCA-{NNN} REC-{N})"

ELIF user_choice == "Skip":
    Display: "Skipped: {rec.id} - {rec.title} (acknowledged, no action taken)"
```
...
```
Display:
    "Phase 7.5 Complete — Recommendation Pipeline Results:"
...
**Phase 7.5 Output:**
```
Recommendation-to-Story Pipeline complete
CRITICAL/HIGH processed: {actionable_count}
MEDIUM/LOW skipped: {skipped_count} (informational only)
```
