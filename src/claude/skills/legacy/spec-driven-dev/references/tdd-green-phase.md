...
Task(
  ...
  - devforgeai/specs/context/source-tree/governance.json (file placement rules)
...
devforgeai-validate phase-append-reflection ${STORY_ID} \
  --workflow=dev --phase=03 \
  --what-happened="[Specific implementation failure: test name, error type, file:line]" \
  --why-it-failed="[Root cause: missing import, wrong return type, logic error]" \
  --how-to-improve="[Actionable fix: check tech-stack.md, verify return types, review AC]" \
...
```json
{
  "id": "REF-03-1706621500000-a1b2c3d4",
  "phase": "03",
  "failed": true,
  "iteration": 1,
  "reflection": {
