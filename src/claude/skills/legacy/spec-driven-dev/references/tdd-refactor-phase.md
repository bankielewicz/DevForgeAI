...
Task(
  ...
  **Treelint Integration:** This subagent is Treelint-enabled. For AST-aware code smell detection on supported languages, use Treelint patterns from `.claude/agents/references/treelint-search-patterns.md`. Fallback to Grep when unavailable."
)
```
...
Task(
  ...
  **Treelint Integration:** This subagent is Treelint-enabled. For AST-aware pattern validation on supported languages, use Treelint patterns from `.claude/agents/references/treelint-search-patterns.md`. Fallback to Grep when unavailable."
)
```
...
**Invoke spec-driven-qa skill in light mode:**
...
**Note:** The spec-driven-qa skill will detect "light" mode from conversation context. See SKILL.md for mode detection pattern.
...
**Purpose:** Capture verbal reflections when Phase 04 fails to enable contextual learning for retry attempts.

**When to Execute:** AFTER phase validation fails (tests broken during refactoring), BEFORE retry attempt begins.
...
```json
reflection = {
  "id": "REF-04-{epoch_ms}-{uuid8}",
  "phase": "04",
  "failed": true,
  "iteration": "<computed server-side by the CLI, scoped per-phase>",
  "reflection": {
    "what_happened": "[Specific refactoring failure: broken test name, error type, file:line]",
    "why_it_failed": "[Root cause: moved code without updating imports, renamed without grep]",
    "how_to_improve": "[Actionable fix: grep for usages before move, run tests after each change]"
  },
  "timestamp": "{ISO8601_format}"
}
```

**Step 2: Append the Reflection via the CLI**

`devforgeai-validate` is the only writer of the phase-state checkpoint. It computes `id`, `iteration`, and `timestamp` server-side, with `iteration` scoped per-phase — so no manual counting is required here:

```bash
devforgeai-validate phase-append-reflection ${STORY_ID} \
  --workflow=dev --phase=04 \
  --what-happened="[Specific refactoring failure: broken test name, error type, file:line]" \
  --why-it-failed="[Root cause: moved code without updating imports, renamed without grep]" \
  --how-to-improve="[Actionable fix: grep for usages before move, run tests after each change]" \
  --failed --project-root=. --format=json
```

**Why the CLI rather than a direct file edit:** the former instruction here was a placeholder tool call carrying no `old_string`/`new_string` — it was never executable. Its companion counter read `phase_state.reflections OR []`, which always evaluated to `[]` because nothing ever created that key, pinning the documented iteration at 1 forever. This migration repairs a feature that never worked; it is not a pure relocation.

**Step 3: Retry Displays the Reflection Just Recorded**

The Step 2 `--format=json` output echoes the computed iteration and the recorded reflection under its `reflection` key — use it directly instead of re-reading the checkpoint.

BEFORE starting retry:

```
result = <parsed JSON from the phase-append-reflection call above>
iteration = result.reflection.iteration

IF iteration > 1:
  latest_reflection = result.reflection

  Display: ""
  Display: "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  Display: "  Recorded Reflection (Iteration {iteration})"
  Display: "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
**Example Reflection (Phase 04):**

```json
{
  "id": "REF-04-1706621600000-a1b2c3d4",
  "phase": "04",
  "failed": true,
  "iteration": 1,
  "reflection": {
    "what_happened": "Refactoring broke test_order_validation: NameError - 'validate_order' undefined",
    "why_it_failed": "Extract method refactoring moved validation but didn't update import in test file",
    "how_to_improve": "After extract method, grep for all usages and update imports; run tests after each change"
  },
...
**spec-driven-qa (light mode):** [NEW - Step 5]
...
- [ ] **Step 5:** Light QA validation executed [MANDATORY]
  - Verification: spec-driven-qa skill invoked in light mode
...
- `spec-driven-qa` skill (light mode) - Intermediate quality gate
