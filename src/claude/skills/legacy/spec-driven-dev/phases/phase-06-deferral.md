...
```
Read(file_path=".claude/skills/spec-driven-dev/references/technical-debt-register-workflow.md")
```
...
### Step 7: Update AC Checklist (Deferral Items) [CONDITIONAL: ac_checklist_updates_enabled]
...
```bash
devforgeai-validate update-ac-checklist --story-file=${STORY_FILE} --category=deferral
```
VERIFY: Exit code 0 = items updated (or no deferral items to update).
...
```bash
devforgeai-validate write-observation --story=${STORY_ID} --phase=06 --observations='${OBS_JSON}' --project-root=${PROJECT_ROOT}
```
OBS_JSON must be a JSON array. Each object requires 7 fields: `id` (obs-STORY-NNN-06-001), `phase` (06), `source` (agent name), `category` (friction|success|pattern|gap|idea|bug), `note` (description), `severity` (low|medium|high), `timestamp` (ISO 8601).
