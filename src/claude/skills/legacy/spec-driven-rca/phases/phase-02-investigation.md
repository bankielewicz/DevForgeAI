...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=02 --step=02.1
```
...
This step is MANDATORY for BOTH modes. The diagnostic-analyst runs in isolated read-only context using a scoped handoff for constitutional rules. The handoff is mandatory for the registry-protected `(spec-driven-rca, 02, diagnostic-analyst)` dispatch.

Build the handoff before dispatch:

```bash
DIAGNOSTIC_CHANGED_FILES_PHASE02="tmp/${SESSION_ID}/changed-files-phase02-diagnostic-analyst.txt"
mkdir -p "$(dirname "${DIAGNOSTIC_CHANGED_FILES_PHASE02}")"
: > "${DIAGNOSTIC_CHANGED_FILES_PHASE02}"
HANDOFF_OUTPUT_DIAGNOSTIC_ANALYST=$(devforgeai-validate build-subagent-handoff ${SESSION_ID} \
    --workflow=spec-driven-rca --phase=02 \
    --subagent=diagnostic-analyst \
    --changed-files-file="${DIAGNOSTIC_CHANGED_FILES_PHASE02}" \
    --project-root=${PROJECT_ROOT})
HANDOFF_PATH_DIAGNOSTIC_ANALYST=$(echo "$HANDOFF_OUTPUT_DIAGNOSTIC_ANALYST" | jq -r '.handoff_md_path')
```

VERIFY: `$HANDOFF_PATH_DIAGNOSTIC_ANALYST` is non-empty and the referenced handoff file exists. On failure, HALT; do not dispatch `diagnostic-analyst` without the handoff.

```
Task(
    subagent_type="diagnostic-analyst",
    description="Investigate failure against constitutional context files",
    prompt="Handoff: ${HANDOFF_PATH_DIAGNOSTIC_ANALYST}

    Read the handoff first. Use its context_pack for covered constitutional domains instead of re-reading full context files. Return the existing structured diagnosis and embed subagent-result-v1 coverage_attestation with context_pack_consumed=true and context_miss=[].

    Analyze failure artifacts for spec drift and constraint violations.
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=02 --step=02.2
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=02 --step=02.3
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=02 --step=02.4
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=02 --step=02.5
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=02 --step=02.6
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=02 --step=02.7
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=02 --step=02.8
```
