...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=05 --step=05.1
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=05 --step=05.2
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=05 --step=05.3
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=05 --step=05.4
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=05 --step=05.5
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=05 --step=05.6
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=05 --step=05.7
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=05 --step=05.8
```
...
**Hook-enforced (ADR-074).** `pre-rca-recommendation-write.sh` validates every REC-N block at Write/Edit time and blocks the operation (exit 2) if any Required/Rendered tier field is missing. The fields the hook enforces — and that the recommendation template requires — are:

| Tier | Field | Source |
|---|---|---|
| Required | `**Addresses:** Why #N — "..."` | 5-Whys traceability |
| Required | `**Evidence Files:** ...` | 5-Whys traceability |
| Rendered | `**Type:** Unconditional\|Conditional` | conditional/unconditional |
| Rendered | `### Test Specification` (markdown table) | test_specification |
| Rendered | `**Time:** N hours` | effort_hours |
| Rendered | `- [ ]` success-criteria checklist | success_criteria |
| Context  | `### Current Code Context` (or `N/A — documentation-only change`) | current_code_context |
| Context  | `### Architecture Constraints` (or `N/A — no Rust code changes`) | architecture_constraints |

Per `references/recommendation-contract.md`, Required and Rendered tiers MUST be present. Context-tier fields are emitted with `N/A` when inapplicable.

The composer (or the inline writer for tactical mode) does not need to perform this validation manually — a Write/Edit that omits a Required/Rendered field is blocked at the tool layer before disk write. If the hook blocks, the missing field listed in the block message MUST be added to the REC-N record before retrying.

### VERIFY

- Every REC-N block written in Step 05.6 passed `pre-rca-recommendation-write.sh` (no hook block events in the session log)
- Recommendation count from Step 05.8 equals number of REC-N blocks present in the document

### RECORD

```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=05 --step=05.9
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=05 --step=05.10
```
