...
```
# No required references for this phase. Phase 08 generates the completion
# report, determines next steps via priority counts accumulated in Phases
# 05-06, and orchestrates the recommendation hand-off pipeline by delegating
# to the github-incident-from-rca skill or editing the runtime
# technical-debt-register.md (non-declared categories). No skill references,
# context files, memory, or stories are loaded.
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=08 --step=08.1
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=08 --step=08.2
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=08 --step=08.3
```

---

## Step 08.4: Recommendation Hand-Off Pipeline [CONDITIONAL]
...
CATCH any error:
    Display: "WARNING: Recommendation hand-off pipeline encountered an error."
...
**Bulk hand-off prompt (ADR-074):** Before per-rec triage, ask the user which downstream artifact (if any) should be produced for all actionable recommendations at once. This is the in-flow hand-off to the sibling skill — `/rca` itself never calls `gh issue create`; that responsibility stays exclusive to `github-incident-from-rca`.

```
AskUserQuestion:
    Question: "{count} CRITICAL/HIGH recommendations generated. How should they be handed off?"
    Header: "Bulk Action"
    Options:
        - label: "Post all as GitHub issues"
          description: "Invoke github-incident-from-rca to draft and post issues for every CRITICAL/HIGH recommendation. The incident skill owns approval/preview/posting; /rca never calls gh directly."
        - label: "Per-recommendation triage"
          description: "Review each recommendation individually: create story, add to debt register, or skip."
        - label: "Defer all"
          description: "Take no downstream action now. The RCA document remains the record."
    multiSelect: false

IF user_choice == "Post all as GitHub issues":
    Display: "Invoking github-incident-from-rca with RCA-{RCA_NUMBER} as input..."
    Skill(command="github-incident-from-rca")
    # The incident skill reads the RCA document, drafts issue bodies per its own
    # rules (Github-incident-template.md), prompts for approval, and posts via
    # `gh issue create`. Control returns here when it completes.
    Display: "Incident hand-off complete. Proceeding to final completion."
    Skip to Step 08.5

ELIF user_choice == "Defer all":
    Display: "All {count} CRITICAL/HIGH recommendations deferred. Document: {rca_file}"
    Skip to Step 08.5

# Otherwise (Per-recommendation triage) — fall through to the per-rec loop below.
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=08 --step=08.4
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=08 --step=08.5
```
