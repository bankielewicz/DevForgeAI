...
IF Read fails: HALT -- "Phase 00 reference files not loaded."
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=00 --step=00.1
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=00 --step=00.2
```
...
Set `MODE`, `ACTIVE_PHASES`, **and the `WORKFLOW` key**. The workflow key selects the
phase-state schema and is used by EVERY `phase-init`/`phase-check`/`phase-record`/
`phase-complete` call in this skill (`--workflow=${WORKFLOW}`). Strategic uses a distinct
key whose `valid_phases` omit phase 03, so the shared sequential gate makes `02→04` legal
by construction (OBS-1) — strategic never traverses phase 03:
```
IF MODE == "tactical":
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=00 --step=00.3
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=00 --step=00.4
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=00 --step=00.5
```
...
```
Extract `current_phase` to determine where to resume. In addition to reading `current_phase`
from this yaml checkpoint, **rehydrate `composition_state` from `tmp/.rca-composition-${SESSION_ID}.json`**
when that file exists (strategic mode, composition phases 04-07) — it is the authoritative
latest merged state. The yaml checkpoint still supplies `current_phase`.
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=00 --step=00.6
```
...
```bash
devforgeai-validate phase-record ${SESSION_ID} --workflow=${WORKFLOW} --phase=00 --step=00.7
```
