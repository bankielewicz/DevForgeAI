...
```
Read(file_path="references/template-format-specification.md")
```
...
SWITCH $FEEDBACK_TYPE:
  CASE "conversation":
    Read(file_path="references/adaptive-questioning.md")
    Read(file_path="references/feedback-question-templates.md")

  CASE "summary":
    Read(file_path="references/feedback-persistence-guide.md")

  CASE "metrics":
    Read(file_path="references/feedback-export-formats.md")

  CASE "checklist":
    Read(file_path="references/feedback-question-templates.md")

  CASE "ai_analysis":
    Read(file_path="references/feedback-analysis-patterns.md")
    Read(file_path="references/feedback-question-templates.md")

  CASE "config":
    Read(file_path="references/user-customization-guide.md")

  CASE "search":
    Read(file_path="references/feedback-search-help.md")
...
IF $FEEDBACK_TYPE in ["conversation", "summary", "metrics", "checklist", "ai_analysis"]:
  ...
  # Load the template
  Read(file_path="templates/${template_file}")

ELSE IF $FEEDBACK_TYPE in ["config", "search", "export", "import"]:
...
ELSE:
  VERIFY: $FEEDBACK_TYPE is in ["config", "search", "export", "import"]
```
