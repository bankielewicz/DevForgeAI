...
- **REQUIRED ARTIFACTS:** Feedback data captured — questions answered, summary generated, metrics collected, analysis produced, or operation result
...
References for this phase were loaded in Phase 02 (Step 2.2). Phase 02's fresh load is authoritative; do not reload here.
...
**EXECUTE:**
```
Apply the adaptive questioning algorithm from references/adaptive-questioning.md:

1. Map (status, duration) → category using the table in adaptive-questioning.md §"Category Selection"
   (success_standard | success_long_running | failure_focused | partial_mixed | generic_fallback)
2. Pull category-matched questions from feedback-question-templates.md
3. Apply variable substitution per the {placeholder → source} table in adaptive-questioning.md §"Variable Substitution"
   (operation_type, duration, story_id, error_message, failed_todo, longest_phase, …)
