...
```bash
devforgeai-validate phase-check ${RESEARCH_ID} --workflow=research --from=02 --to=03 --project-root=. 2>&1
```
...
```
Read(file_path=".claude/skills/spec-driven-research/references/citation-standards.md")
```
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=03 --step=3.1 --project-root=. 2>&1
```
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=03 --step=3.2 --project-root=. 2>&1
```
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=03 --step=3.3 --project-root=. 2>&1
```
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=03 --step=3.4 --project-root=. 2>&1
```
...
```bash
devforgeai-validate phase-complete ${RESEARCH_ID} --workflow=research --phase=03 --checkpoint-passed --project-root=. 2>&1
```
