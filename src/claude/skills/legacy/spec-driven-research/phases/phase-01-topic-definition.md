...
```bash
devforgeai-validate phase-check ${RESEARCH_ID} --workflow=research --from=00 --to=01 --project-root=. 2>&1
```
...
```
Read(file_path=".claude/skills/spec-driven-research/references/parameter-extraction.md")
```
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=01 --step=1.1 --project-root=. 2>&1
```
---
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=01 --step=1.2 --project-root=. 2>&1
```
---
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=01 --step=1.3 --project-root=. 2>&1
```
---
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=01 --step=1.4 --project-root=. 2>&1
```
---
...
```bash
devforgeai-validate phase-complete ${RESEARCH_ID} --workflow=research --phase=01 --checkpoint-passed --project-root=. 2>&1
```