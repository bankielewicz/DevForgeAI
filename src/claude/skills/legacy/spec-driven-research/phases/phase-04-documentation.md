...
```bash
devforgeai-validate phase-check ${RESEARCH_ID} --workflow=research --from=03 --to=04 --project-root=. 2>&1
```
...
```
Read(file_path=".claude/skills/spec-driven-research/references/research-workflow.md")
```
...
**EXECUTE:**
```
template = Read(file_path=".claude/skills/spec-driven-research/assets/templates/research-template.md")
```
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=04 --step=4.1 --project-root=. 2>&1
```
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=04 --step=4.2 --project-root=. 2>&1
```
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=04 --step=4.3 --project-root=. 2>&1
```
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=04 --step=4.4 --project-root=. 2>&1
```
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=04 --step=4.5 --project-root=. 2>&1
```
...
```bash
devforgeai-validate phase-complete ${RESEARCH_ID} --workflow=research --phase=04 --checkpoint-passed --project-root=. 2>&1
```
