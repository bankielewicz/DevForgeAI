...
```bash
devforgeai-validate phase-check ${RESEARCH_ID} --workflow=research --from=04 --to=05 --project-root=. 2>&1
```
...
```
# No required references for this phase. Self-contained logic — operates on
# dynamically discovered runtime artifacts (epic/story/ADR files matched via
# Glob from user-supplied IDs). The Step 5.1/5.2 Reads of those discovered
# files are runtime cross-references, not pre-declared reference loads.
```
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=05 --step=5.1 --project-root=. 2>&1
```
...
"""
    ...
    create_adr = AskUserQuestion:
      ...
      Options:
        - label: "Yes, create ADR now"
          description: "Create the ADR via /create-system-architecture, referencing this research"
    ...
    IF "Yes, create ADR now":
      Display: f"To create the ADR, run: /create-system-architecture (and reference {RESEARCH_ID})"
...
**RECORD:**
```bash
devforgeai-validate phase-record ${RESEARCH_ID} --workflow=research --phase=05 --step=5.2 --project-root=. 2>&1
```
...
```bash
devforgeai-validate phase-complete ${RESEARCH_ID} --workflow=research --phase=05 --checkpoint-passed --project-root=. 2>&1
```
