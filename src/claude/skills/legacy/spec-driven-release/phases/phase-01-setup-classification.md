...
IF $WORKFLOW_KEY == "release-package": STOP — this phase is a zero-step gate-pass.
  Display: "Release-package route: phase 01 is a gate-pass — no STORY_ID or QA validation."
  Skip Reference Loading and ALL mandatory steps 1.1–1.12.
  Proceed directly to Exit Gate.
```

For the release-package workflow, phases 01/02/03 have `steps_required=[]` — they are
intentional gate-passes (ISSUE-566). Steps 1.4 (STORY_ID extraction) and 1.5 (QA-Approved
validation) MUST NOT execute; they false-HALT when there is no story file (ISSUE-585).
