...
Build the registry-protected Phase 05 handoff for `security-auditor`. This
audit dispatch reads release and security context and returns findings, so the
handoff is floor-only.

```bash
mkdir -p tmp/${STORY_ID}
: > tmp/${STORY_ID}/release-phase05-security-auditor-handoff-changed-files.txt

HANDOFF_OUTPUT_SECURITY_AUDITOR=$(devforgeai-validate build-subagent-handoff ${STORY_ID} \
    --workflow=spec-driven-release \
    --phase=05 \
    --subagent=security-auditor \
    --changed-files-file=tmp/${STORY_ID}/release-phase05-security-auditor-handoff-changed-files.txt \
    --project-root=. 2>&1)
HANDOFF_PATH_SECURITY_AUDITOR=$(echo "$HANDOFF_OUTPUT_SECURITY_AUDITOR" | jq -r '.handoff_md_path')
```

VERIFY before dispatch:
- `$HANDOFF_PATH_SECURITY_AUDITOR` is non-empty and exists.
- `tmp/${STORY_ID}/handoffs/phase-05-security-auditor-handoff.manifest.json` exists.
- The manifest has `context_pack.coverage_matrix == {}` and `unresolved == []`.

```
Agent(subagent_type="security-auditor",
      prompt="Handoff: ${HANDOFF_PATH_SECURITY_AUDITOR}
              ...
              Pre-production security audit for ${STORY_ID}.
              ...
              Include a subagent-result-v1 coverage_attestation with context_pack_path='${HANDOFF_PATH_SECURITY_AUDITOR}', context_pack_consumed=true, and context_miss=[].")
...
Build the registry-protected Phase 05 handoff for `deployment-engineer`. This
deployment dispatch operates on the current build artifacts and deployment
target, so the handoff is floor-only unless this phase is modified to write
repository deployment files.

```bash
mkdir -p tmp/${STORY_ID}
: > tmp/${STORY_ID}/release-phase05-deployment-engineer-handoff-changed-files.txt

HANDOFF_OUTPUT_DEPLOYMENT_ENGINEER=$(devforgeai-validate build-subagent-handoff ${STORY_ID} \
    --workflow=spec-driven-release \
    --phase=05 \
    --subagent=deployment-engineer \
    --changed-files-file=tmp/${STORY_ID}/release-phase05-deployment-engineer-handoff-changed-files.txt \
    --project-root=. 2>&1)
HANDOFF_PATH_DEPLOYMENT_ENGINEER=$(echo "$HANDOFF_OUTPUT_DEPLOYMENT_ENGINEER" | jq -r '.handoff_md_path')
```

VERIFY before dispatch:
- `$HANDOFF_PATH_DEPLOYMENT_ENGINEER` is non-empty and exists.
- `tmp/${STORY_ID}/handoffs/phase-05-deployment-engineer-handoff.manifest.json` exists.
- The manifest has `context_pack.coverage_matrix == {}` and `unresolved == []`.

```
Agent(subagent_type="deployment-engineer",
      prompt="Handoff: ${HANDOFF_PATH_DEPLOYMENT_ENGINEER}
              Read the handoff first. Use its context_pack.coverage_matrix and role-domain rules as the context source. If a required domain or artifact is absent, return H-CONTEXT-MISS and stop.

              Deploy ${STORY_ID} to PRODUCTION environment.
              ...
              Include a subagent-result-v1 coverage_attestation with context_pack_path='${HANDOFF_PATH_DEPLOYMENT_ENGINEER}', context_pack_consumed=true, and context_miss=[].")
