...
STEP COUNT: 8 mandatory steps (3.1–3.7 + 3.8 InfoSec gate; story route only — the release-package route runs the InfoSec gate in phase-09 Step 9.0.5)

---

## Package-Route Gate [READ FIRST]

```
IF $WORKFLOW_KEY == "release-package": STOP — this phase is a zero-step gate-pass.
  Display: "Release-package route: phase 03 is a gate-pass — no QA gate or deployment strategy needed."
  Skip Reference Loading and ALL mandatory steps 3.1–3.7.
  Proceed directly to Exit Gate.
```

For the release-package workflow, phases 01/02/03 have `steps_required=[]` — they are
intentional gate-passes (ISSUE-566). Steps 3.2 (load story file), 3.3 (QA Approval Gate),
and 3.5 (dependency gate) MUST NOT execute; they false-HALT without a story file (ISSUE-585).
...
### Step 3.8: InfoSec release-gate (ADR-150)

EXECUTE: A security review is a release prerequisite at the **project** level (the whole project is the review target, not this story file). Run the gate against the project root:
```bash
devforgeai-validate validate-infosec-gate --project-root=.
```
Act on the exit code:
- **0 (PASS)** — a valid, current review exists with no Open Critical/High finding. Proceed.
- **2 (BLOCK)** — no review exists, the envelope is invalid, or an Open Critical/High finding is present. **HALT.** Run `/infosec` and resolve (or accept) the Critical/High findings, then re-run this step.
- **3 (STALE)** — the review is clean but its `git_baseline` is not an ancestor of HEAD. Use AskUserQuestion to let the user re-run `/infosec` or confirm the existing review still applies (options: "Re-run /infosec (Recommended)" / "Proceed with existing review").

This step is on the `release` (story) route only. The `release-package` route runs the identical gate in phase-09 Step 9.0.5 (phase-03 is a zero-step gate-pass on that route).
VERIFY: `validate-infosec-gate` exited 0, OR exited 3 and the user chose to proceed. A BLOCK (exit 2) never advances.
RECORD: `devforgeai-validate phase-record ${STORY_ID} --workflow=release --phase=03 --step=3.8 --project-root=.`
