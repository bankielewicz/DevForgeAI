...
| `/create-system-architecture` | Generate architecture constraint files |
| `/create-solution-architecture` | Create epic with feature breakdown |
...
| `devforgeai/specs/context/source-tree/` | Required file structure |
...
**Important:** If context files don't exist yet, run `/create-system-architecture` before implementation.
...
*Note: Tech stack will be finalized during /create-system-architecture phase.*
...
2. **If no stories:** Run `/ideate` then `/create-solution-architecture` then `/create-story`
3. **If no context files:** Run `/create-system-architecture` first
