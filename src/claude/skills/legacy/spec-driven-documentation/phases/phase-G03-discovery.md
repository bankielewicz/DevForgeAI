...
ELIF $MODE == "brownfield":
    ...
    : > tmp/${SESSION_ID}/doc-g03-code-analyzer-handoff-changed-files.txt

    HANDOFF_OUTPUT_CODE_ANALYZER=$(devforgeai-validate build-subagent-handoff ${SESSION_ID} \
      --workflow=spec-driven-documentation \
      --phase=G03 \
      --subagent=code-analyzer \
      --changed-files-file=tmp/${SESSION_ID}/doc-g03-code-analyzer-handoff-changed-files.txt \
      --project-root=. 2>&1)
    HANDOFF_PATH_CODE_ANALYZER=$(echo "$HANDOFF_OUTPUT_CODE_ANALYZER" | jq -r '.handoff_md_path')

    VERIFY: HANDOFF_PATH_CODE_ANALYZER exists, is non-empty, and its manifest has context_pack.coverage_matrix == {} and unresolved == [].

    Task(
        subagent_type="code-analyzer",
        description="Analyze codebase for documentation",
        prompt="Handoff: ${HANDOFF_PATH_CODE_ANALYZER}
        ...
        Include a subagent-result-v1 coverage_attestation with context_pack_path='${HANDOFF_PATH_CODE_ANALYZER}', context_pack_consumed=true, and context_miss=[]."
    )
