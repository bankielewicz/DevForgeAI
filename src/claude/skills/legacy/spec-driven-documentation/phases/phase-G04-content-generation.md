...
FOR each doc_type in TYPES:
    ...
    : > tmp/${SESSION_ID}/doc-g04-documentation-writer-${doc_type}-handoff-changed-files.txt
    ...
    HANDOFF_OUTPUT_DOCUMENTATION_WRITER=$(devforgeai-validate build-subagent-handoff ${SESSION_ID} \
      ...
      --changed-files-file=tmp/${SESSION_ID}/doc-g04-documentation-writer-${doc_type}-handoff-changed-files.txt \
      --project-root=. 2>&1)
    ...
    Task(
        ...
        Generate a SECTION (not a full document) for insertion into an existing
        framework document.
