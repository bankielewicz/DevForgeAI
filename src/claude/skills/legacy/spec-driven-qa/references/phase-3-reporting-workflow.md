...
**2. Generate QA Report (mode-dependent path):**

Both modes emit a report to DISTINCT paths so a deep (`/qa`) run never overwrites the dev-time (light, `/dev`) report (Issue #717): light → `{STORY-ID}-qa-report.dev.md`, deep → the canonical `{STORY-ID}-qa-report.md`.
```
report_path = ("devforgeai/qa/reports/{STORY-ID}-qa-report.dev.md"
               if mode == "light"
               else "devforgeai/qa/reports/{STORY-ID}-qa-report.md")
Write(file_path=report_path, content=formatted_report)
