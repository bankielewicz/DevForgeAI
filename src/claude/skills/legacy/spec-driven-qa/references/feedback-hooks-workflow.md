...
# Result:
# check-hooks --status=failure → exit 0 (trigger)
# check-hooks --status=success → exit 1 (skip)
```
...
**Usage:**
```bash
devforgeai-validate check-hooks --operation=qa --status=[success|failure|partial]
```
...
- Phase 6 in `spec-driven-qa` skill invokes feedback hooks
