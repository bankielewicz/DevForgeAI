...
> **Phase mapping:** This file uses legacy step numbering (4.1-4.5.5). In SKILL.md v3.0, Cleanup is **Phase 6** with steps 6.1-6.5.5. The step content is identical — only the numbering prefix differs.

**Purpose:** Release locks, invoke feedback hooks, display final summary, and persist QA deliverables before PR merge/delete cleanup.
...
### Step 4.5.5: Persist QA Deliverables Before Merge/Delete

**Purpose:** Commit the QA report, QA recommendations, and story status update before any PR squash-merge deletes the story branch.
...
**Behavior:**
- Existing PR flow: when story frontmatter has non-null `pr_number`, the command commits and pushes QA deliverables onto that PR branch and does not create a PR.
- Standalone flow: when `pr_number` is null, the command commits QA deliverables to the current branch, reuses an existing open PR for that branch when present, and creates exactly one PR only when none exists.
- The command uses normal `git commit`; it never bypasses hooks with `--no-verify`.
...
Expected output: empty. If any deliverable remains uncommitted, halt before PR merge/delete cleanup.
