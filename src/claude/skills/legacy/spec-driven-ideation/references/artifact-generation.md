...
Generate YAML-structured requirements.md from discovered requirements per F4 schema. Epic creation is delegated to the /create-solution-architecture workflow.
...
Phase 3 transforms all discovered requirements into a YAML-structured requirements.md document that feeds downstream DevForgeAI skills. This phase creates the primary output from ideation: a structured requirements specification for consumption by the architecture skill and /create-solution-architecture command.

**Duration:** 5-15 minutes
**Output:** YAML-structured requirements.md (per F4 schema), transition to /create-solution-architecture workflow
...
# UI Design Context (Optional — populated when project has UI components)
ui_design_context:
  has_mockup: {true|false|null}
  # true: Existing mockup available at mockup_path
  # false: No UI component in this project
  # null: Undecided — deferred from brainstorm
  mockup_type: "{html|react|vue|wpf|figma|none|null}"
  design_source_of_truth: "{path to design.md or null}"
  # If design.md exists, all downstream skills consume it
  aesthetic_direction: "{description or null}"
  ui_complexity: "{low|medium|high|null}"
  component_count_estimate: {N or null}
  responsive_required: {true|false|null}
  accessibility_level: "{WCAG AA|AAA|null}"
  design_decisions:
    - id: "UI-DD-001"
      decision: "{what UI decision was made}"
      rationale: "{why this approach}"
  screenshots:
    desktop: "{path to desktop.png or null}"
    tablet: "{path to tablet.png or null}"
    mobile: "{path to mobile.png or null}"
  # Populated when /create-design --extract --url was run
  # These flow downstream to stories, dev, qa as visual references
```
...
**F4 Schema Optional Fields:**
- `ui_design_context` - UI design metadata, mockup references, design source of truth path. Populated when project has UI components. All downstream skills check for this field.
...
Next Steps:
1. Run `/create-solution-architecture {project-name}` to create epic documents from requirements
2. Run `/create-system-architecture {project-name}` to create context files (greenfield)
3. Run `/create-sprint 1` to begin sprint planning
```

**Recommended Next Action (display-only, no auto-invocation):**

Run `/create-solution-architecture {project-name}`
...
the architecture skill. The user manually runs `/create-solution-architecture` when ready.
