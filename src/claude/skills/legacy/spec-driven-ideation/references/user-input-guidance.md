...
- `spec-driven-system-architecture` (context file creation, constraint validation)
- `spec-driven-design` (user requirement specification before design)
- `spec-driven-lifecycle` (feature decomposition, scope clarification)
...
1. Load this guidance at Phase 2 start via:
   Read(file_path=".claude/skills/spec-driven-ideation/references/user-input-guidance.md")  # Note: this file loads itself as an example for skill integration
...
### 5.3 Integration: spec-driven-system-architecture
...
2. For each context file being created, check for ambiguities:
   ...
   - source-tree/: Are file location rules clear? (ambiguous → clarify)
...
- source-tree/ specifies file location rules
- anti-patterns.md lists forbidden patterns with rationale

---

### 5.4 Integration: spec-driven-design
...
### 5.5 Integration: spec-driven-lifecycle
