...
```bash
devforgeai-validate phase-check ${IDEATION_ID} --workflow=ideation --from=06 --to=07 --project-root=. 2>&1
```
...
```bash
devforgeai-validate phase-record ${IDEATION_ID} --workflow=ideation --phase=07 --step=7.1 --project-root=. 2>&1
```
...
IF project_mode == "greenfield":
  ...
  IF context_count == 0:
    ...
    next_steps = [
      {
        order: 1,
        command: "/create-solution-architecture {project_name}",
    ...
    Recommended workflow:
      1. /create-solution-architecture {project_name} -- Generate epic from requirements
      2. /create-system-architecture -- Create architecture context files
...
ELSE IF project_mode in ["brownfield", "modernization"]:
  ...
  IF context_count >= 1:
    ...
    IF tech_stack_exists:
      ...
      # Deep validation is deferred to /create-solution-architecture or /create-system-architecture
...
```bash
devforgeai-validate phase-record ${IDEATION_ID} --workflow=ideation --phase=07 --step=7.2 --project-root=. 2>&1
```
...
```bash
devforgeai-validate phase-record ${IDEATION_ID} --workflow=ideation --phase=07 --step=7.3 --project-root=. 2>&1
```
...
options.append({
  label: "Run /create-solution-architecture {project_name}",
...
# Handle response
IF response == "Run /create-solution-architecture {project_name}":
  Display:
  "Run: /create-solution-architecture {project_name}
...
ELSE IF response == "Review the requirements first":
  ...
  After review, run:
    /create-solution-architecture {project_name}"
...
```bash
devforgeai-validate phase-record ${IDEATION_ID} --workflow=ideation --phase=07 --step=7.4 --project-root=. 2>&1
```
...
```bash
devforgeai-validate phase-record ${IDEATION_ID} --workflow=ideation --phase=07 --step=7.5 --project-root=. 2>&1
```
...
```bash
devforgeai-validate phase-complete ${IDEATION_ID} --workflow=ideation --phase=07 --checkpoint-passed --project-root=. 2>&1
```
...
| Conflict detection false positive | Brownfield conflict flagged incorrectly | User can choose "Run /create-solution-architecture" anyway. Conflicts are advisory. |
