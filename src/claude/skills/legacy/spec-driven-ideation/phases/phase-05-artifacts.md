...
```bash
devforgeai-validate phase-check ${IDEATION_ID} --workflow=ideation --from=04 --to=05 --project-root=. 2>&1
```
...
| Attribute | Value |
|-----------|-------|
| **PURPOSE** | Generate the inception PM-plan output file (`{project}-requirements.md`) in YAML per the F4 schema plus the WI-3 PM-plan sections, and write/merge the standalone idea-backlog artifact. This file is the primary deliverable of the entire ideation workflow and feeds into /create-solution-architecture and /create-system-architecture commands. |
| **REFERENCE** | `.claude/skills/spec-driven-ideation/references/artifact-generation.md` (299 lines), `.claude/skills/spec-driven-ideation/references/examples.md`, `.claude/skills/spec-driven-ideation/assets/templates/idea-backlog-template.md` |
| **STEP COUNT** | 7 mandatory steps (5.1, 5.2, 5.3, 5.4, 5.5, 5.5.5, 5.6) |
...
- [ ] compiled_data assembled from all completed_outputs (Phases 02-04), including the PM-plan registers (risk register, milestone roadmap, stakeholder register, assumptions, idea backlog)
- [ ] F4 schema YAML generated with all required sections
- [ ] Inception PM-plan sections generated (risks, roadmap, stakeholder_register, assumptions, idea_backlog_ref)
...
- [ ] Idea backlog written/merged to `devforgeai/specs/ideation/{project_name}-idea-backlog.md` (Step 5.5.5) when deferred ideas exist
...
Read(file_path=".claude/skills/spec-driven-ideation/assets/templates/idea-backlog-template.md")
```

IF any Read fails: HALT -- "Phase 05 cannot proceed without reference files and templates."

Do NOT rely on memory of previous reads. Load ALL FIVE references fresh every time this phase executes.
...
```bash
devforgeai-validate phase-record ${IDEATION_ID} --workflow=ideation --phase=05 --step=5.1 --project-root=. 2>&1
```
...
compiled_data = {
  ...
  # ===== Inception PM-plan registers (WI-3) =====
  # Risk register: brainstorm risks OR Phase-03 elicited risks (Step 3.7.5).
  risk_register: session.completed_outputs.risk_register or session.brainstorm_risks or [],
  # Milestone roadmap: brainstorm roadmap OR Phase-03 derived roadmap (Step 3.7.6).
  milestone_roadmap: session.completed_outputs.milestone_roadmap or session.brainstorm_roadmap or [],
  # Stakeholder register: brainstorm stakeholder registry (influence/interest/RACI).
  stakeholder_register: session.stakeholder_registry or [],
  # Assumptions register: brainstorm Phase-08 assumptions + ideation assumptions.
  assumptions_register: [],
  # Idea backlog: brainstorm DEF-NNN deferred ideas + ideation-session descoped ideas.
  idea_backlog: [],
...
# ===== Assemble the assumptions register (WI-3, G5) =====
# Merge brainstorm Phase-08 assumptions (session.brainstorm_assumptions) with
# ideation's own critical/Phase-03 assumptions (session.assumptions). Each
# becomes an ASMP-NNN item. Dedupe on normalized statement.
asmp_counter = 1
seen_assumption_statements = []
FOR each statement in (session.brainstorm_assumptions or []):
  IF normalize(statement) not in seen_assumption_statements:
    compiled_data.assumptions_register.append({
      id: "ASMP-" + zero_pad(asmp_counter, 3),
      statement: statement,
      validation_method: "To be validated during architecture phase",
      source: "brainstorm:{session.brainstorm_input.brainstorm_id}"
    })
    seen_assumption_statements.append(normalize(statement))
    asmp_counter += 1
FOR each statement in (session.assumptions or []):
  IF normalize(statement) not in seen_assumption_statements:
    compiled_data.assumptions_register.append({
      id: "ASMP-" + zero_pad(asmp_counter, 3),
      statement: statement,
      validation_method: "To be validated during architecture phase",
      source: "ideation:{IDEATION_ID}"
    })
    seen_assumption_statements.append(normalize(statement))
    asmp_counter += 1

# ===== Assemble the merged idea backlog (WI-2) =====
# Merge session.brainstorm_deferred (DEF-NNN ideas from the brainstorm) with
# session.completed_outputs.idea_backlog (DEF-NNN ideas descoped during the
# ideation conversation in Step 2.6). Dedupe on id first, then normalized
# description. The on-disk merge with any pre-existing backlog file happens in
# Step 5.5.5 -- this compiled list is the in-session union.
compiled_data.idea_backlog = []
seen_backlog_ids = []
seen_backlog_descriptions = []
FOR each idea in (session.brainstorm_deferred or []) + (session.completed_outputs.idea_backlog or []):
  IF idea.id in seen_backlog_ids:
    CONTINUE
  IF normalize(idea.description) in seen_backlog_descriptions:
    CONTINUE
  compiled_data.idea_backlog.append(idea)
  seen_backlog_ids.append(idea.id)
  seen_backlog_descriptions.append(normalize(idea.description))
...
"Compilation Summary:
  ...
  --- Inception PM-plan registers ---
  Risk Register: {len(compiled_data.risk_register)} risk(s)
  Milestone Roadmap: {len(compiled_data.milestone_roadmap)} phase(s)
  Stakeholder Register: {len(compiled_data.stakeholder_register)} stakeholder(s)
  Assumptions: {len(compiled_data.assumptions_register)} assumption(s)
  Idea Backlog: {len(compiled_data.idea_backlog)} deferred idea(s)"
```
...
```bash
devforgeai-validate phase-record ${IDEATION_ID} --workflow=ideation --phase=05 --step=5.2 --project-root=. 2>&1
```
...
# ============================
# INCEPTION PM-PLAN SECTIONS (WI-3)
# ============================
# These elevate the ideation output into a full inception PM plan.
# BOUNDED: no schedule durations, no resource/capacity loading, no budget
# breakdown -- those are ungrounded until the architecture skills exist.

# RISK REGISTER -- session.brainstorm_risks OR Phase-03 elicited risks.
yaml_frontmatter += """
# RISK REGISTER  (RISK-NNN -- carried from brainstorm or elicited in Phase 03)
risk_register:
"""
IF len(session.compiled_data.risk_register) > 0:
  FOR each risk in session.compiled_data.risk_register:
    yaml_frontmatter += """  - id: "{risk.id}"
    description: "{risk.description}"
    likelihood: "{risk.likelihood}"
    impact: "{risk.impact}"
    severity: "{risk.severity or 'Medium'}"
    mitigation: "{risk.mitigation or 'To be determined'}"
    owner: "{risk.owner or 'Unassigned'}"
"""
ELSE:
  yaml_frontmatter += """  []  # No risks identified (no brainstorm + none elicited).
"""

# MILESTONE ROADMAP -- session.brainstorm_roadmap OR Phase-03 derived roadmap.
yaml_frontmatter += """
# MILESTONE ROADMAP  (phased releases with build order + dependencies)
milestone_roadmap:
"""
IF len(session.compiled_data.milestone_roadmap) > 0:
  FOR each phase in session.compiled_data.milestone_roadmap:
    yaml_frontmatter += """  - phase: "{phase.phase}"
    items:
"""
    FOR each item in phase.items:
      yaml_frontmatter += """      - "{item}"
"""
    yaml_frontmatter += """    dependencies:
"""
    IF phase.dependencies is not null AND len(phase.dependencies) > 0:
      FOR each dep in phase.dependencies:
        yaml_frontmatter += """      - "{dep}"
"""
    ELSE:
      yaml_frontmatter += """      []
"""
    yaml_frontmatter += """    rationale: "{phase.rationale or ''}"
"""
ELSE:
  yaml_frontmatter += """  []  # No roadmap derived (no requirements to phase).
"""

# STAKEHOLDER REGISTER -- session.stakeholder_registry (PM depth: influence/RACI).
yaml_frontmatter += """
# STAKEHOLDER REGISTER  (influence / interest / RACI -- PM depth)
stakeholder_register:
"""
IF len(session.compiled_data.stakeholder_register) > 0:
  FOR each stk in session.compiled_data.stakeholder_register:
    yaml_frontmatter += """  - id: "{stk.id}"
    name_or_role: "{stk.name_or_role}"
    category: "{stk.category or 'Primary'}"
    influence: "{stk.influence or 'Medium'}"
    interest: "{stk.interest or 'Medium'}"
    raci: "{stk.raci or 'Consulted'}"
"""
ELSE:
  yaml_frontmatter += """  []  # No stakeholder registry (no brainstorm; see user_personas above).
"""

# ASSUMPTIONS REGISTER -- ASMP-NNN merged from brainstorm + ideation.
yaml_frontmatter += """
# ASSUMPTIONS  (ASMP-NNN -- assumptions that, if invalidated, become defects)
assumptions:
"""
IF len(session.compiled_data.assumptions_register) > 0:
  FOR each asmp in session.compiled_data.assumptions_register:
    yaml_frontmatter += """  - id: "{asmp.id}"
    statement: "{asmp.statement}"
    validation_method: "{asmp.validation_method}"
    source: "{asmp.source}"
"""
ELSE:
  yaml_frontmatter += """  []  # No assumptions recorded.
"""

# IDEA BACKLOG REFERENCE -- path to the standalone living idea-backlog file.
# The file itself is written/merged in Step 5.5.5; the path is computed here.
IF len(session.compiled_data.idea_backlog) > 0:
  idea_backlog_ref_value = "devforgeai/specs/ideation/{session.project_name}-idea-backlog.md"
ELSE:
  idea_backlog_ref_value = "N/A"
yaml_frontmatter += """
# IDEA BACKLOG REFERENCE  (standalone living artifact -- DEF-NNN items)
idea_backlog_ref: "{idea_backlog_ref_value}"
"""
...
- **Risk Register:** See `risks` -- RISK-NNN entries (carried from brainstorm or elicited)
...
3. Proceed to `/create-solution-architecture` with this requirements document
4. Run `/create-system-architecture` if greenfield project
...
# Inception PM-plan sections present in generated content (WI-3).
pm_plan_sections = [
  "risks:",
  "roadmap:",
  "stakeholder_register:",
...
FOR each section in pm_plan_sections:
  IF section NOT in requirements_content:
    HALT -- "Step 5.3: Inception PM-plan section '{section}' missing from generated YAML."

# The generated output SHOULD validate against the published PM->Architecture
# output contract: references/ideation-output.schema.json v1.0. The frontmatter
# keys above (scope, risks, roadmap, stakeholder_register, assumptions,
# constraints, success_criteria, idea_backlog_ref, source_brainstorm) map to the
# contract's properties. A strict validator is not run inline; this is a
# documented expectation for downstream architecture skills.
...
```bash
devforgeai-validate phase-record ${IDEATION_ID} --workflow=ideation --phase=05 --step=5.3 --project-root=. 2>&1
```
...
```bash
devforgeai-validate phase-record ${IDEATION_ID} --workflow=ideation --phase=05 --step=5.4 --project-root=. 2>&1
```
...
```bash
devforgeai-validate phase-record ${IDEATION_ID} --workflow=ideation --phase=05 --step=5.5 --project-root=. 2>&1
```
...
### Step 5.5.5: Write / Merge the Idea Backlog (WI-2)

**Condition:** Writes the standalone living idea-backlog artifact from
`session.compiled_data.idea_backlog` (the in-session union of brainstorm DEF-NNN
deferred ideas + ideation-session descoped ideas, assembled in Step 5.2).

This step is numbered **5.5.5** — BEFORE the Step 5.6 context-window check — so
the backlog is always written before any session-exit branch. (Step 5.6 may EXIT
the skill on "save and continue later"; a step appended after it would be
skipped on that path and the backlog would never be written.)

**EXECUTE:**
```
backlog_path = "devforgeai/specs/ideation/{session.project_name}-idea-backlog.md"

# Skip cleanly when there are no deferred ideas at all.
IF session.compiled_data.idea_backlog is null OR len(session.compiled_data.idea_backlog) == 0:
  Display: "No deferred ideas captured (brainstorm or ideation). Idea backlog not created."
  session.phases["05"].idea_backlog_written = false
  session.phases["05"].idea_backlog_path = "N/A"
  SKIP to RECORD below.

# Detect an existing backlog file (re-ideation / resume -> living artifact).
existing = Glob(pattern=backlog_path)

IF len(existing) > 0:
  # ---- APPEND-MERGE path: the file already exists. NEVER overwrite. ----
  Read(file_path=backlog_path)
  parsed_backlog = parse_yaml_frontmatter(file_content)
  merged_items   = parsed_backlog.idea_backlog or []
  source_sessions = parsed_backlog.source_sessions or []

  # ---- DEDUP RULE (WI-2 idea-loss fix) ----
  # The dedup UNIT is "the same idea", keyed ONLY on the normalized
  # `description` -- trim, lowercase, collapse internal whitespace.
  # `id` (DEF-NNN) is NOT a dedup key; it is only an identifier. Two
  # different ideas that happen to carry the same DEF-NNN id are an
  # id COLLISION, not a duplicate -- the colliding incoming idea MUST
  # still be kept, with a fresh id. Skipping an incoming idea purely
  # because its id matches an on-disk id loses ideas -- the exact
  # failure WI-2 exists to prevent. Never regress to an id-first skip.
  #
  # normalize(text) := collapse_whitespace(lowercase(trim(text)))
  #
  # Index existing items: descriptions are the dedup key; ids are
  # tracked only to detect collisions and to compute the next free id.
  existing_descriptions = [normalize(item.description) FOR item in merged_items]
  existing_ids          = [item.id FOR item in merged_items]

  appended_count = 0
  FOR each new_idea in session.compiled_data.idea_backlog:
    # --- Rule 1: same normalized description => same idea. ---
    IF normalize(new_idea.description) in existing_descriptions:
      # Do NOT append a duplicate. Keep the on-disk item -- it owns
      # the canonical id. Optionally advance the on-disk item's status
      # if the incoming status is more advanced (open -> promoted/rejected).
      on_disk = the merged_items entry whose normalize(description) matches
      IF on_disk.status == "open" AND new_idea.status in ["promoted", "rejected"]:
        on_disk.status = new_idea.status
      CONTINUE  # Same idea -- duplicate suppressed by description match.

    # --- Rule 2: no description match, but id collides => NOT a duplicate. ---
    IF new_idea.id in existing_ids:
      # id collision, different idea. Assign a FRESH id (max DEF-NNN
      # across the merged set + 1) and append. NEVER skip it.
      max_n        = max(parse_DEF_number(id) FOR id in existing_ids)  # 0 if none
      new_idea.id  = format_DEF_id(max_n + 1)   # e.g. "DEF-007"
      merged_items.append(new_idea)
      existing_ids.append(new_idea.id)
      existing_descriptions.append(normalize(new_idea.description))
      appended_count += 1
      CONTINUE

    # --- Rule 3: no description match, no id collision => append as-is. ---
    merged_items.append(new_idea)
    existing_ids.append(new_idea.id)
    existing_descriptions.append(normalize(new_idea.description))
    appended_count += 1

  # Add the current ideation session to source_sessions (no duplicates).
  IF IDEATION_ID not in source_sessions:
    source_sessions.append(IDEATION_ID)

  backlog_created  = parsed_backlog.created or current_date_YYYY_MM_DD
  backlog_updated  = current_date_YYYY_MM_DD
  Display: "Idea backlog exists. Appended {appended_count} new item(s); {len(merged_items)} total. Existing items untouched."

ELSE:
  # ---- CREATE path: no existing file. Build from the template. ----
  merged_items     = session.compiled_data.idea_backlog
  source_sessions  = [IDEATION_ID]
  backlog_created  = current_date_YYYY_MM_DD
  backlog_updated  = current_date_YYYY_MM_DD
  Display: "Creating new idea backlog with {len(merged_items)} item(s)."

# Render the file from assets/templates/idea-backlog-template.md.
# Frontmatter carries the machine-readable idea_backlog[]; the Markdown body
# renders Open / Promoted / Rejected tables grouped by status.
backlog_content = render_idea_backlog_template(
  project        = session.project_name,
  created        = backlog_created,
  updated        = backlog_updated,
  source_sessions = source_sessions,
  idea_backlog   = merged_items
)

Write(file_path=backlog_path, content=backlog_content)
Display: "Idea backlog written to: {backlog_path}"
```

**VERIFY:**
```
IF session.phases["05"].idea_backlog_written == false:
  # No-backlog path already recorded -- nothing to verify.
  SKIP verification.

# Confirm the file exists on disk.
glob_result = Glob(pattern=backlog_path)
IF len(glob_result) == 0:
  HALT -- "Step 5.5.5: Idea backlog file NOT created on disk at '{backlog_path}'."

# Confirm the machine-readable block and current session are present.
grep_island = Grep(pattern="idea_backlog:", path=backlog_path)
IF grep_island returns 0 matches:
  HALT -- "Step 5.5.5: Idea backlog file missing the 'idea_backlog:' frontmatter block."

grep_session = Grep(pattern=IDEATION_ID, path=backlog_path)
IF grep_session returns 0 matches:
  HALT -- "Step 5.5.5: Current session '{IDEATION_ID}' not recorded in source_sessions."

# Confirm no DEF-NNN item from the compiled union was dropped (no idea loss).
FOR each idea in session.compiled_data.idea_backlog:
  grep_id = Grep(pattern=idea.id, path=backlog_path)
  IF grep_id returns 0 matches:
    HALT -- "Step 5.5.5: Deferred idea '{idea.id}' missing from the written backlog. Idea-loss defect."
```

**RECORD:**

```bash
devforgeai-validate phase-record ${IDEATION_ID} --workflow=ideation --phase=05 --step=5.5.5 --project-root=. 2>&1
```


```
session.phases["05"].idea_backlog_written = (len(session.compiled_data.idea_backlog) > 0)
session.phases["05"].idea_backlog_path = backlog_path or "N/A"
session.phases["05"].idea_backlog_item_count = len(merged_items) if backlog written else 0
session.completed_outputs.idea_backlog_path = session.phases["05"].idea_backlog_path
Update checkpoint: { phase: 5, step: "5.5.5", status: "complete" }
session.phases["05"].step_5_5_5_completed = true
```
...
```bash
devforgeai-validate phase-record ${IDEATION_ID} --workflow=ideation --phase=05 --step=5.6 --project-root=. 2>&1
```
...
VERIFY_EXIT:
  ...
  CHECK: session.phases["05"].step_5_5_5_completed == true
    IF FAIL: HALT -- "Exit blocked: Step 5.5.5 (idea backlog write/merge) not completed."

  CHECK: IF session.compiled_data.idea_backlog has items THEN session.phases["05"].idea_backlog_written == true
    IF FAIL: HALT -- "Exit blocked: Deferred ideas exist but the idea backlog was not written."
...
checkpoint.phases["05"] = {
  ...
  "idea_backlog_written": session.phases["05"].idea_backlog_written or false,
  "idea_backlog_path": session.phases["05"].idea_backlog_path or "N/A",
  "idea_backlog_item_count": session.phases["05"].idea_backlog_item_count or 0,
  ...
  "steps_completed": ["step_5_1", "step_5_2", "step_5_3", "step_5_4", "step_5_5", "step_5_5_5", "step_5_6"]
}
...
```bash
devforgeai-validate phase-complete ${IDEATION_ID} --workflow=ideation --phase=05 --checkpoint-passed --project-root=. 2>&1
```
...
Inception PM-Plan:
  Risk Register:        {len(session.compiled_data.risk_register)} risk(s)
  Milestone Roadmap:    {len(session.compiled_data.milestone_roadmap)} phase(s)
  Stakeholder Register: {len(session.compiled_data.stakeholder_register)} stakeholder(s)
  Assumptions:          {len(session.compiled_data.assumptions_register)} assumption(s)
  Idea Backlog:         {session.phases['05'].idea_backlog_item_count} idea(s) -> {session.phases['05'].idea_backlog_path}
