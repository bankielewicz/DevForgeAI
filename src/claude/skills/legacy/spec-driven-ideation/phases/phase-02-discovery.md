...
```bash
devforgeai-validate phase-check ${IDEATION_ID} --workflow=ideation --from=01 --to=02 --project-root=. 2>&1
```
...
EXECUTE (Capture Descoped Ideas into the Idea Backlog -- WI-2):
```
# When an idea was EXPANDED during the ideation conversation and then DESCOPED
# (placed out-of-scope or deferred to a future release), it MUST be captured as
# a structured DEF-NNN item appended to session.idea_backlog -- NOT lost as a
# bare scope-boundary string. This is the ideation-side companion to the
# brainstorming Deferred Ideas Register; the idea survives for later evaluation.
# Reuse the DEF-NNN shape verbatim (see assets/templates/idea-backlog-template.md):
#   id, description, why_deferred, business_value, mvp_dependency,
#   revisit_when, source, status.
# Existing scope-boundary behavior above is UNCHANGED -- this ADDS structured
# capture on top of it.

# Initialize the backlog if absent.
IF session.idea_backlog is null:
  session.idea_backlog = []

# DEF-NNN sequencing rule:
#   next_id continues sequentially after the highest DEF-NNN already present in
#   EITHER session.brainstorm_deferred (DEF-NNN items seeded from the brainstorm)
#   OR session.idea_backlog (already-captured items). This guarantees no id
#   collision with brainstorm-supplied deferred ideas.
existing_ids = [item.id FOR item in (session.brainstorm_deferred or [])] +
               [item.id FOR item in session.idea_backlog]
IF len(existing_ids) == 0:
  next_def_number = 1
ELSE:
  next_def_number = max(parse_def_number(id) FOR id in existing_ids) + 1
# parse_def_number("DEF-007") -> 7 ; ids are padded to 3 digits: DEF-001.

# Identify ideas that were expanded in conversation then descoped.
# An idea qualifies when it appears in scope_boundaries.out_of_scope OR
# scope_boundaries.future AND it was discussed/expanded during this session
# (not merely a brainstorm-supplied out_of_scope item -- those already have
# DEF-NNN ids via session.brainstorm_deferred and must NOT be re-captured).
descoped_session_ideas = ideas in (session.scope_boundaries.out_of_scope +
  session.scope_boundaries.future) that were expanded during the ideation
  conversation AND are not already represented in session.brainstorm_deferred
  or session.idea_backlog (dedupe on normalized description).

FOR each descoped_idea in descoped_session_ideas:
  # For each descoped idea, ask the user to qualify it so the DEF-NNN item is
  # rich (why deferred / revisit trigger), not a bare string.
  AskUserQuestion:
    questions:
      - question: "You explored '{descoped_idea}' but it is not in the first release. Why is it deferred, and what would trigger revisiting it?"
        header: "Backlog: {descoped_idea[:30]}"
        multiSelect: false
        options:
          - label: "Let me explain"
            description: "I'll give the deferral reason and revisit trigger"
          - label: "Skip detail -- just record it"
            description: "Capture the idea with minimal rationale"

  IF response == "Let me explain":
    Capture why_deferred and revisit_when from user
  ELSE:
    why_deferred = "Descoped during ideation -- not in the current MVP"
    revisit_when = "Revisit when MVP is validated"

  def_id = "DEF-" + zero_pad(next_def_number, 3)
  session.idea_backlog.append({
    id:             def_id,
    description:    descoped_idea,
    why_deferred:   why_deferred,
    business_value: "",            # optional -- left empty if not elicited
    mvp_dependency: "",            # optional -- left empty if not elicited
    revisit_when:   revisit_when,
    source:         "ideation:{IDEATION_ID}:phase-02",
    status:         "open"
  })
  next_def_number += 1

Display: "Idea backlog: {len(session.idea_backlog)} session-descoped idea(s) captured as DEF-NNN items."
```
...
# Idea-backlog capture is non-blocking: session.idea_backlog may legitimately be
# empty (no ideas were expanded then descoped). It must, however, be a list.
IF session.idea_backlog is null:
  session.idea_backlog = []
FOR each item in session.idea_backlog:
  IF item.id does NOT match "^DEF-\d{3,}$":
    HALT -- "Step 2.6: Idea-backlog item has malformed id '{item.id}'. Must match DEF-NNN."
```

RECORD: Update checkpoint: `session.phases["02"].questions_answered += 3 + len(descoped_session_ideas)`; `session.completed_outputs.scope_boundaries = session.scope_boundaries`; `session.completed_outputs.idea_backlog = session.idea_backlog` (persists DEF-NNN items so they survive --resume / context clears)
...
### Step 2.8.5: UI Design Context Discovery

EXECUTE:
```
# Check if UI context was carried from brainstorm
IF session.ui_context is not null AND session.ui_context.has_mockup is not null:
  # UI context pre-populated from brainstorm
  IF session.ui_context.has_mockup == true:
    Display: "UI mockup detected from brainstorm: ${session.ui_context.mockup_path}"

    IF session.ui_context.design_doc_path is not null:
      Display: "Design Source of Truth already exists: ${session.ui_context.design_doc_path}"
      # Verify it still exists
      Glob(pattern="${session.ui_context.design_doc_path}")
      IF found:
        Display: "design.md verified on disk. Will be carried to architecture phase."
      ELSE:
        Display: "WARNING: design.md path from brainstorm no longer exists."
        session.ui_context.design_doc_path = null

    ELSE:
      AskUserQuestion:
        questions:
          - question: "A mockup was referenced in brainstorm but no Design Source of Truth (design.md) exists yet. Extract one now?"
            header: "UI Design"
            multiSelect: false
            options:
              - label: "Extract now"
                description: "Run /create-design --extract to generate design.md from the mockup"
              - label: "Defer to architecture"
                description: "Generate design.md later during /create-system-architecture or /create-solution-architecture"
              - label: "Skip — no design.md needed"
                description: "Proceed without a formal design specification"

      IF response == "Extract now":
        session.ui_context.extract_requested = true
        Display: "After ideation completes, run:"
        Display: "  /create-design --extract ${session.ui_context.mockup_path} --origin=ideation:${SESSION_ID}"
        Display: "  (spec-driven-design will route you back to resume ideation when done)"

      ELIF response == "Defer to architecture":
        session.ui_context.deferred_to = "architecture"

  ELIF session.ui_context.has_mockup == false:
    # No UI — skip entirely
    Display: "No UI component identified in brainstorm. Skipping UI design context."

ELSE:
  # No brainstorm UI context — ask fresh
  AskUserQuestion:
    questions:
      - question: "Does this project include a user interface (web, desktop, terminal)?"
        header: "UI Context"
        multiSelect: false
        options:
          - label: "Yes, I have an existing mockup"
            description: "I've already built a prototype or mockup in code"
          - label: "Yes, but no mockup yet"
            description: "The project needs UI but I haven't designed it"
          - label: "No UI needed"
            description: "This is a backend/CLI/library project"
          - label: "Undecided"
            description: "I'll decide during architecture phase"

  IF response == "Yes, I have an existing mockup":
    AskUserQuestion:
      questions:
        - question: "Where is the mockup located? (Provide filesystem path)"
          header: "Mockup Path"
          multiSelect: false
          options:
            - label: "In this project"
              description: "The mockup files are within the current project directory"
            - label: "Separate directory"
              description: "The mockup is in a different location — I'll provide the path"

    session.ui_context = {
      has_mockup: true,
      mockup_path: user_provided_path,
    ...
    AskUserQuestion:
      questions:
        - question: "Would you like to extract a Design Source of Truth (design.md) from this mockup?"
          header: "Extract"
          multiSelect: false
          options:
            - label: "Yes, after ideation"
              description: "Generate design.md after this workflow completes"
            - label: "Defer to architecture"
              description: "Generate during /create-system-architecture or /create-solution-architecture"
            - label: "No"
              description: "I'll handle design documentation separately"

    IF response == "Yes, after ideation":
      session.ui_context.extract_requested = true

  ELIF response == "Yes, but no mockup yet":
    session.ui_context = {
      has_mockup: false,
      mockup_path: null,
      design_doc_path: null,
      aesthetic_vibe: null,
      complexity: "unknown",
      deferred_to: "architecture"
    }

  ELIF response == "No UI needed":
    session.ui_context = {
      has_mockup: false,
      mockup_path: null,
      design_doc_path: null
...
VERIFY:
- session.ui_context is populated (not null)
- If mockup path provided: path is non-empty string
- If extract requested: flag is set for post-ideation guidance

RECORD: Update checkpoint: `session.phases["02"].questions_answered += N`; `session.completed_outputs.ui_context = session.ui_context`
...
```bash
devforgeai-validate phase-complete ${IDEATION_ID} --workflow=ideation --phase=02 --checkpoint-passed --project-root=. 2>&1
```
