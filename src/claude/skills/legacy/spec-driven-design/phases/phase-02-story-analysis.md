...
```
Read(file_path=".claude/skills/spec-driven-design/references/story-analysis.md")
Read(file_path=".claude/skills/spec-driven-design/references/user-input-guidance.md")
```
...
**RECORD:**
```
Bash(command="devforgeai-validate phase-record ${IDENTIFIER} ${WORKFLOW_FLAG} --phase=02 --step=2.1 --project-root=. 2>&1")
```
...
ELIF MODE == "standalone":
    ...
    Read(file_path=".claude/skills/spec-driven-design/references/user-input-guidance.md")
    If Read fails: Read(file_path=".claude/skills/spec-driven-design/references/user-input-guidance.md")
...
**RECORD:**
```
Bash(command="devforgeai-validate phase-record ${IDENTIFIER} ${WORKFLOW_FLAG} --phase=02 --step=2.2 --project-root=. 2>&1")
```
...
**RECORD:**
```
Bash(command="devforgeai-validate phase-record ${IDENTIFIER} ${WORKFLOW_FLAG} --phase=02 --step=2.3 --project-root=. 2>&1")
```
...
**RECORD:**
```
Bash(command="devforgeai-validate phase-record ${IDENTIFIER} ${WORKFLOW_FLAG} --phase=02 --step=2.4 --project-root=. 2>&1")
```
...
**EXECUTE:**
```
Bash(command="devforgeai-validate validate-design-phase ${IDENTIFIER} ${WORKFLOW_FLAG} --mode=${MODE} --phase=02 --project-root=. 2>&1")
Bash(command="devforgeai-validate phase-complete ${IDENTIFIER} ${WORKFLOW_FLAG} --phase=02 --project-root=. 2>&1")
```
