...
Recovery:
Please invoke the spec-driven-system-architecture skill first to create the required context files:

Skill(command=\"spec-driven-system-architecture\")

Or use the slash command:
/create-system-architecture ${project_name}

After context files are created, run /create-design again."
```

**Recovery:** User creates context files, then re-runs /create-design
...
Expected location: src/claude/skills/spec-driven-design/assets/${template_file}
Fallback location: .claude/skills/spec-driven-design/assets/${template_file}"
```

**Recovery:**
```
Validate assets directory:
Glob(pattern="src/claude/skills/spec-driven-design/assets/*")
# If fails, try operational path:
# Glob(pattern=".claude/skills/spec-driven-design/assets/*")
...
**Error message:**
```
"Found ${placeholder_count} placeholder(s) in generated code:
