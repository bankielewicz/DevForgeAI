...
- Compare to source-tree/ component location rules
...
IF validation_summary has HIGH severity issues:
  ...
  ELSE IF "Accept with warnings":
    Display warning:
    "Proceeding with critical violations is not recommended.
...
ELSE IF validation_summary has MEDIUM or LOW severity issues:
  ...
  "Validation detected ${medium_severity_count + low_severity_count} warning(s):
...
IF SPEC_QUALITY == "FAILED":
  ...
  - Run /create-design again after corrections"
...
ELSE IF SPEC_QUALITY == "PARTIAL":
  ...
  Display notice:
  "Specification marked as PARTIAL
