...
**Context:**
- Mode: ${MODE} (story|standalone|extract)
- Story ID: ${STORY_ID or 'standalone'}
- UI Spec Summary: devforgeai/specs/ui/UI-SPEC-SUMMARY.md
- Design Source of Truth: devforgeai/specs/ui/design.md (check if exists)
- Framework: ${FRAMEWORK} (from tech-stack.md)
- Styling: ${STYLING_APPROACH} (user selected)
- Component Count: ${COMPONENT_COUNT}
- Generated Files: ${GENERATED_FILES_LIST}
- Token Normalization: ${TOKEN_NORMALIZATION_PCT}% (from extraction/generation step, or 'unknown')

**Generated Files List:**
${List all files created in Phase 5, or extracted artifacts in extract mode}

**Validation Context from Phase 7:**
${validation_context from Step 7.5/7.8}

**Requirements:**
1. Read UI-SPEC-SUMMARY.md (YAML frontmatter + body)
2. Read design.md if it exists at devforgeai/specs/ui/design.md
3. Extract component details (type, framework, styling, accessibility, responsive)
4. Load framework constraints from reference file (ui-result-formatting-guide.md)
5. Validate against context files:
   - tech-stack.md: Framework consistency
   - source-tree/: File location compliance
...
6. IF design.md exists, validate completeness:
   - Children integrity: Every COMP-ID in children[] must exist in component inventory
   - Parent integrity: Every parent must be 'root' or valid COMP-ID
   - Token normalization: Count token refs vs raw values (warn if < 50%)
   - Props completeness: Components rendering children must have children in data.props
   - Callback coverage: Every on* callback prop must appear in at least one FLOW entry
7. Determine appropriate display template (SUCCESS/PARTIAL/FAILED)
8. Generate structured result with:
   - Display template (formatted markdown)
   - Component details (structured metadata)
   - design_md_validation object (exists, token_normalization_pct, dangling_refs, etc.)
...
Return structured JSON following this schema:
{
  ...
  \"design_md_validation\": {
    ...
    \"interaction_count\": N,
    \"animation_count\": N,
    \"token_normalization_pct\": N,
    \"dangling_children_refs\": N,
    \"uncovered_callbacks\": N,
    \"props_missing_children\": N,
...
Follow constraints in src/claude/skills/spec-driven-design/references/ui-result-formatting-guide.md reference file.
If that path fails, try: .claude/skills/spec-driven-design/references/ui-result-formatting-guide.md

**Validation Rules:**
- Tech-stack consistency: STRICT (no substitution)
- File structure compliance: STRICT (source-tree/ mandatory)
...
- Display tone: POSITIVE (use checkmarks for success, warnings for issues, X for failures)"
)
```
...
**Command will output:**
The /create-design command receives this structured result and outputs the display template directly. No additional parsing, formatting, or validation needed in the command.
...
The /create-design command will display the formatter_result.display.template with:
- Component details
- Generated files list
- Validation status
...
Run /create-design again with new story ID or component description.
