...
Generated files **MUST** follow source-tree/ structure:
...
**Display guidance:**
- Always recommend file locations per source-tree/
...
**Validation rules:**
- Check EVERY generated file path against source-tree/
...
**Always enforce:** Structure follows source-tree/; reorganize if needed
...
File Locations (per source-tree/)
...
1. **Header** (2 lines)
   - Status + Title: "UI Component Specification Generated"
...
**source-tree/:**
- "Component files should follow source-tree/ structure"
- "Recommend placing forms in src/components/forms/ (per source-tree/)"
- Alert: "Generated files violate source-tree/ structure; reorganize?"
...
- File structure fundamentally breaks source-tree/ requirements
...
## Design Source of Truth (design.md) Validation

When `devforgeai/specs/ui/design.md` exists, the formatter MUST validate it and include results in the output. This is the framework-agnostic UI specification that any AI can use to recreate the complete UI.

### How to Validate design.md

```
Read(file_path="devforgeai/specs/ui/design.md")

IF file exists:
  1. Extract all COMP-NNN IDs from components section → master_inventory
  2. FOR each component.children[] array:
       Check every ID exists in master_inventory
       Count dangling refs
  3. FOR each component.parent (where parent != "root"):
       Check parent exists in master_inventory
  4. Count visual property values that use token references vs raw values
       Token ref: starts with "colors.", "spacing.", "borders.", "shadows.", "motion."
       Raw value: hex (#XXXXXX), rgba(), px values that match a Section 2 token
       Calculate: token_normalization_pct
  5. Check every component data.props for children prop (if component renders children)
  6. Extract all on* callback props across all components
       Check each appears in at least one FLOW-NNN step
       Count uncovered callbacks
```

### design.md Severity Thresholds

| Check | PASS | WARNING | FAILED |
|-------|------|---------|--------|
| Dangling children refs | 0 refs | 1-2 refs | 3+ refs |
| Token normalization | ≥ 70% | 50-69% | < 50% |
| Props missing children | 0 | 1-2 | 3+ |
| Uncovered callbacks | 0 | 1-3 | 4+ |

### design.md Display Section

Add this section to ALL display templates (SUCCESS, PARTIAL, FAILED) when design.md exists:
...
| Metric | Value |
|--------|-------|
| **File** | `devforgeai/specs/ui/design.md` |
| **Components** | ${component_count} documented |
| **Interactions** | ${interaction_count} flows |
| **Animations** | ${animation_count} defined |
| **Token Usage** | ${token_normalization_pct}% (target: ≥ 70%) |
| **Children Integrity** | ${dangling_count == 0 ? "✓ Clean" : "⚠ " + dangling_count + " dangling"} |
| **Callback Coverage** | ${uncovered == 0 ? "✓ All covered" : "⚠ " + uncovered + " uncovered"} |
...
- This is valid for older invocations or simple standalone mode
- Set `design_md_validation.exists = false` and `status = "N/A"`
- Do NOT report as an error or warning
- Do NOT include the "Design Source of Truth" display section
...
```
Detection: Generated files violate source-tree/ structure

Display:
"File Structure Advisory

Generated files:
- Component: src/ui/LoginForm.jsx (violates source-tree/)

Expected location (source-tree/):
- Component: src/components/forms/LoginForm.jsx

Recommended Actions:
1. Move component files to correct location per source-tree/
...
Recovery:
1. Regenerate specification: `/create-design STORY-XXX`
...
### File Structure
- [ ] Components placed in src/components/ (or per source-tree/)
- [ ] No files in root directory (violates source-tree/)
- [ ] Styles organized per source-tree/
...
- Place files outside source-tree/ without warning (alert)
