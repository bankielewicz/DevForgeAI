...
This guide explains how the `spec-driven-design` skill integrates with the DevForgeAI spec-driven development framework.
...
- If source-tree/ doesn't specify UI location → default to `devforgeai/specs/ui/`
...
### Integration with spec-driven-system-architecture

**Dependency:** UI Generator requires context files created by the system architecture skill.

**Flow:**
```
spec-driven-system-architecture (creates context files)
    ↓
spec-driven-design (validates context, generates UI)
```
...
2. If missing → HALT
3. Message: "Context files missing. Please run spec-driven-system-architecture first."

User: [runs spec-driven-system-architecture]
...
### Integration with spec-driven-lifecycle

**Invocation:** Orchestration can automatically invoke UI Generator when story contains UI requirements.

**Flow:**
```
spec-driven-lifecycle (detects UI requirements in story)
    ↓
spec-driven-design (generates UI components)
    ↓
...
**Flow:**
```
spec-driven-design (generates UI spec)
    ↓
...
### Priority 1: source-tree/ Specification

If `source-tree/` defines UI component location:
```markdown
src/
├── components/
...
Resolution:
  Please invoke the 'spec-driven-system-architecture' skill to create the required context files.
...
**Response:**
```
Story file not found: STORY-999.story.md
...
- [ ] Output location follows source-tree/ structure
...
The `spec-driven-design` skill is deeply integrated with the DevForgeAI framework:
