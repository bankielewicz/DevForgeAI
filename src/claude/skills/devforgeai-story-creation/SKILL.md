...
- **Change:** Added advisory story fields to YAML frontmatter
  - `advisory: true/false` - Identifies advisory (non-blocking) stories
  - `source_gap: GAP-NNN` - Traceability to original gap ID
  - `source_story: STORY-NNN` - Traceability to story where gap was detected
- **Advisory Story Identification:**
  - Title includes `[ADVISORY]` prefix: `[ADVISORY] Fix code quality issue in processor.py`
  - Filename includes `advisory` slug: `STORY-XXX-advisory-fix-code-quality.story.md`
  - Priority defaults to `Low` (regardless of gap severity)
- **Blocking Story Behavior:** Standard stories (blocking: true gaps) omit advisory fields or have `advisory: false`
- **Advisory Priority Default:** Advisory stories automatically receive `Low` priority regardless of source gap severity
- **Impact:** Clear visual distinction between blocking remediation work and advisory improvements
- **References:** `EPIC-054: QA Warning Follow-up System`, `devforgeai/specs/Stories/STORY-348-*.story.md`
...
**Purpose:** Verify Phase 2 (Requirements Analysis) produced all required outputs before proceeding.
...
FOR each section in required_sections:
    IF section NOT found in Phase 2 subagent output:
        missing_sections.append(section)

IF missing_sections is NOT empty:
    HALT: "Phase 2-3 Gate FAILED: Missing {len(missing_sections)} required sections from requirements analysis output: {missing_sections}"
    Display: "Missing sections:"
    FOR each missing_section in missing_sections:
...
**Purpose:** Verify the written story file contains all 28 required sections before proceeding.
...
**Note:** `## Provenance` is OPTIONAL and excluded from the required section checks. Stories without brainstorm origins may not have this section.

**Verification:**
```
<!-- FULL READ MANDATORY — do not use offset/limit -->
story_file = Read(file_path="${STORY_FILE_PATH}")
missing_sections = []

FOR each pattern in required_h2_headers:
    IF NOT Grep(pattern=pattern, path=story_file):
...
IF missing_sections is NOT empty:
    HALT: "Phase 5-6 Gate FAILED: Missing {len(missing_sections)} required sections in story file"
    Display: "Missing sections:"
    FOR each missing_section in missing_sections:
        Display: "  - {missing_section}"
...
**Purpose:** Verify Phase 7 (Self-Validation) was executed and produced validation evidence before proceeding.

**Verification Checks:**
```
# Verify validation-checklists.md was loaded during Phase 7
# Phase 7 must produce at least one validation finding OR explicit "all checks passed" statement

validation_evidence = false

IF Phase 7 produced validation findings:
    validation_evidence = true
    Display: "✓ Validation findings detected"

IF Phase 7 produced "all checks passed" validation result:
    validation_evidence = true
    Display: "✓ All validation checks passed confirmation detected"

IF validation_evidence == false:
    HALT: "Phase 7-8 Gate FAILED: No validation evidence found. Phase 7 (Self-Validation) must load validation-checklists.md and produce validation findings or an explicit all-checks-passed statement."
    Display: "Missing validation evidence - ensure validation-checklists.md was loaded and Phase 7 executed"
    DO NOT proceed to Phase 8

ELSE:
    Display: "✓ Phase 7-8 Gate PASSED: Validation evidence confirmed"
    Proceed to Phase 8
```
