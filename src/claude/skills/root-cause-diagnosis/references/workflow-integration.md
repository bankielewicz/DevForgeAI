...
Reference file for integrating the root-cause-diagnosis skill into existing DevForgeAI workflow phases.
...
AND fix_attempts >= 2
THEN invoke root-cause-diagnosis
```
...
def green_phase_with_diagnosis(story_id, test_command, max_attempts=5):
    ...
    while fix_attempts < max_attempts:
        ...
        if fix_attempts >= 2:
            ...
            diagnosis = Task(
                subagent_type="root-cause-diagnosis",
