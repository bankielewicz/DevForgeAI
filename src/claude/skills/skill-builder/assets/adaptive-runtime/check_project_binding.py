...
def binding(value):
    ...
    for row in value['bindings']:
        ...
        name(row['name'])
        # The project owner chooses where skills are installed, so only the final
        # component is fixed. The schema requires a relative path and nothing more.
        require(relpath(row['package_path']).rsplit('/', 1)[-1] == row['name'])
...
def observe(project_root, skill_root):
    ...
    try:
        ...
        record_path = under(project, '.claude/devforgeai/project-binding.json')
