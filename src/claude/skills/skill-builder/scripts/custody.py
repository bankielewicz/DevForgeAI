...
def adoption_record(root, files, record, problems):
    ...
    # Structural check only: the target lives inside the selected project and its final
    # component is the target name. No fixed skills-directory layout is assumed, because
    # the host project decides where its skills live.
    if not target.startswith(project + "/") or target.rsplit("/", 1)[-1] != name or not (Path(record["project_root"]).is_absolute()):
        raise ValueError("adoption target must identify a directory named for the target inside the selected project")
