...
```
current_phase_id = "05"  // Extracted from filename: phase-{NN}-*.md → NN

phase_steps = registry[current_phase_id].steps  // Filter for current phase only

IF phase_steps is empty:
    Display: "No steps defined for phase 05 in registry"
```
...
FOR each step in phase_steps:
    // Example: "Step 05.1: Run integration tests" or "Step 05.2: Validate coverage"
    subject = "Step 05.{step.id}: {step.check}"
    ...
    TaskCreate(
        ...
        activeForm="Executing Step 05.{step.id}"
    )
```
