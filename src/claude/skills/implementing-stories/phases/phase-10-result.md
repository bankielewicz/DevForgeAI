...
```
current_phase_id = "10"  // Extracted from filename: phase-{NN}-*.md → NN

phase_steps = registry[current_phase_id].steps  // Filter for current phase only

IF phase_steps is empty:
    Display: "No steps defined for phase 10 in registry"
```
...
FOR each step in phase_steps:
    // Example: "Step 10.1: Generate result summary" or "Step 10.2: Display template"
    subject = "Step 10.{step.id}: {step.check}"
    ...
    TaskCreate(
        ...
        activeForm="Executing Step 10.{step.id}"
    )
```
...
d. **Archive Session Memory** (STORY-341)

   When story reaches completion (status="Dev Complete" or "QA Approved"), archive the session:

   ```
   # Archive session memory on story completion
   ...
   IF result.status == "success":
       ...
       Display: "Session memory archived for ${STORY_ID}"
   ```

   **Archive Criteria:**
   - Story completed Phase 10 successfully
   - Result status is "success"
   - Session status changes from `active` to `archived`

   **Reference:** EPIC-052 Session Memory Layer specification

**Reference:** `references/dev-result-formatting-guide.md`
    Read(file_path="references/dev-result-formatting-guide.md")
...
- [ ] dev-result-interpreter subagent invoked
- [ ] Structured result returned to command
- [ ] Display template generated (success/incomplete/failure)
...
| Date | Story | Change |
|------|-------|--------|
| 2026-02-02 | STORY-341 | Added Archive Session Memory (Step d) for session lifecycle completion |
