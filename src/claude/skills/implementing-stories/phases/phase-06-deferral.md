...
```
current_phase_id = "06"  // Extracted from filename: phase-{NN}-*.md → NN

phase_steps = registry[current_phase_id].steps  // Filter for current phase only

IF phase_steps is empty:
    Display: "No steps defined for phase 06 in registry"
```
...
FOR each step in phase_steps:
    // Example: "Step 06.1: Challenge deferrals" or "Step 06.2: Get user approval"
    subject = "Step 06.{step.id}: {step.check}"
    ...
    TaskCreate(
        ...
        activeForm="Executing Step 06.{step.id}"
    )
```
