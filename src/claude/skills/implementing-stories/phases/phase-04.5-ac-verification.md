...
```
current_phase_id = "4.5"  // Extracted from filename: phase-{NN}-*.md → NN

phase_steps = registry[current_phase_id].steps  // Filter for current phase only

IF phase_steps is empty:
    Display: "No steps defined for phase 4.5 in registry"
```
...
FOR each step in phase_steps:
    // Example: "Step 4.5.1: Invoke AC verifier" or "Step 4.5.2: Review results"
    subject = "Step 4.5.{step.id}: {step.check}"
    ...
    TaskCreate(
        ...
        activeForm="Executing Step 4.5.{step.id}"
    )
```
