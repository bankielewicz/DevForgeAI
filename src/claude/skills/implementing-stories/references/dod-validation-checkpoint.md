...
**Skill Reference:** implementing-stories
...
**Integration Pattern:** The implementing-stories skill loads this reference via:
```
...
FOR EACH item in incomplete_items:
  ...
  Return control to implementing-stories skill

  Instructions for /dev command:
  "Re-invoke Skill(command='implementing-stories') with focus on completing: {item.text}
...
**Invoked by:** `/dev` command (after implementing-stories skill completes)

**References:**
- `.claude/skills/implementing-stories/SKILL.md` (Phase 08 Step 1b has similar logic)
