...
### 6. Recommendations Triage (STORY-458)

Process accumulated recommendations queue and convert selected items to stories.

**Trigger:** `**Feedback Mode:** triage` context marker
...
feedback_config:
  ...
  source: devforgeai/feedback/ai-analysis/aggregated/recommendations-queue.json
  actions: [read_queue, display, selection, story_creation, queue_update, summary]
```

**Reference:** `references/triage-workflow.md` for complete 6-phase workflow
...
**Type 6: Triage** (STORY-458)
```
1. Read recommendations-queue.json and parse priority buckets
2. Display recommendations grouped by priority
3. Process user selections from context markers
4. Invoke devforgeai-story-creation per selected item
5. Update queue JSON (move items to implemented)
6. Return formatted completion summary
```

**See `references/triage-workflow.md` for complete workflow.**
...
**Triage mode:**
```
**Feedback Mode:** triage
**Priority Filter:** HIGH
**Selected Items:** rec-001,rec-003
...
- `references/triage-workflow.md` - Triage mode workflow for /recommendations-triage (STORY-458)
