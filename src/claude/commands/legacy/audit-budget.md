...
model: opus
allowed-tools: Read, Glob, Bash(devforgeai-validate:*)
---
...
### Phase 1: Scan All Commands (devforgeai-validate audit-fs engine)

**Invoke the deterministic CLI engine for FS scan + threshold classification:**

```
Write(file_path="tmp/_framework/audit-fs-budget.json", content=json.dumps({
  "kind": "file_metrics",
  "glob": ".claude/commands/*.md",
  "metrics": ["lines", "chars"],
  "threshold_classifications": [
    {"op": "gt", "field": "chars", "value": 15000, "label": "❌ OVER BUDGET", "priority": "CRITICAL"},
    {"op": "gt", "field": "chars", "value": 12000, "label": "⚠️ HIGH USAGE",  "priority": "WARNING"},
    {"op": "gt", "field": "chars", "value": 10000, "label": "⚠️ APPROACHING", "priority": "MONITOR"},
    {"op": "default",                               "label": "✅ COMPLIANT",   "priority": "OK"}
  ],
  "derived_fields": {"budget_pct": "($chars / 15000) * 100"}
}))
result = Bash(devforgeai-validate audit-fs --params tmp/_framework/audit-fs-budget.json)

results = parse_json(result).findings
# Each entry now has: file, lines, chars, budget_pct, status, priority
# Add `name` field by stripping prefix/suffix from path:
for entry in results:
    entry["name"] = basename(entry["file"]).removesuffix(".md")
```
