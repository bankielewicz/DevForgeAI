...
**Step 0.1-pkg: Package-Release Route Detection (ISSUE-566)**

IF `$1 == "--package"`:
- Set `$VERSION = $2`
- If `$2` is empty or doesn't look like a version: Use AskUserQuestion to get the target version
- Set context markers: **Release Workflow:** `release-package`, **Version:** `$VERSION`
- SKIP Steps 0.1–0.6 (story validation does not apply to package releases)
- Proceed directly to Phase 1 with `--workflow=release-package` context active

ELSE: continue to Step 0.1 (story route unchanged).
