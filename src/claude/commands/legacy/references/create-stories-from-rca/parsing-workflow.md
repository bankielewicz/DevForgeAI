...
FOR each recommendation_header in matches:
    ...
    rec = {
        ...
        description: "",
        # ADR-074 Required-tier fields (5-Whys traceability)
        addresses_why: "",        # Required: "Why #N — \"<quoted answer>\""
        evidence_files: "",       # Required: comma-separated paths
        # ADR-074 Rendered-tier fields
        test_specification: "",   # Markdown table content
        conditional: { type: "Unconditional", condition: null },
        effort_hours: null,
        effort_points: null,
        success_criteria: []
    }
    ...
    # Extract ADR-074 Required-tier: addresses_why (5-Whys traceability)
    addresses_line = Grep(pattern="^\\*\\*Addresses:\\*\\*\\s+Why\\s+#?[0-9]+", section_content)
    IF addresses_line found:
        rec.addresses_why = extract everything after "**Addresses:**" up to end of line
    ELSE:
        Display: "Warning: ${rec.id} missing **Addresses:** Why #N — Required-tier contract violation per ADR-074"

    # Extract ADR-074 Required-tier: evidence_files
    evidence_line = Grep(pattern="^\\*\\*Evidence Files:\\*\\*", section_content)
    IF evidence_line found:
        rec.evidence_files = extract everything after "**Evidence Files:**" up to end of line
    ELSE:
        Display: "Warning: ${rec.id} missing **Evidence Files:** — Required-tier contract violation per ADR-074"

    # Extract ADR-074 Rendered-tier: test_specification (markdown table)
    IF section contains "### Test Specification":
        rec.test_specification = content between "### Test Specification" header and next "###" or section break,
                                 preserving the markdown table verbatim

    # Extract ADR-074 Rendered-tier: conditional
    type_line = Grep(pattern="^\\*\\*Type:\\*\\*\\s+(Unconditional|Conditional)", section_content)
    IF type_line found:
        rec.conditional.type = extracted value
        IF rec.conditional.type == "Conditional":
            cond_line = Grep(pattern="^\\*\\*Condition:\\*\\*", section_content)
            IF cond_line found:
                rec.conditional.condition = extract everything after "**Condition:**"
