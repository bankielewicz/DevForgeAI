...
class TestAC2CreateNewState:
    ...
    def test_create_returns_state_with_all_10_phases(self, phase_state):
        """
        Given: A new state file creation
        When: create() is called
        Then: phases object contains keys "01" through "10"
        """
        # Arrange & Act
        state = phase_state.create("STORY-001")

        # Assert
        expected_phases = [f"{i:02d}" for i in range(1, 11)]
...
class TestIntegrationFullWorkflow:
    ...
    def test_complete_workflow_from_creation_to_phase_10(
        ...
        for i in range(1, 11):
            ...
            # Record a subagent for each phase
            ps.record_subagent("STORY-001", phase_id, f"subagent-{i}")
        ...
        assert len(final_state["observations"]) == 10
