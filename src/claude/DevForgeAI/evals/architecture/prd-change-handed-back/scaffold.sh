...
non_functional_requirements:
  ...
  - id: NFR-002
    status: active
    category: constraint
    statement: "Members sign in with their existing Google accounts (applies to the whole product)."
    priority: must
    release: current
```
...
Members sign in with their existing Google accounts (PRD-001#NFR-002).
...
| 3 | 2026-09-14 | claude-code | Added NFR-002: members sign in with Google accounts. Policy resolution: interview.max_calls=8 (default); architecture.mandated_platforms=none (default); quality.required_categories=floor only (default) | NFR-002 |
| 3 | 2026-09-14 | Priya Nair | Approved | status |
FIXTURE_EOF
