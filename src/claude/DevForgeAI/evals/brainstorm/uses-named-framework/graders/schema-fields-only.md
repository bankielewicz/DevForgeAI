...
FAIL if any `yaml items` block has another top-level key or more than one, any item has a field not listed above (for example a framework-specific key such as `phase`, `cluster`, `votes` or `impact`), or the evaluation method section doesn't name diverge-converge.
