...
The final prompt belongs in a fenced text block for the user's Codex conversation. For explicit planning-only READY, an optional later execute prompt selects the actual saved plan, candidate, specification identities, evidence destination and intended test effects; it does not itself authorize execution. Full-run READY is an internal continuation, never a final prompt requesting selection of its own plan. For FAIL, include all eight QA-024 fields and exact selected defects. Check the current host catalog or supported selection mechanism for the relevant skill. Missing dev discovery leaves its prompt pending and the fix packet intact. For INCOMPLETE or NEEDS_INPUT, give the exact unresolved input/evidence, affected cases and responsible owner. For PASS, use the project's defined downstream review; if none is defined, report that decision as unspecified without granting release permission.
