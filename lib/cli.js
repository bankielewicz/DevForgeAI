...
function invokePythonInstaller(args, pythonCommand = 'python3') {
  ...
  // SECURITY: spawn() with array arguments (NOT shell string) prevents command injection
  // Array format: spawn(command, [args]) does NOT invoke shell
  // Shell metacharacters ($, `, ;, |, &&, etc.) are treated as literal characters
  // pythonCommand is validated by checkPython() (only 'python3' or 'python')
