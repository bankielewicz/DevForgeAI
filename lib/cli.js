 ...
 * @returns {boolean} True if Python is available and meets version requirements
 * @throws {Error} Exits with error if Python not found or version invalid
 */
function checkPython() {
  let pythonVersionOutput;

  // Try to get Python version (catch only execSync errors)
  try {
    pythonVersionOutput = execSync('python3 --version', {
      encoding: 'utf8',
      stdio: 'pipe'
    }).trim();

    if (process.env.DEBUG_CLI) console.log('[DEBUG] pythonVersionOutput:', pythonVersionOutput);
  } catch (error) {
 ...
 */
function invokePythonInstaller(args) {
  const installerPath = path.join(__dirname, '..', 'installer', 'install.py');
  const pythonArgs = [installerPath, ...args];
...
function run(argv, options = {}) {
  ...
  // Check Python before invoking installer (throws on error)
  checkPython();

  // Invoke Python installer subprocess
  const pythonProcess = invokePythonInstaller(argv);
