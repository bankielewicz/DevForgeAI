...
The first subordinate unit is now [DFF-WORKER-FEAS-01 v1.0.0](codex-worker-feasibility-v1.md): an isolated one-turn Rust feasibility harness, specified for offline implementation against captured Codex 0.154.0 schemas. It chooses foreground lifetime and an append-only journal only for the probe; it does not select production lifetime or database topology. Native profile/trial prerequisites, supported production dispatch and the full engine remain unresolved. See the [coding handoff](../../../plan/framework-worker-coding-handoff.md).
