...
upstream:
  - {id: PRD-001, item: NFR-002, relation: informed_by, version: 8, hash: null}
  - {id: PRD-001, item: NFR-003, relation: informed_by, version: 8, hash: null}
supersedes: []
superseded_by: null
blocked_by: []
# --- adr-specific ---
...
worktree name, for example `story-001-brainstorm`. Step 2 sets `W=.claude/worktrees/<name>` on its own line.
...
2. **Deploy into the worktree before starting Claude there.** If a worktree has no
   ...
   ```
   Put `W=…` on its own line. Written as `W=value command` on one line, bash sets `W` only in that
   command's environment while `$W` in its arguments expands empty, so every path points at `/`. The
   `[ -d … ]` guard is needed because `find` reports a missing folder on stderr, which `test -z` doesn't
   see, so without the guard a missing source folder passes the symlink check. Continue only when the
   command prints `deployed`. The commands are chained, so a symlink in `src/` stops the deploy. Otherwise `rsync -a` would copy the
...
| 2026-09-23 | accepted | Version 4: hardened deploy snippet (`W=` on its own line, `[ -d ]` guard, quoted paths, final `diff` with success message) after a one-line `W=` deploy targeted `/` while starting STORY-002; the hardened main-checkout redeploy was run successfully after PR #5 |
