"""Assemble developer observations after completed checks; not framework authority."""
import datetime
import json
import pathlib

root = pathlib.Path(__file__).resolve().parent
coverage = json.loads((root / 'coverage-05.summary.json').read_bytes())
tests = json.loads((root / 'final-test-inventory.json').read_bytes())
candidate = '177b5894a236a02c7d322df7f04d647632094a4a3f3998e7f2a150db246e1604'
attempts = ['063-clippy', '064-format-check', '065-coverage', '066-tests']
for attempt in attempts:
    receipt = json.loads((root / attempt / 'receipt.json').read_bytes())
    assert receipt['exit_code'] == 0 and receipt['candidate']['sha256'] == candidate
assert all(t['mandatory_passes'] == 20 and all(status == 'ok' for _, status in t['all_tests']) for t in tests)
assert coverage['covered'] * 100 >= 95 * coverage['count']
assert all(c['matches'] for c in json.loads((root / 'candidate-readback.json').read_bytes()))
assert all(c['matches'] for c in json.loads((root / 'input-final-checks.json').read_bytes()))
rows = '\n'.join(f"| {pathlib.Path(f['path']).name} | {f['covered']}/{f['count']} | {f['percent']}% |" for f in coverage['files'])
commands = '\n'.join(f"| `{a}` | `{ ' '.join(json.loads((root/a/'receipt.json').read_bytes())['argv']) }` | 0 / PASS |" for a in attempts)
supplemental = tests[0]['supplemental_total']
text = f'''# Offline worker harness delivery
...
- Supplemental cases: **{supplemental}/{supplemental}** passed in each final suite. See [actual test names and retained fixture roots](final-test-inventory.json). The production 120-second watchdog and real Ctrl+C/descendant/crash checks executed.
- Executed-line coverage: **{coverage['covered']}/{coverage['count']} = {coverage['percent']}%**, meeting >=95% without rounding. [Full JSON](coverage-05.json) and [raw summary](coverage-05.summary.json).
...
Original handoff selected a fresh absolute child of docs/plan/framework-worker-implementation/. Before writes, [context.md](context.md) bound the complete destination to `{root}`. The original handoff was reread; all promised source outputs remain under the isolated package and reports under that exact root. [output-readback.json](output-readback.json) enumerates required absolute paths, actual readback paths, byte lengths and SHA256. No path substitution or location correction is pending. R01..R18 have implementation/check accounting in traceability.md; native and protected dependencies are expressly separate.
...
(root / 'checkpoint.md').write_text(f'''# Completed offline development checkpoint

- UTC: {datetime.datetime.now(datetime.timezone.utc).isoformat()}.
- Selection: user requested the framework-worker-coding-handoff.md; offline DFF-WORKER-FEAS-01 v1.0.0 only in C:\\Projects\\DevForgeAI.
- Bound evidence root: {root}; original selection and output map in context.md, readback in output-readback.json.
- Final source/test manifest: final-source-test-manifest.json, SHA256 {candidate}. Contract SHA256 7cb5b0cb87e515bb4d59235b615922ab4ea07ef607b8111dde6cc73c8f101b23.
- S1..S5 complete for offline developer delivery. Final checks: 063,064,065,066; exact metrics in delivery.md. Source ownership only the isolated codex-worker-probe package; retained reports only this evidence root.
- All launched final test commands returned exit 0. Test-owned jobs were observed stopped through the declared process cases. No Codex/model/service process was launched.
- Native WN-01/WN-02 NOT_RUN; captured launcher reparse path and reviewed Pro profile are unresolved native prerequisites. Independent QA not selected; framework acceptance NOT_EVALUATED.
- Before any later work, compare source/test/contract/tool identities and original evidence destination. Changes invalidate dependent current qualification; preserve earlier evidence. Do not replay historical PIDs, reset source, install, or rerun native trials from this checkpoint.
- Next safe action: separately selected independent QA using independent-qa-handoff.md. Native trial selection must name model/effort, sanitized review and resource bounds.
''', encoding='utf-8')
print(json.dumps({'delivery': str(root/'delivery.md'), 'coverage': coverage['percent'], 'mandatory': '20/20', 'supplemental': supplemental}))
