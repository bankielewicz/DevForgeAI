...
## 9. Continued discussion: dedicated terminal versus a Rust engine

This section continues the earlier foundation delivery; section 8 describes that earlier task. The current next task is the [MVP handoff](framework-mvp-next-session.md). Original QA findings, quantitative evidence and ownership remain unchanged.

The user proposed a dedicated Adaptive Spec-Driven Engineering terminal, analogous to Codex or Claude Code, to make guardrails native. The adversarial review distinguished a different interface from ownership of workflow state and effects. A terminal could coordinate work, but cannot make an LLM correct, create a protected principal, or manufacture meaningful tests. The original incident included honest incomplete/failing results; refusing a COMPLETE flag alone would not have prevented unfinished remediation.

Official Codex app-server documentation provides a custom-client path with authentication, conversations, approvals and streamed events. The installed native CLI had exposed app-server help as experimental. Current official documentation also states that the app-server command and WebSocket transport are experimental and unsupported for production workloads. Documented capability, successful prototype execution and production support are different conclusions. No custom-client authentication or worker execution was tested during this discussion. [Official app-server documentation](https://learn.chatgpt.com/docs/app-server)

The user then favored retaining Codex's existing interface and putting the work inside a Rust DevForgeAI CLI. They explicitly corrected an interpretation that reduced the CLI to a command/check runner: the proposed CLI contains the engineering workflow engine, dispatch and recovery. That architecture is not already implemented by the index application. The selected direction is recorded in [runtime architecture](../specs/framework/runtime/architecture.md).

The foreground Codex conversation submits requests, carries user steering and retrieves progress. Rust owns the workflow and bounded dispatch; AI workers produce proposals/artifacts, while Rust evaluates required evidence and transitions through the actual authority boundary. A Rust-assisted alternative remains useful but depends on Codex choosing the next call. It cannot silently replace the selected Rust-owned dispatch capability. No automatic always-running model service, recursive worker hierarchy, extra API billing or guaranteed unlimited continuation is selected.

App-server could supply delegated AI execution; it is not needed merely to read deterministic Rust progress. Progress needs stable run identity, bounded event/status retrieval, concrete current operation, results and stop reasons. Stdout flushing does not guarantee immediate foreground model narration. Heartbeats establish liveness, not useful progress; timestamps cannot supply fabricated percentages. User cancellation, account limits, pending approvals and uncertain external effects require explicit behavior.

## 10. Continued discussion: history, knowledge and SQLite

The user proposed a user-home `.devforgeai` directory, analogous to `.codex`, containing framework communications, state and project memory that Codex can query through the CLI. The example `C:\Users\bryan\.devforgeai` is a user-specific location, not a portable hardcoded path. No such directory, database or operational configuration was created by this planning work.

History records what was said/done. Project knowledge records attributed facts, decisions, hypotheses and unresolved claims. Workflow state records obligations and permitted next operations. Persisting a model claim does not promote it to truth or acceptance. Canonical specifications/policy remain in project artifacts; the store references identified revisions and retains necessary evidence. A database cannot wake an idle Codex session by itself.

The user suggested separate SQLite databases by project, role or workflow for contention/isolation. A metadata-only inspection of this user's `.codex` directory observed separate state, goals, queue, memories, thread-history and logs database filenames; their contents were not read for this comparison. File separation did not establish Codex's internal rationale or any measured performance gain. No dependency on private Codex database schemas was selected.

An initial assistant recommendation of a registry plus two databases per project was challenged as premature after the user clarified that suggestions were starting ideas, not mandates. The current guidance is logical separation and transaction ownership first, physical separation when different protection/lifecycle/recovery needs or measured contention justify it. No fixed database count, database per role, or workflow-specific silo is selected. A project-scoped store with separate logical record classes is a starting candidate for the next contract, not a measured optimum.

Shared rules must remain shared across developer, architect and QA views. State transitions and their required events need an atomic update. Separate logs/artifacts and additional stores require explicit missing-file, revision and recovery behavior. SQLite WAL has one writer per database; splitting files does not prove lower I/O and does not provide a single atomic commit across the files. A native owner and explicit remote evidence exchange are preferred to sharing one database between Windows, WSL and remote machines. User-writable home storage cannot substitute for protected authority. [SQLite WAL documentation](https://www.sqlite.org/wal.html)

## 11. Continued discussion: placement, MVP and competitive evidence

The user approved keeping this execution architecture within `docs/specs/framework/`, with subordinate runtime, MVP and evaluation contracts. Existing capability documents continue owning definitions. A sibling CLI product specification and copied MVP foundation were considered; neither was selected because they risk duplicate policy and divergent contracts. This planning memo preserves alternatives and rationale; it is not a runnable implementation specification.

The selected next approach defines scope and evaluation together. The [MVP scope](../specs/framework/mvp/scope.md) targets one complete engineering loop and a second contrasting project, rather than shallow implementations of every framework capability. The [acceptance scenarios](../specs/framework/mvp/acceptance.md) identify observable obligations. Fixture bytes, exact commands, independent oracles and execution profiles still require bounded finalization; no product case has run.

The user's goal is a framework that scores above alternatives. That is an optimization objective, not a factual ranking or permission to invent scores. Workflow adherence, document count, specialist count and the framework's own >=95% coverage are not substitutes for correctly delivered work. A strict framework can be slower, over-block valid actions, or repeatedly fail honestly without improving completion.

The [evaluation protocol](../specs/framework/evaluation/benchmark-protocol.md) separates controlled framework contribution from recommended-product configuration comparisons. It calls for common tasks/tools/budgets where applicable, actual model/effort provenance, all-worker resource accounting, independent oracles, held-out tasks, honest failure denominators, adaptation and recovery cases, and visible human interventions. Safety/integrity failures cannot disappear inside an aggregate. A pilot validates the harness; repeated frozen comparisons support only their declared workload and conditions, not universal superiority.

Competitor documentation was checked, not their implementations or performance. Spec Kit documents extensible workflows and Codex integration; BMad documents flexible entry and customization. Those are credible comparison candidates subject to actual compatibility and setup. SWE-bench supplies independently defined issue-resolution tasks, but its solved-task metric alone does not evaluate the whole framework lifecycle. No competitor trial, install or ranking was performed. [Spec Kit](https://github.github.com/spec-kit/), [BMad](https://docs.bmad-method.org/), [SWE-bench](https://www.swebench.com/)

## 12. Current delivery and next-session boundary

The user approved proceeding and asked whether to preserve the conversation and code in a fresh session or continue expanding ideas. The immediate action is this durable decision record, a bounded MVP scope/acceptance design, evaluation protocol, and [next-session handoff](framework-mvp-next-session.md). Broad brainstorming is not a prerequisite for every next step. However, coding from the entire not-ready foundation would recreate the original ambiguity and overlarge task.

The next session must finalize one smallest useful implementation contract and its exact dependent decisions, or a clearly bounded nonproduction feasibility contract when integration support is the uncertainty. It need not resolve every future framework feature. It must not label a hypothetical interface or an unprotected prototype as a qualified runtime. A coding session then selects that contract and follows red/green/refactor/QA. No runtime implementation, skill change, install, startup change or remote synchronization is included in this documentation delivery.

The [new verification record](framework-mvp-planning/20260915T145455Z/verification.md) retains original input identities/copies, changed-document identities, link observations and independent review. Earlier foundation verification remains evidence of the earlier bytes and is not rerun or overwritten.
