...
## BINDING CONSTRAINTS
`AGENTS.md:3 -> "The assessment is read-only."`
`AGENTS.md:3 -> "A happy-path test does not establish conformance with invalid-input requirements."`
...
The repository contains one happy-path test at `tests/test_limit_cli.py:11 -> "def test_happy_path(self):"`.
