...
def grade_coverage(data, expected):
    ...
    statements = set(expected['statement_lines'])
    executed = set(file['executed_lines']) & statements
    if len(statements) != expected['total_statements'] or len(executed) != s['covered_lines'] or statements - executed != set(file['missing_lines']):
