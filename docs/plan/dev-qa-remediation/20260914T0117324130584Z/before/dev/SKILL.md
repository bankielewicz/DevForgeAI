...
1. Read [context.md](references/context.md). Resolve exact inputs and project identity, inspect current code/rules/tools, and select the evidence destination. Record bounded context before production changes.
...
6. Read [failure-delivery.md](references/failure-delivery.md) whenever work cannot proceed and before the final report. Account for every selected requirement and distinguish delivery, checks, native qualification, and external acceptance.
