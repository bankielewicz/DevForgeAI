"""Report independently verified archived Linux evidence; no test execution."""
import hashlib
import json
...
REG=OUT/'regression'
v=json.loads((OUT/'independent-archive-verification.json').read_text())
coverage=json.loads((REG/'linux-combined/coverage.json').read_text())
runtime=v['runtime'][0]
lines=['# Linux V3 regression and coverage','',
...
f"WSL Ubuntu; `{runtime['platform']}`. Executable `/usr/bin/python3`, Python 3.12.3, coverage.py 7.13.1. Commands used `-B -X utf8`. The suite working directory was `{runtime['cwd']}` on Linux's native `/tmp` filesystem. PowerShell invoked WSL; tests ran as Linux Python processes. Exact argument vectors, times and exits are retained in each process receipt.",'',
...
for b in v['batches']:lines.append(f"| {b['batch']} | {b['cases']} / {b['cases']} | {b['seconds']:.6f} | {b['exit']} |")
lines += ['', 'Each suite subprocess had a 120-second execution ceiling; the outer orchestrator used 150 seconds per command. Every final batch completed below both ceilings. Batches have disjoint expected inventories, so each case is counted once.','',
...
for name,item in coverage['files'].items():
    summary=item['summary'];missing=', '.join(map(str,item['missing_lines'])) or 'None'
    lines.append(f"| {name} | {summary['covered_lines']} / {summary['num_statements']} | {missing} |")
lines += ['', 'The reported line threshold is met despite the remaining listed lines; no exclusions were used. Platform results remain separate from native model-driven acceptance or protected framework decisions.','']
(OUT/'REPORT.md').write_text('\n'.join(lines),encoding='utf-8')
manifest={p.relative_to(OUT).as_posix():{'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in sorted(OUT.rglob('*')) if p.is_file() and p.name!='archive-manifest.json'}
(OUT/'archive-manifest.json').write_text(json.dumps(manifest,indent=2),encoding='utf-8')
print(json.dumps({'report':str(OUT/'REPORT.md'),'archived_files':len(manifest),'verified':v['status']}))
