...
(out/'scope.json').write_text(json.dumps({'source':actual,'files':[str(p.relative_to(PACKAGE)) for p in PACKAGE.rglob('*.py')],'inputs':inputs,'selection':'Final V2-source batches are disjoint; each required case appears once. All earlier V1 attempts and failures remain separate. Coverage includes only these declared inputs from identical source bytes; no exclusions.'},indent=2),encoding='utf-8')
