...
dump(OUT/'scope.json',{'denominator':'Every executable statement in every shipped first-party Python file including unimported modules and runtime asset. No exclusions. Branches separately.','files':list(str(p.relative_to(PACKAGE)) for p in PACKAGE.rglob('*.py')),'line_floor':95,'pass_floor':95,'platform':platform.platform(),'source':source,'tests':tests,'readonly_fixture_dependencies':dependencies})
