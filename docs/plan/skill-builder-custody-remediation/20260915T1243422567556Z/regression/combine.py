...
(out/'scope.json').write_text(json.dumps({'source':actual,'files':[str(p.relative_to(PACKAGE)) for p in PACKAGE.rglob('*.py')],'inputs':inputs,'selection':'Declared source-bound batches are disjoint; each required case appears once. All earlier attempts and failures remain separate. Coverage includes only these declared inputs from identical source bytes; no exclusions.'},indent=2),encoding='utf-8')
