...
class AuthoringSafeguards(unittest.TestCase):
    ...
    def test_authored_origin_identity_and_publication_are_required(self):
        prior = self.create()
        c = self.contract('edit', prior=prior / 'authoring-baseline.json')
        with self.assertRaisesRegex(ValueError, 'baseline identity mismatch'):
            a.origin(dict(c, target_name='different'))
        publication = prior / 'publication-readback.json'
        value = json.loads(publication.read_bytes())
        value['state'] = 'PARTIAL'
        publication.write_text(json.dumps(value))
        with self.assertRaisesRegex(ValueError, 'publication/readback mismatch'):
            a.origin(c)

    def test_legacy_source_and_evidence_drift_rejected(self):
        root, p, c = self.legacy()
        contract = json.loads((root / 'trace/evidence/build-contract.json').read_bytes())
        source = root / contract['inputs'][0]['path']
        original = source.read_bytes()
        source.write_bytes(original + b'changed')
        with self.assertRaisesRegex(ValueError, 'legacy source input changed'):
            a.origin(c)
        source.write_bytes(original)
        record = json.loads(p.read_bytes())
        evidence = root / record['evidence'][0]['path']
        evidence.write_bytes(evidence.read_bytes() + b' ')
        with self.assertRaisesRegex(ValueError, 'legacy evidence digest mismatch'):
            a.origin(c)

    def test_legacy_contract_and_authorized_input_mismatches_rejected(self):
        root, p, c = self.legacy()
        contract_path = root / 'trace/evidence/build-contract.json'
        original = contract_path.read_bytes()
        contract_path.write_bytes(original + b' ')
        with self.assertRaisesRegex(ValueError, 'legacy contract identity/digest mismatch'):
            a.origin(c)
        ...
        c['prior'] = a.reference(p)
        with self.assertRaisesRegex(ValueError, 'legacy authorized inputs mismatch'):
            a.origin(c)

    def test_legacy_baseline_drift_rejected(self):
        root, p, c = self.legacy()
        record = json.loads(p.read_bytes())
        generated = next(row for row in record['outputs'] if row['ownership'] == 'generated')
        baseline = root / generated['baseline_path']
        baseline.write_bytes(b'changed')
        with self.assertRaisesRegex(ValueError, 'invalid legacy baseline bytes'):
            a.origin(c)
