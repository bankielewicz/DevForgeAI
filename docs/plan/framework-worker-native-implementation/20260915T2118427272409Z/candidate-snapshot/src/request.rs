...
impl Request {
    ...
    pub fn verify_review(&self) -> Result<()> {
        ...
        // NI-02 is not version-qualified. V2 intake verifies identity only; it cannot
        // approve a launch until the restrictive policy is implemented and qualified.
        if r.schema_version != self.schema_version
