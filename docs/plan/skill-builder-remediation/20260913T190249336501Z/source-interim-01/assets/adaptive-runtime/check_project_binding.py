...
def excluded(path):
    return any(p.casefold() in EXCLUDED | PRIVATE_KEY_NAMES or 'backup' in p.casefold() or p.startswith('.env') for p in path.parts) or path.suffix.casefold() in PRIVATE_KEY_SUFFIXES | {'.pyc', '.pyo'}
