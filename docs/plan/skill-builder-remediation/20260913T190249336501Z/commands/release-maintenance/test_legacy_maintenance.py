...
class LegacyMaintenance(unittest.TestCase):
    ...
    def test_authoring_publication_failure_retains_effects(self):
        c=t.contract(self.folder)
        cp=t.put(self.folder/'contract.json',c)
        run=self.folder/'docs/plan/run'
        authoring.begin(cp,run)
        t.put(run/'candidate/SKILL.md',b'new authored content\n')
        original=authoring.save
        def fail_request(path,value):
            if path.name=='validation-request.json':raise OSError('synthetic request publication failure')
            return original(path,value)
        self.before()
        with patch.object(authoring,'save',fail_request):result=authoring.publish(run)
        self.assertEqual(result['state'],'PARTIAL')
        self.assertEqual((self.folder/'skills/sample/SKILL.md').read_bytes(),b'new authored content\n')
        self.assertTrue((run/'publication-failure.json').exists())

    def test_authoring_receipt_io_failure_reports_partial(self):
        c=t.contract(self.folder);cp=t.put(self.folder/'contract.json',c);run=self.folder/'docs/plan/run'
        authoring.begin(cp,run);t.put(run/'candidate/SKILL.md',b'new\n')
        original=authoring.save
        def fail_records(path,value):
            if path.name in ('authoring-record.json','publication-failure.json'):raise OSError('synthetic receipt I/O failure')
            return original(path,value)
        self.before()
        with patch.object(authoring,'save',fail_records):result=authoring.publish(run)
        self.assertEqual(result['state'],'PARTIAL')
        self.assertTrue(result['evidence_write_failed'])
        self.assertEqual(result['applied_paths'],['SKILL.md'])

    def test_authoring_interrupted_before_write(self):
        c=t.contract(self.folder);cp=t.put(self.folder/'contract.json',c);run=self.folder/'docs/plan/run'
        authoring.begin(cp,run);t.put(run/'candidate/SKILL.md',b'new\n')
        self.before()
        def interrupt(*args):raise OSError('synthetic interrupted operation')
        result=authoring.publish(run,before_write=interrupt)
        self.assertEqual(result['state'],'BLOCKED')
        self.assertEqual(result['applied_paths'],[])
        self.assertFalse((self.folder/'skills/sample/SKILL.md').exists())

    def test_authoring_capture_failure(self):
        cp=t.put(self.folder/'contract.json',t.contract(self.folder));run=self.folder/'docs/plan/run'
        self.before()
        with patch.object(authoring,'copy_files',side_effect=OSError('synthetic snapshot I/O failure')):
            with self.assertRaises(OSError):authoring.begin(cp,run)
        self.assertEqual(json.loads((run/'capture-failure.json').read_text())['state'],'PARTIAL')
        self.assertFalse((self.folder/'skills/sample').exists())

    def test_safe_boundaries_and_capture_limits(self):
        source=t.put(self.folder/'regular.txt',b'12345')
        self.before()
        for p in ('../escape','a//b','/absolute','C:/absolute','x\\y'):
            with self.assertRaises(ValueError):authoring.relative(p)
        with self.assertRaises(ValueError):authoring.safe(self.folder/'.agents/skills/sample',write=True)
        with self.assertRaises(ValueError):authoring.safe(self.folder/'backups/file')
        with patch.object(authoring,'MAX_BYTES',4):
            with self.assertRaises(ValueError):authoring.read_bytes(source)
        with patch.object(authoring,'MAX_FILES',0):
            with self.assertRaises(ValueError):authoring.files(self.folder)
        with patch.object(build,'MAX_FILES',0):
            with self.assertRaises(ValueError):build.inventory(self.folder)
        with patch.object(custody,'MAX_FILES',0):
            with self.assertRaises(ValueError):custody.snapshot(self.folder)
        for obj in ({'path':str(source)}, {'path':str(source),'sha256':'0'*64}):
            with self.assertRaises(ValueError):authoring.referenced(obj)

    def test_record_read_families(self):
        self.before()
        for index,value in enumerate(({'record_kind':'authoring','schema_version':'authoring-v1','authoring_state':'AUTHORED'}, {'record_kind':'authoring_baseline','schema_version':'authoring-baseline-v1'}, {'schema_version':'1'}, {'schema_version':'2'})):
            path=t.put(self.folder/(str(index)+'.json'),value)
            self.assertEqual(authoring.read_record(path),value)
            value['schema_version']='unknown';t.put(path,value)
            with self.assertRaises(ValueError):authoring.read_record(path)

    def test_schema_families(self):
        import record_schema
        self.before()
        schema={'type':'object','properties':{'choice':{'anyOf':[{'type':'string'},{'type':'null'}]}},'additionalProperties':{'type':'integer'}}
        record_schema.validate({'choice':None,'extra':4},schema)
        for value in ({'choice':False},{'extra':False}):
            with self.assertRaises(ValueError):record_schema.validate(value,schema)
        with self.assertRaises(ValueError):record_schema.validate({}, {'$ref':'https://invalid.example/schema'})
