...
from evidence import RUN, ROOT, PACKAGE, manifest as source_manifest, dump
...
def manifest(root):
    return source_manifest(root, synthetic=True)
...
def adopted(project):
    target = project/'src/agents/skills/sample'
...
class Regression(unittest.TestCase):
    def setUp(self):
        self.folder = ATTEMPT/(self._testMethodName+'-'+hashlib.sha256(self._testMethodName.encode()).hexdigest()[:8])
        self.folder.mkdir(parents=True, exist_ok=False)

    def capture_before(self):
        self.before = manifest(self.folder)
        dump(ATTEMPT/'receipts'/(self.folder.name+'-before.json'), self.before)

    def tearDown(self):
        dump(ATTEMPT/'receipts'/(self.folder.name+'-after.json'), manifest(self.folder))
    ...
    def pointer(self, mutation=None, valid=False):
        ...
        c.update(target_root=str(self.folder/'src/agents/skills/sample'), prior=ref(put(hist/'pointer.json', pointer)), legacy_root=str(hist), history_review='known')
        path = put(self.folder/'contract.json', c)
        before = manifest(hist)
        target_before = manifest(self.folder/'src/agents/skills/sample')
        run = self.folder/'docs/plan/run'
        self.capture_before()
        if valid:
            self.assertEqual(authoring.begin(path, run)['state'], 'STAGED')
        ...
        self.assertEqual(target_before, manifest(self.folder/'src/agents/skills/sample'))
...
def float_manifest(a, q, folder):
    value = json.loads(Path(q['target_manifest']['path']).read_text())
    value['files'][0]['bytes'] = float(value['files'][0]['bytes'])
    q['target_manifest'] = ref(put(folder/'float-manifest.json', value))


def newline_name(self):
    schema = {'type':'string','pattern':'^[a-z0-9]+(?:-[a-z0-9]+)*$'}
    with self.assertRaises(ValueError):
        adaptive.validate_shape('sample\n', schema, schema)


install('F01_manifest_integer_type', lambda self: self.linked(float_manifest))
install('schema_exact_name', newline_name)
...
for index, name in enumerate(['.ENV', '.Env.production', '.eNv.local']):
    install('F05_casefold_env_'+str(index), lambda self,n=name: self.exclusion(n, True))
