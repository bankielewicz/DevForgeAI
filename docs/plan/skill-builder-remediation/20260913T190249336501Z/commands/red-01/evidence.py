...
def manifest(root):
    ...
    while pending:
        for path in sorted(pending.pop().iterdir()):
            ...
            if any(p.casefold() in {'.git', '.env', 'node_modules', '__pycache__', 'devforgeai_cli', '.venv', 'target', 'dist', 'build'} or 'backup' in p.casefold() for p in path.relative_to(root).parts):
