...
class FinalEdges(unittest.TestCase):
    ...
    def test_adoption_requires_explicit_observed_scope(self):
        ...
        self.before()
        for operation,managed in [('validate',record['managed_paths']),('adopt',['missing.md'])]:
            changed=copy.deepcopy(record);changed['managed_paths']=managed
            t.put(hist/'request.json',{'operation':operation,'record':changed})
            before=t.manifest(hist)
            with self.assertRaises(ValueError):build.adoption_plan(args)
            self.assertEqual(t.manifest(hist),before)

    def test_legacy_inventory_rejects_missing_and_over_limit(self):
        folder=self.folder/'snapshot';t.put(folder/'nested/input.md',b'1234')
        self.before()
        with self.assertRaises(ValueError):build.inventory(self.folder/'missing')
        with self.assertRaises(ValueError):build.selected(self.folder/'backup/old.md')
        for limit,value in [('MAX_FILES',0),('MAX_BYTES',3)]:
            with patch.object(build,limit,value):
                with self.assertRaises(ValueError):build.inventory(folder)
        # Simulate growth after the stat check; the aggregate read limit must reject it.
        with patch.object(build,'MAX_BYTES',4),patch.object(build,'read_bytes',return_value=b'12345'):
            with self.assertRaises(ValueError):build.inventory(folder)
