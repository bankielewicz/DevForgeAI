...
The current documentation request specifies [phase 1: brainstorm (DFF-WF-01)](workflows/phase-1-brainstorm-spec.md). No numbered phase 0 existed in this collection before this revision. [Setup/activation](installation-and-integrations.md#setup-intake-and-proposed-activation-workflow) remains a separate prerequisite where applicable, not a required product-discovery phase. The user's selected route is **brainstorm -> prd-create -> prd-review**, followed by work planning and stories when those are needed. [Core workflows](core-workflows.md#discovery-route-and-workflow-names) defines the proposed consumers and optional entry points.

Phase 1 now has a bounded authoring specification, a discovery-brief contract and 20 future assessment scenarios. This documentation does not author or install its skill. The next workflow specifications to select are prd-create and then prd-review; existing dev/qa/story-create contracts remain unchanged. [The decision disposition](roadmap-and-decisions.md#discovery-workflow-disposition-2026-09-17) separates this scope from unresolved bootstrap/worktree and runtime work. [Before identities and preserved document snapshots](../../plan/framework-discovery-planning/20260918T014059438Z/inputs-before.json) retain the input state for this revision.
