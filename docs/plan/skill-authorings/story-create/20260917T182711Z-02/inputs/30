...
```
Read(file_path=".claude/skills/spec-driven-stories/references/completion-report.md")
```
...
Validation findings: ${checkpoint.output.validation_findings_file}
...
Write updated checkpoint to disk — Write() returns cleanly on success, do NOT Glob to verify
