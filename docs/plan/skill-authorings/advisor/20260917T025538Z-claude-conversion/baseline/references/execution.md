...
Resolve `<skill>` as the directory of the loaded advisor package; it is never a fixed installation root, because the project owner chooses where skills are installed. Resolve executable paths using `Get-Command claude`. All request paths are absolute.

`<operator-configured absolute path to the external contract>` is a required external dependency supplied by the operator, not a bundled copy and not a path this package may assume. Resolve it from the current request or existing project configuration, verify its SHA-256 before use, and report its absence as a dependency gap rather than creating or rewriting it. Run the following from the selected repository:

```powershell
python -B -X utf8 <skill>/scripts/advisor_run.py preflight --claude <claude-executable> --auth-mode subscription
```
...
```json
{
  ...
  "claude_path": "<resolved absolute path to the Claude executable>",
  "contract_path": "<operator-configured absolute path to the external contract>",
...
```powershell
python -B -X utf8 <skill>/scripts/advisor_run.py run --request C:/path/intake/request.json --briefing C:/path/intake/briefing.md --run-dir C:/path/runs/unique-id
```

The equivalent development-package launcher command is:

```powershell
& <skill>/scripts/advisor.ps1 -Request C:/path/intake/request.json -Briefing C:/path/intake/briefing.md -RunDir C:/path/runs/unique-id
```
...
Interactive progress is an allowlist derived from parsed events. It prints elapsed time, the session UUID, assistant activity, Read/Grep/Glob tool names, final-result receipt and periodic waiting messages to stderr. A `system/api_retry` event also prints `retry A/M` when `attempt` and `max_retries` are integers, with `status S` appended only for an integer `error_status`. Booleans are excluded; invalid or missing counts suppress the retry line, while an invalid or missing status is omitted. Retry error text and other event fields are never displayed. After evidence is saved, the runner prints the validated execution status, response status, verdict and any reported-cost/cap discrepancy to stderr. It does not print assistant text, tool arguments, file paths, credential values, arbitrary event values, or untrusted terminal control characters. Progress is observational and does not change limits, policy, receipt interpretation, or the final stdout value.
...
Sources checked during authoring: [Claude CLI reference](https://code.claude.com/docs/en/cli-reference), local Claude 2.1.273 `--help`, and [Claude skills](https://code.claude.com/docs/en/skills). Supported flags are checked at runtime rather than inferred from a version number.
