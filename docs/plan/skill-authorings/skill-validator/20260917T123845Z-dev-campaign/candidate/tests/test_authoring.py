...
class AuthoringTests(unittest.TestCase):
    ...
    def test_windows_path_spellings_round_trip_to_intake(self):
        if os.name != 'nt':
            self.skipTest('Windows separator equivalence requires Windows')
        for spelling in ('native', 'forward', 'mixed'):
    ...
    def test_old_noncanonical_stage_requires_fresh_run(self):
        if os.name != 'nt':
            self.skipTest('Windows separator equivalence requires Windows')
        c = self.contract()
        run = self.begin(c)
        ...
        self.assertTrue(any('fresh run' in issue for issue in result['issues']))
        self.assertFalse(self.target.exists())
    ...
    def test_new_initializer_and_metadata_preservation(self):
        ...
        self.assertFalse((self.target / 'agents').exists(), 'Claude Code reads no separate metadata file')
        entry.write_text('---\nname: brief-note\ndescription: Brief supplied notes\n'
                         'allowed-tools:\n  - Read\n  - Write\n'
                         'metadata:\n  owner: notes-team\n'
                         'disable-model-invocation: false\n---\nBody kept.\n')
        # A focused metadata change is an ordinary text edit; unrelated fields and the
        # body must survive it byte-for-byte.
        text = entry.read_text()
        entry.write_text(text.replace('disable-model-invocation: false',
                                      'disable-model-invocation: true'))
        value = skill_format.frontmatter(entry.read_text())
        self.assertTrue(value['disable-model-invocation'])
        self.assertEqual(['Read', 'Write'], value['allowed-tools'])
        self.assertEqual({'owner': 'notes-team'}, value['metadata'])
        self.assertTrue(entry.read_text().endswith('Body kept.\n'))
    ...
    def test_metadata_explicit_policy_change(self):
        ...
        entry = self.target / 'SKILL.md'
        entry.write_text('---\nname: brief-note\ndescription: Brief supplied notes\n---\n')
        # allow_implicit_invocation: false maps onto disable-model-invocation: true.
        # There is no $skill-name prompt substitution in Claude Code, so no equivalent
        # of default_prompt is written; the skill is invoked as /brief-note.
        entry.write_text('---\nname: brief-note\ndescription: Brief supplied notes\n'
                         'disable-model-invocation: true\n---\n')
        value = skill_format.frontmatter(entry.read_text())
        self.assertTrue(value['disable-model-invocation'])
        self.assertNotIn('default_prompt', value)
