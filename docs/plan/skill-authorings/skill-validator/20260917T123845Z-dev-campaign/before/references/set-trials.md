...
### 5.2 Native Claude Code CLI trials

Use the actual installed CLI through terminal execution when available. Inspect `claude --version` and `claude --help` first and record supported options and actual host restrictions; never copy a command shape you have not re-verified against the installed build. A baseline non-interactive shape, whose flags were verified present in Claude Code 2.1.274 during this conversion, is:

```text
claude --print --output-format json --add-dir <disposable-project> --permission-mode acceptEdits
```

Run it with the disposable project as the working directory, supply the cold prompt on stdin or as the trailing positional argument, and capture stdout/stderr with the parent terminal runner. There is no `--cd` equivalent; the working directory selects the project. No shell-built prompt interpolation. Unsupported required options produce NOT_RUN for that native case, with static and helper work continuing.

Use existing authorized CLI authentication and model selection; do not copy credentials, select a new paid provider, install profiles, disable trust checks, add writable real-project roots, or use sandbox/approval bypass flags. A native model invocation needs the host's existing execution authorization; if unavailable, record NOT_RUN. Inspect relevant inherited configuration and enabled hooks without exposing secrets. If effects cannot be contained, do not run the fixture. A permission-mode argument, an independent task prompt, and a before/after manifest are not proof of OS isolation; report the actual boundary. Never reach for `--dangerously-skip-permissions` or `--allow-dangerously-skip-permissions` to make a case run.

For explicit workflow trials the cold prompt receives only the selected skill, raw task inputs, user outcome, and permitted effects. For discovery trials place the package in the disposable project's `.claude/skills/<name>`, which is the project discovery location the host actually scans, and omit explicit skill invocation. Capture host selection evidence; when no reliable selection event is available, native activation is unverified even if the output resembles expected behavior. Description-only classification uses the description and unlabeled prompts, not the package body or answer labels.
