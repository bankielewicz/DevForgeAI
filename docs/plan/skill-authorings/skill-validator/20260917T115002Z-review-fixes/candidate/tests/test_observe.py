...
The specification ID is the origin document identity and is preserved verbatim;
the host these checks target is Claude Code.
...
class ObserveCliTests(unittest.TestCase):
    ...
    def test_nonfinite_json_constants_and_overflow_are_record_mismatches(self):
        run = self.root / "records"
        run.mkdir()
        for number in ("NaN", "Infinity", "-Infinity", "1e999", "-1e999"):
            with self.subTest(number=number):
                (run / "finding-data.json").write_text('{"value":' + number + '}', encoding="utf-8")
                self.invoke("records", "--run-root", run, expected=1)
    ...
    def test_ready_requires_current_review_bound_to_proposal_and_target(self):
        run, _ = self.records_fixture()
        source_bytes = VALID_SKILL.encode("utf-8")
        (run / "source").mkdir()
        (run / "source" / "SKILL.md").write_bytes(source_bytes)
        source_rows = [{"path": "SKILL.md", "bytes": len(source_bytes), "sha256": sha256(source_bytes)}]
        package_digest = sha256(json.dumps(source_rows, ensure_ascii=False, separators=(",", ":")).encode("utf-8"))
        self.save_record(run, "source-manifest.json", {"schema_version": "1", "files": source_rows, "package_digest": package_digest,
                                                       "excluded_boundaries": [], "complete": True})
        ...
        handoff = {"builder_readiness": "READY", "proposal_review_state": "approved", "review_instruction": "Synthetic authorization for these exact bytes",
                   ...
                   "target_package_digest": package_digest,
                   "review_authorization": {"proposal_sha256": sha256(proposal), "target_package_digest": package_digest}}
