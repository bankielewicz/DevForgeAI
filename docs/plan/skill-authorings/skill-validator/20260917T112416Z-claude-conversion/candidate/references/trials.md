...
Where the host offers an appropriate task runner — `Task` in Claude Code, or a separate `claude --print` child session — exercise the workflow from minimal inputs without injecting intended fixes into its prompt. Otherwise mark native workflow execution unperformed and retain script trials or a manual walkthrough as separate evidence. Lack of a runner does not block package inspection or report generation. Execution that is required to substantiate a workflow claim cannot be replaced by an invented PASS.
...
Use separate host subagents for independent reviews where the selected build task requires them; ordinary validation can use one primary agent. A `Task` subagent has its own context, which is what makes an independent review independent, but it shares this session's authentication and budget and is not an enforced permission boundary. A cold workflow task receives only the selected skill entrypoint, minimal raw inputs, user outcome and authorized scope. Keep evaluator expectations out of that prompt. An agent's assigned path boundary is not OS-enforced isolation; inspect before/after effects.
