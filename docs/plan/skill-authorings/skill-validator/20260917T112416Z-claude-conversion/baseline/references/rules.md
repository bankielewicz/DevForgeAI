...
| Discovery and invocation | Explicit and implicit trigger behavior, near-misses, applicable invocation policy, and relevant duplicate names. Development source outside a host discovery location such as `.claude/skills` is not a defect. Installation is separately scoped. |
...
The bundled rule summaries were recompiled on 2026-09-17 from the live Claude Code skills reference, the Agent Skills specification, the Anthropic skill-authoring best-practices page, the in-repository Agent Skills guidance copies, and the selected project specification. The superseded OpenAI Build-skills, prompt-engineering, model-guidance and citation-formatting authorities are retained by identity and digest in the snapshot's `supersedes` block rather than reinterpreted against a host they do not describe; no Claude equivalent of the citation-formatting guidance was retrieved, so REC-006 survives as project policy with its official-source freshness recorded unavailable. Their source digests identify the extracted representations retained in the build evidence; URLs permit a new refresh. Copy fallback summaries into the run, record snapshot_only for unavailable original content, and do not cite a passage you have not read. A live retrieval must be saved before its digest is recorded. Preserve prior expected rules when refresh changes them mid-run.

Agent Skills permits `compatibility`; the installed Skill Creator checker observed during construction does not list that field. Report its rejection as a named checker limitation where the pinned applicable standard supports the field. Optional directories are not mandatory, and Claude Code reads no separate metadata file at all, so an `agents/*.yaml` carried over from another host is unread configuration rather than an optional contract. Keep unknown extension metadata as an applicability question unless an authoritative selected rule resolves it. No model override or fixed model is required.
