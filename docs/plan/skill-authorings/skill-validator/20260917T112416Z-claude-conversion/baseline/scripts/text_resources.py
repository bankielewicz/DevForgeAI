...
def package(source, tokenizer_name=None, encoding=None):
    ...
    # the SKILL.md frontmatter. AV-F04 therefore covers the optional frontmatter
    # fields rather than another host's configuration document.
    if 'SKILL.md' in texts:
        try:
            optional_meta = frontmatter(texts['SKILL.md'])
            optional = set(optional_meta) - {'name', 'description'}
            unknown = optional - set(skill_format.SUPPORTED_FIELDS)
            divergences = [row for row in skill_format.advisory_checks(optional_meta) if not row[1]]
            if not optional:
                ...
                checks.append(check('AV-F04','SKILL.md','NOT_RUN','Supported optional fields and value domains checked; recorded source divergence needs review: ' + '; '.join(row[2] for row in divergences),'unknown'))
            else:
                checks.append(check('AV-F04','SKILL.md','PASS','Supported optional frontmatter fields and value domains checked.'))
        except ImportError:
            checks.append(check('AV-F04','SKILL.md','NOT_RUN','Installed YAML parser unavailable.'))
        except Exception as error:
            checks.append(check('AV-F04','SKILL.md','FAIL','Optional frontmatter parse/type/domain failure: ' + type(error).__name__))
