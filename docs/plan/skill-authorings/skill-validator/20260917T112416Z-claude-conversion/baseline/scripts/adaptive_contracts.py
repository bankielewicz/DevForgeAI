...
class Reader:
    ...
    def binding(self, value):
        ...
        for row in value['bindings']:
            # The installation root is a deployment fact, not a package constant: a
            # binding is read wherever the host installs skills. Check that the path
            # is a normalized relative locator identifying this named package, and do
            # not assert a fixed root such as .agents/skills or .claude/skills.
            package_path = row['package_path']
            require(observe.normalized_relative(package_path), 'invalid installed path')
            require(package_path.split('/')[-1] == row['name'], 'installed path must identify the bound package name')
