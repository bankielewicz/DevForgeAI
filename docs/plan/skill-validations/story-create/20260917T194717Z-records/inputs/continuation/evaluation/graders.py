    ...
    for line in text.splitlines():
        if opener is None:
            ...
            if match and not (match[1][0]=='`' and '`' in match[2]):
                opener=(match[1][0],len(match[1]),match[2].strip().lower())
                content=[]
        elif re.fullmatch(r' {0,3}'+re.escape(opener[0])+'{'+str(opener[1])+r',}[ \t]*',line):
            yield opener[2],'\n'.join(content)
            opener=None
        else:
            content.append(line)
...
def grade_story(text, expected):
    ...
    for language,block in fenced_blocks(text):
        if language not in ('yaml','yml'):continue
