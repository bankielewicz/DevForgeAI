...
def main():
    ...
    for line in (RAW/'evaluation/attempt-001/results.jsonl').read_text(encoding='utf-8').splitlines():
        ...
        if result['observation'] is None:
            assert result['case_id'] in ('H13','H14') and result['result']=='PASS'
            continue
    ...
    print(json.dumps({'root':str(ROOT),'resources':len(resources),'bindings':len(bindings),'nonbinding_cli_cases':['H13','H14']}))
