...
The subsequent [runtime architecture](runtime/architecture.md) records the selected headless Rust workflow-engine direction, the distinct assisted/orchestrated execution modes, and progress/persistence boundaries. [MVP scope](mvp/scope.md) selects the first connected candidate outcome. Neither document supplies an installed authority or changes this contract's protection requirements.
