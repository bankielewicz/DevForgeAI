...
for stage in ('candidate','delivered'):
    ...
    schema_rows=[json.loads(line) for line in schema['stdout'].splitlines()]
    assert len(schema_rows)==10 and all(row['expected_valid']==row['observed_valid'] for row in schema_rows)
