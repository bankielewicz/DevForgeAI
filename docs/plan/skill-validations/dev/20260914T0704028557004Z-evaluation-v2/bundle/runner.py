...
def main():
    ...
    schema = graders.load((bundle / 'evidence.schema.json').read_bytes())
    ...
    for case in scenarios:
        ...
        for trial in observed.get('native_trials', []):
            attempt_id = observed.get('native_attempts', {}).get(trial, '001')
            command = graders.load((run / 'commands' / (trial + '-' + attempt_id) / 'command.json').read_bytes())
            effects = graders.load((run / 'trials' / trial / ('attempt-' + attempt_id) / 'effects.json').read_bytes())
        ...
        results.append({'schema_version': 'dev-evaluation-v1', 'case_id': cid, 'required': True,
                        ...
                        'evidence': observed['evidence'], 'started_at': started, 'ended_at': dt.datetime.now(dt.timezone.utc).isoformat()})
        jsonschema.validate(results[-1], schema)
