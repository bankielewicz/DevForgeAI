...
def trial(name, attempt_id='001'):
    ...
    cmd = [shutil.which('codex'), 'exec', '--cd', str(project), '--sandbox', 'workspace-write', '--skip-git-repo-check',
           '--json', '--output-last-message', str(project / '.trial-output' / ('final-' + attempt_id + '.txt')), '-']
    result = h.execute(name + '-' + attempt_id, cmd, cwd=project, stdin=plan['task_prompt'], timeout=360)
    ...
    save(attempt / 'effects.json', {'changed_paths': changed, 'immutable_changes': [p for p in changed if any(p == x or p.startswith(x) for x in immutable)],
                                   'exclusions': after['exclusions'], 'command_receipt': str(RUN / 'commands' / (name + '-' + attempt_id) / 'command.json')})
...
if __name__ == '__main__':
    if sys.argv[1] == 'setup':
        setup()
    elif sys.argv[1] in ('batch', 'host-start-escalation'):
        names = sys.argv[2:] or [p['trial_id'] for p in json.loads((RUN / 'native-plan.json').read_bytes())['trials']]
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
            futures = {pool.submit(trial, name, '002' if sys.argv[1] == 'host-start-escalation' else '001'): name for name in names}
