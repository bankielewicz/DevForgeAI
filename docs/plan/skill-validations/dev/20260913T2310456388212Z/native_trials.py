...
def setup():
    ...
    for case in cases:
        ...
        for variant in variants:
            ...
            plan=dict(case,trial_id=name,permitted_write_root=str(project),task_prompt=prompt,fixture_origin=str(fixture),fixture_manifest=before,selected_specs=selected,authorization=str(RUN/'authorization.json'),timeout_seconds=360)
...
def trial(name,attempt):
    ...
    record=h.execute(name+'-'+attempt,cmd,cwd=project,stdin=prompt,timeout=360)
...
if __name__=='__main__':
    if sys.argv[1]=='setup': setup()
        ...
        prior=Path(json.loads((RUN/'extension-policy.json').read_bytes())['prior_native_run'])
        deadline=time.monotonic()+600
        names=[]
        while True:
            ready=True; selection=[]
            for plan in plans:
                name=plan['trial_id']; attempt='002' if name=='DV-01' else '001'
                result=prior/'trials'/name/('attempt-'+attempt)/'result.json'
                if not result.is_file(): ready=False; break
                observed=json.loads(result.read_bytes())
                final=Path(observed['cwd'])/'.trial-output'/('final-'+attempt+'.txt')
                if observed['termination']!='exited' or observed.get('exit_status')!=0 or not final.exists():
                    selection.append(name)
            if ready:
                names=selection; break
            if time.monotonic()>deadline: raise RuntimeError('Prior batch outcome unresolved; no extended launch')
            time.sleep(5)
        save(RUN/'launch-selection.json',{'selected_trials':names,'basis':'Prior batch terminated and retained after-manifests/results; only incomplete attempts selected. Original prompts/oracles preserved.','timeout_seconds':360,'maximum_concurrent_cold_tasks':2,'selected_at':h.now()})
