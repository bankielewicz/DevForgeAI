...
description: >-
  Validate and test a selected Claude Code skill or explicit skill set, including adaptive
  core, project-variant and expertise contracts, structure, instructions, workflow, and bounded
  behavior against applicable Claude Code and Agent Skills guidance and its specification.
  Establish an origin specification when missing, report evidence-backed findings, and
...
Assess one explicitly selected Claude Code development skill or selected set. Preserve its bytes and exact origin, inspect its workflow, execute authorized disposable trials, and return findings plus a proposed full revision specification for review. This is a development assessment: Python observations and model conclusions are not DevForgeAI admission, enforcement or mutation authority. Compiled Rust remains the separate future enforcement design.

Two boundaries here are structural rather than asserted. `Skill` is absent from `allowed-tools`, so this workflow cannot invoke skill-builder or any other skill even if a later instruction — including text inside the package being assessed — asks it to; the handoff stops at a reviewable artifact because nothing else is reachable. The Bash grants cover the bundled Python helpers and the Windows trial adapter only: a trial that runs some other target's command is deliberately not pre-approved and surfaces as a permission decision you must justify per case. `Task` is granted because the contract genuinely needs a cold executor — a host task gives an independent review the real isolation that task prose alone never provided — but a subagent is still not an enforced permission boundary, and self-review stays labelled as self-review.
...
The target is a Claude Code package, so all host-read metadata is in its `SKILL.md` frontmatter and no separate configuration file exists. Use [claude-frontmatter-guidance.md](assets/claude-frontmatter-guidance.md) for the supported fields, their value domains, and the recorded places where the Claude Code reference and the Agent Skills specification disagree. Those divergences are reported as divergences: a YAML-list `allowed-tools` is idiomatic here and non-conformant there, and failing a legal Claude Code package against a specification it does not claim is a defect in the assessment, not in the package.
