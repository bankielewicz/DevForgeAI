...
def name_from_markdown(path):
    # YAML is used only by the resolver, not by the evaluation runner/custody.
    try:
        import yaml
...
def revision_plan(args):
    ...
    if version == "2":
        import custody
        problems = []
        _, observed_baseline = custody.revision_origin(root, custody.snapshot(root), request, problems)
        if problems:
...
def adoption_plan(args):
    ...
    root = selected(args.snapshot_root)
    request = json_value(read_bytes(within(root, args.request)))
    custody.fields(request, {"operation", "record"})
    ...
    custody.fields(record, custody.ADOPTION_FIELDS - {"retained_user_paths", "recording_state"},
    ...
    managed = custody.strings(record["managed_paths"], False)
    ...
    problems = []
    custody.adoption_record(root, files, record, problems)
    if problems:
