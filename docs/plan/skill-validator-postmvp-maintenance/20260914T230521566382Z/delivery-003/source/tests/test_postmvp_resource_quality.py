...
class ResourceQualityTests(unittest.TestCase):
    ...
    def test_structure_optional_policy_valid_and_invalid_shapes(self):
        for policy, expected in [('policy: {allow_implicit_invocation: false}', 0),
                                 ('policy: []', 1), ('policy: {allow_implicit_invocation: "false"}', 1),
                                 ('policy: [', 1)]:
            with self.subTest(policy=policy):
                self.write('agents/openai.yaml', policy)
    ...
    def test_package_configuration_icons_tools_and_extensions(self):
        self.write('assets/icon.png', b'\x00\xff')
        ...
        icon = next(v for v in details['resources'] if v['path'] == 'assets/icon.png')
        self.assertTrue(icon['reachable'])
        self.write('agents/openai.yaml', 'interface: {future: value}\ndependencies: {tools: [{type: custom, value: x}]}\npolicy: {future: true}')
        checks, _, _ = text_resources.package(self.skill)
        self.assertEqual(next(v for v in checks if v['rule_id'] == 'AV-F04')['result'], 'NOT_RUN')

    def test_package_bad_configuration_shapes_are_failed_not_crashes(self):
        for value in ('interface: []', 'interface: {display_name: 1}', 'policy: []',
                      'policy: {allow_implicit_invocation: "yes"}', 'dependencies: []',
                      'dependencies: {tools: 2}', 'dependencies: {tools: [{type: mcp}]}',
                      'interface: {icon_large: missing.png}', 'not a mapping'):
            with self.subTest(value=value):
                self.write('agents/openai.yaml', value)
                checks, _, _ = text_resources.package(self.skill)
                self.assertEqual(next(v for v in checks if v['rule_id'] == 'AV-F04')['result'], 'FAIL')
