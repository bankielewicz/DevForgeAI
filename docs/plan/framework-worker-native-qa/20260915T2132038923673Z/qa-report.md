...
Evidence retention is also **INCOMPLETE**. The exact-suite receipt is partial/transcribed, the coverage-command receipt is an authored summary, and helper attempt 001 does not retain the exact native Cargo exit code or Cargo-only duration. These are evidence-retention limits (`QA-GAP-05`), not product defects. The genuine raw LLVM `coverage.json` remains authoritative for the 1,542/1,602 line metric.
...
The receipt set is not uniformly raw. `helper-002.txt`, `schema2.txt`, and `quality.txt` retain their recorded command output/status material. `helper-001.txt` is a partial terminal capture with wrapper status 1; its exact native Cargo exit code and Cargo-only duration are **NOT_RETAINED**. `exact-suite.txt` is a partial/transcribed test summary with dependency compilation output omitted. `coverage-command.txt` is an authored command summary with selected status lines and no retained elapsed duration. The raw LLVM JSON, rather than that summary receipt, is the coverage authority.

| Purpose | Exact command | Recorded result/status | Evidence class |
| --- | --- | ---: | --- |
| Independent identity cases, first attempt | `cargo test --locked --offline --lib -- --test-threads=1` | Wrapper/capture status 1; exact native Cargo exit code NOT_RETAINED | Partial terminal capture: `receipts/helper-001.txt` |
| Independent identity cases, corrected QA oracle | `cargo test --locked --offline --lib -- --test-threads=1` | 0 | Recorded combined output: `receipts/helper-002.txt` |
| Independent schema-2 negatives and fail-closed path | `cargo test --locked --offline --test qa_schema2 -- --test-threads=1` | 0 | Recorded combined output: `receipts/schema2.txt` |
| Frozen-candidate regression | `cargo test --locked --offline --all-targets -- --test-threads=1` | 0 | Partial/transcribed summary: `receipts/exact-suite.txt` |
| Format | `cargo fmt --all -- --check` | 0 | Recorded status/output: `receipts/quality.txt` |
| Static analysis | `cargo clippy --locked --offline --all-targets -- -D warnings` | 0 | Recorded status/output: `receipts/quality.txt` |
| Coverage | `cargo llvm-cov --locked --offline --all-targets --json --output-path C:\Projects\DevForgeAI\docs\plan\framework-worker-native-qa\20260915T2132038923673Z\coverage.json -- --test-threads=1` | 0 | Authored summary: `receipts/coverage-command.txt`; genuine raw LLVM artifact: `coverage.json` |

Attempt `helper-001` retains output showing 4 cases passed and 1 harness assertion failed because the QA assertion compared the same Windows physical path with and without the `\\?\` extended-path spelling. The wrapper/capture returned status 1; the exact native Cargo exit code was not retained. The QA-only assertion was corrected to resolve the expected path before comparison. Candidate source and expected behavior were unchanged. Attempt `helper-002` then passed 5/5. The failed attempt is not counted as a product failure and is not erased from evidence.
...
- Open prerequisite/evidence gaps: 5 (`QA-GAP-01` through `QA-GAP-05`).
- Source repair: none.
...
- `receipts/`: retained command evidence with per-receipt raw/partial/transcribed/summary limits stated above.
- `prior-publication/`: byte-preserved copies of the earlier report, manifest, and receipt versions before the evidence-label correction.
