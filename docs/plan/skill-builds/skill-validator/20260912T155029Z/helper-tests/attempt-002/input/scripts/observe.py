...
def strict_json(data):
    ...
    return json.loads(data, object_pairs_hook=strict_object, parse_constant=nonfinite)
...
def structure(args):
    ...
    if isinstance(metadata, dict):
        ...
        check("name", isinstance(name, str) and bool(re.fullmatch(r"[a-z0-9]+(?:-[a-z0-9]+)*", name)) and len(name) <= 64,
              "name: 1-64 lowercase letters/digits with single separating hyphens")
        check("description", isinstance(description, str) and bool(description.strip()) and len(description) <= 1024,
        ...
        for key, expected_type in types.items():
            if key in metadata:
                check("metadata_" + key, isinstance(metadata[key], expected_type), "optional field has supported type")
        if isinstance(metadata.get("compatibility"), str):
            check("compatibility_length", 1 <= len(metadata["compatibility"]) <= 500, "compatibility 1-500 characters")
        if isinstance(metadata.get("metadata"), dict):
            check("metadata_values", all(isinstance(k, str) and isinstance(v, str) for k, v in metadata["metadata"].items()), "metadata maps string keys to string values")
        extra = sorted(set(metadata) - {"name", "description", *types})
        if extra:
    ...
    if config in files:
        try:
            value = yaml.load(read_stable(*files[config]).decode("utf-8"), Loader=UniqueLoader)
            check("configuration_yaml", isinstance(value, dict), "optional configuration must be YAML mapping", config)
            if isinstance(value, dict) and "policy" in value:
                policy = value["policy"]
                check("configuration_policy", isinstance(policy, dict), "policy mapping", config)
                if isinstance(policy, dict) and "allow_implicit_invocation" in policy:
                    check("invocation_policy_type", type(policy["allow_implicit_invocation"]) is bool, "allow_implicit_invocation boolean", config)
        except (yaml.YAMLError, ValueError, TypeError) as exc:
            check("configuration_yaml", False, str(exc), config)
    response = result("structure", "MISMATCH" if required_mismatch else ("INCOMPLETE" if excluded else "OBSERVED"),
                      ["required metadata", "selected optional metadata types", "ordinary inline links", "ATX heading anchors", "optional configuration syntax"],
    ...
    response["limitations"].extend([
        ...
        "Extra host metadata and configuration keys require selected-source applicability review; installed checker compatibility is a separate observation.",
