...
EXCLUDED = {'.git', 'node_modules', '.venv', 'venv', 'target', 'dist', 'build', '__pycache__', 'backup', 'backups', 'devforgeai_cli'}
...
def excluded(path):
    return any(p.casefold() in EXCLUDED or 'backup' in p.casefold() or p.startswith('.env') for p in path.parts) or path.suffix.casefold() in ('.pem', '.key', '.pyc', '.pyo')
...
def observe(project_root, skill_root):
    ...
    try:
        ...
        for item in value['bindings']:
            ...
            groups.add(group)
        require(capture.read(record_path) == raw, 'INVALID_BINDING')
