...
def validate_shape(value, schema, document, location='$'):
    ...
    if '$ref' in schema:
        ...
        if reference.startswith('adaptive-common.schema.json#/$defs/'):
            document = runtime.strict_json((PACKAGE / 'schemas/adaptive-common.schema.json').read_bytes())
            reference = reference[len('adaptive-common.schema.json'):]
        need(reference.startswith('#/$defs/'), 'unsupported schema reference')
        return validate_shape(value, document['$defs'][reference[8:]], document, location)
    if 'anyOf' in schema:
        for option in schema['anyOf']:
            try:
                validate_shape(value, option, document, location)
                return
            except ValueError:
                pass
    ...
    if 'enum' in schema:
        need(value in schema['enum'], location + ': unknown enum')
    kind = schema.get('type')
    types = {'object': dict, 'array': list, 'string': str, 'integer': int, 'boolean': bool, 'null': type(None)}
    if kind:
        need(type(value) is types[kind], location + ': wrong type')
    if kind == 'object':
        need(set(schema['required']) <= set(value) and set(value) <= set(schema['properties']), location + ': missing/extra fields')
        for key, child in value.items():
            validate_shape(child, schema['properties'][key], document, location + '.' + key)
    elif kind == 'array':
        need(len(value) >= schema.get('minItems', 0), location + ': too few items')
        if schema.get('uniqueItems'):
            need(len({runtime.compact(x) for x in value}) == len(value), location + ': duplicate items')
        for index, child in enumerate(value):
            validate_shape(child, schema['items'], document, location + '[' + str(index) + ']')
    elif kind == 'string':
        need(len(value) >= schema.get('minLength', 0) and len(value) <= schema.get('maxLength', len(value)), location + ': invalid length')
...
class Reader:
    ...
    def legacy_ref(self, ref, version):
        value = runtime.strict_json(self.ref(ref))
        filename = {'authoring-v1': 'authoring-record', 'validation-request-v1': 'validation-request'}[version]
        schema = runtime.strict_json((PACKAGE / 'schemas' / (filename + '.schema.json')).read_bytes())
        # Legacy semantics remain with their existing reader; this shape check
        # uses the actual frozen schema, and binds all legacy absolute inputs.
        need(value.get('schema_version') == version, 'legacy family mismatch')
        need(set(schema.get('required', [])) <= set(value), 'legacy required fields missing')
        if schema.get('additionalProperties') is False:
            need(set(value) <= set(schema['properties']), 'legacy extra fields')
        self.references_in(value)
        return value
    ...
    def set_authoring(self, value):
        ...
        for key in order:
            ...
            if row['authoring_record']:
                ...
                if row['package']:
                    manifest = runtime.strict_json(self.ref(record['delivered_manifest']))
                    need(record['target_root'] == row['package']['root'] and manifest['package_digest'] == row['package']['package_digest'], 'per-member delivered bytes mismatch')
                if row['status'] == 'AUTHORED':
            ...
            if row['validation_request']:
                ...
                need(row['package'] is not None and request['package_digest'] == row['package']['package_digest'] and request['authoring_record'] == row['authoring_record'], 'per-member validation request mismatch')
