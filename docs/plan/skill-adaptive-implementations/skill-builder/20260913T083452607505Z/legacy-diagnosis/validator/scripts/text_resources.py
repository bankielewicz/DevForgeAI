...
def frontmatter(text):
    ...
    for name in ('name','description'):
        if type(metadata.get(name)) is not str or not metadata[name].strip():
            raise ValueError('nonempty string required: ' + name)
    for name in ('license','compatibility'):
        if name in metadata and type(metadata[name]) is not str:
            raise ValueError('optional metadata string type: ' + name)
    if 'metadata' in metadata and type(metadata['metadata']) is not dict:
        raise ValueError('metadata mapping required')
    if 'allowed-tools' in metadata and type(metadata['allowed-tools']) not in (str,list):
        raise ValueError('allowed-tools string/list applicability')
    return metadata


def check(rule, subject, result, reason, applicability='applicable', required=True):
    return {'schema_version':'1','run_id':'helper','check_id':rule + '-' + observe.sha256(subject.encode())[:12], 'rule_id':rule,'subject_path':subject,'method':'deterministic','required':required,'applicability':applicability,'result':result,'reason':reason,'evidence':[]}
...
def candidates(path, text):
    ...
    for position, char in enumerate(text):
        ...
        nfkc = normalized != char and context in ('metadata','command','path')
        if special or nfkc:
...
def package(source, tokenizer_name=None, encoding=None):
    ...
    if 'SKILL.md' not in texts:
        ...
        try:
            meta = frontmatter(texts['SKILL.md'])
            checks.append(check('AV-F01','SKILL.md','PASS','Parsed unique-key YAML mapping and required strings; optional fields preserved.'))
            checks.append(check('AV-F02','SKILL.md','PASS' if meta['name'] == root.name else 'FAIL','Name/directory identity; semantic routing and selected limits require review.'))
            unknown = set(meta)-{'name','description','license','metadata','allowed-tools','compatibility'}
            if unknown:
    ...
    for path,text in texts.items():
        ...
        for line,target,kind in parsed:
            ...
            edges.append({'source':path,'line':line,'target':target,'kind':kind,'resolution':resolution})
            if resolution == 'resolved':
                adjacency[path].add(dest)
            elif resolution == 'missing':
                checks.append(check('AV-R01',path+':'+str(line)+':'+target,'FAIL','Referenced local resource is missing.'))
            elif resolution in ('dynamic','unsupported_anchor'):
                checks.append(check('AV-R01',path+':'+str(line)+':'+target,'NOT_RUN','Dynamic/reference syntax or renderer anchor needs manual resolution.'))
    if 'agents/openai.yaml' in texts:
        try:
            config = yaml_mapping(texts['agents/openai.yaml'])
            unknown = set(config)-{'interface','policy','dependencies'}
            interface = config.get('interface',{})
            if type(interface) is not dict:
                raise ValueError('interface mapping')
            known_interface = {'display_name','short_description','icon_small','icon_large','brand_color','default_prompt'}
            unknown |= set(interface)-known_interface
            if any(type(value) is not str for key,value in interface.items() if key in known_interface):
                raise ValueError('interface string type')
            policy = config.get('policy',{})
            if type(policy) is not dict or ('allow_implicit_invocation' in policy and type(policy['allow_implicit_invocation']) is not bool):
                raise ValueError('implicit invocation boolean')
            unknown |= set(policy)-{'allow_implicit_invocation'}
            dependencies = config.get('dependencies',{})
            if type(dependencies) is not dict or type(dependencies.get('tools',[])) is not list:
                raise ValueError('dependency shape')
            unknown |= set(dependencies)-{'tools'}
            for tool in dependencies.get('tools',[]):
                if type(tool) is not dict or not {'type','value'} <= tool.keys() or any(type(v) is not str for k,v in tool.items() if k in ('type','value','description','transport','url')):
                    raise ValueError('dependency tool shape')
                unknown |= set(tool)-{'type','value','description','transport','url'}
                if tool['type'] != 'mcp':
                    unknown.add('dependency type')
            for field in ('icon_small','icon_large'):
                if field in interface:
                    resolution,dest = resolve('SKILL.md',interface[field],texts,set(raw))
                    edges.append({'source':'agents/openai.yaml','line':1,'target':interface[field],'kind':'image','resolution':resolution})
                    if resolution == 'resolved':
                        adjacency['agents/openai.yaml'].add(dest)
                    elif resolution == 'missing':
                        raise ValueError('missing icon')
            checks.append(check('AV-F04','agents/openai.yaml','NOT_RUN' if unknown else 'PASS','Unknown extensions need applicable guidance.' if unknown else 'Supported optional shapes and local icons checked.', 'unknown' if unknown else 'applicable'))
        except ImportError:
            checks.append(check('AV-F04','agents/openai.yaml','NOT_RUN','Installed YAML parser unavailable.'))
        except Exception as error:
            checks.append(check('AV-F04','agents/openai.yaml','FAIL','Optional configuration parse/type/resource failure: ' + type(error).__name__))
    reachable, pending = set(), [p for p in ('SKILL.md','agents/openai.yaml') if p in raw]
    while pending:
        path = pending.pop()
