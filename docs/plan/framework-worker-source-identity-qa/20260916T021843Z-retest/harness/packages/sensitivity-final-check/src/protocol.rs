...
impl<'a> Session<'a> {
    ...
    fn rpc(&mut self, method: &str, params: Value) -> Result<Value> {
        self.control()?;
        if self.preflight_guard && !self.process.only_worker_started()? {
    ...
    pub fn dispatch(&mut self, r: &Request) -> Result<()> {
        if r.schema_version == 2 {
            let sources = r.verify_preflight_review()?;
