BeforeAll {
    ...
    $script:seal = [regex]::Match($script:text, "manifestSha256 = '([a-f0-9]{64})'").Groups[1].Value
...
Describe 'Operator PowerShell entrypoint' {
    BeforeEach {
        ...
        Mock Get-FileHash {
            ...
            if ($LiteralPath.EndsWith('runtime-manifest.json')) {
                return [pscustomobject]@{ Hash = $(if ($global:DiagnosticTestMode -eq 'manifest-drift') { 'bad' } else { $script:seal }) }
            }
    ...
    It 'prepares by default with literal module arguments and no native run flag' {
        ...
        Should -Invoke 'C:\Program Files\Python310\python.exe' -Times 1 -Exactly
    }
    ...
    It 'stops if the sealed runtime manifest changed' {
        ...
        Should -Invoke 'C:\Program Files\Python310\python.exe' -Times 0 -Exactly
    }
