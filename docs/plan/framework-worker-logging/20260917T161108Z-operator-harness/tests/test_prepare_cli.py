...
class PreparationCliTests(unittest.TestCase):
    def test_prepare_emits_bound_packet_without_native_dispatch(self):
        ...
        latch = TRIAL / "launch-invocation.json"
        reservation_before = latch.read_bytes() if latch.exists() else None
        ...
        self.assertFalse((TRIAL / "native-001").exists())
        self.assertEqual(reservation_before, latch.read_bytes() if latch.exists() else None)
        self.assertFalse((TRIAL / "run").exists())
        self.assertFalse((TRIAL / "operator-harness-001/launch-invocation-after-preparation-stop.json").exists())
