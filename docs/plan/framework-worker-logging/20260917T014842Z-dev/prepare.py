...
ROOT = pathlib.Path(r'C:\Projects\DevForgeAI')
EVIDENCE = ROOT / 'docs/plan/framework-worker-logging/20260917T014842Z-dev'
CANDIDATE = ROOT / 'devforgeai/experiments/codex-worker-probe-logging'
BASE = ROOT / 'docs/plan/framework-worker-diagnostics/20260916T181820Z-dev'
def digest(p): return hashlib.sha256(p.read_bytes()).hexdigest()
inputs = [ROOT/'AGENTS.md', BASE/'candidate-v2-manifest.json', BASE/'qa-handoff.md', BASE/'diagnostic-contract.md']
inputs += [ROOT/'docs/plan/framework-worker-startup-investigation/20260916T204555Z'/n for n in ['logging-design.md','investigation-report.md']]
inputs += [ROOT/'docs/plan/framework-worker-console-helper/20260916T231915Z/diagnosis.md', ROOT/'docs/plan/advisor-runs/20260916T185825Z-diagnostic-handoff/qa-handoff-addendum.md']
inputs += list((ROOT/'docs/specs/framework/runtime').glob('codex-worker-*.md'))
inputs += [ROOT/'docs/plan/framework-worker-native-diagnostics/20260916T202125Z/artifact-index.json', ROOT/'Start-CodexAppServerDiagnostic.ps1']
bindings = [{'path':str(p), 'sha256':digest(p)} for p in inputs]
manifest = json.loads((BASE/'candidate-v2-manifest.json').read_text())
for item in manifest:
    for tree in [BASE/'candidate-v2-snapshot', ROOT/'devforgeai/experiments/codex-worker-probe']:
        assert digest(tree/item['path']) == item['sha256'], tree/item['path']
assert not CANDIDATE.exists()
(EVIDENCE/'input-bindings.json').write_text(json.dumps(bindings, indent=2)+'\n')
(EVIDENCE/'context.md').write_text('''# Rust logging development context
...
CANDIDATE.mkdir()
for item in manifest:
    target = CANDIDATE/item['path']; target.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(BASE/'candidate-v2-snapshot'/item['path'], target)
    assert digest(target) == item['sha256']
print(json.dumps({'candidate':str(CANDIDATE),'copied_verified':len(manifest),'frozen_verified':len(manifest)*2}))
