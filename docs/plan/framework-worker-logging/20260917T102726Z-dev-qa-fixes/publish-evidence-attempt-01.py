...
write("preservation-readback.json", {"checked_at": now(), "checks": checks,
...
for attempt in ["01-red", "02-green", "04-refactor", "08-coverage"]:
    ...
    observations = [read(path) for path in files if path.name.endswith("-observation.json")]
...
required = ["context.md", "plan.md", "delivery.md", "resolution-map.json", "checkpoint.json",
            ...
            "preservation-readback.json", "inspection-observation-summary.json", "implementation-review.md",
