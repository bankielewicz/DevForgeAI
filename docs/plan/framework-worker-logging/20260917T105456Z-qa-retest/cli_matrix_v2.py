...
def prepare():
    ...
    assert not (ROOT / 'cli-matrix-plan-v2.json').exists()
    save(ROOT/'cli-matrix-plan-v2.json',plan)
    print(json.dumps({'catalog':len(catalog),'subcases':len(entries),'success':sum(e['group']=='success' for e in entries),'compatibility':sum(e['group']=='compatibility' for e in entries),'plan_sha256':sha(ROOT/'cli-matrix-plan-v2.json')},indent=2))
...
def execute(group):
    identities()
    plan=load(ROOT/'cli-matrix-plan-v2.json')
    ...
    attempt=ROOT/'attempts'/('18-rt01-corrected' if group=='success' else '19-rt02')
    attempt.mkdir(exist_ok=False)
    save(attempt/'launch.json',{'action':'CLI matrix '+group,'started_utc':datetime.now(timezone.utc).isoformat(),'plan_sha256':sha(ROOT/'cli-matrix-plan-v2.json'),'script_sha256':sha(Path(__file__)),'executable_sha256':sha(EXE)})
    records=[]
    start=time.monotonic()
    for entry in [entry for entry in plan['entries'] if entry['group']==group]:
        ...
        request=load(run/'request.json')
        # Validated Windows requests retain Rust canonicalize's verbatim drive prefix.
        assert request['run_dir'].startswith('\\\\?\\') and run.is_absolute()
        request['run_dir']='\\\\?\\' + str(run.resolve())
