...
fn qa_01_config_denials_and_exact_boundary() {
    ...
    for mode in ["digest","traversal","junction"] {
        ...
        if mode=="digest" {input["diagnostics_sha256"]=json!("0".repeat(64));}
            ...
            let out=std::process::Command::new("C:/Windows/System32/cmd.exe").args(["/d","/c","mklink","/J"]).arg(&alias).arg(&target).output().unwrap();
...
fn qa_13_preservation_and_documentation() {
    ...
    let readme=fs::read_to_string("C:/Projects/DevForgeAI/devforgeai/experiments/codex-worker-probe-logging/README.md").unwrap();
