...
function DisplayTarget([string]$Path) {
    # PowerShell omits the extended-path prefix present in Python's retained observation.
    # Strip only that display prefix; do not resolve, traverse or rewrite any junction.
...
foreach ($runName in @('20260917T105456Z-qa-retest','20260917T102726Z-dev-qa-fixes','20260917T023835Z-qa')) {
    ...
    foreach ($link in $evidence.reparse_points) {
        ...
        $target = @($item.Target)
        $matches = $target.Count -eq 1 -and (DisplayTarget $target[0]) -ceq (DisplayTarget $link.target) -and [bool]($item.Attributes -band [IO.FileAttributes]::ReparsePoint)
        if (-not $matches) { $problems.Add("Evidence reparse metadata drift: $($link.path)") }
        $evidenceLinks.Add([ordered]@{path=$link.path; target=$target; retained_target=$link.target; comparison='Exact target after removing only extended-path display prefix'; matches=$matches; traversed=$false})
    }
}
...
if ($Phase -eq 'readback') {
    $baseline = Get-Content -Raw -LiteralPath (Join-Path $PSScriptRoot 'intake-v2-verification.json') | ConvertFrom-Json
...
WriteNewJson (Join-Path $PSScriptRoot "$Phase-v2-verification.json") $result
