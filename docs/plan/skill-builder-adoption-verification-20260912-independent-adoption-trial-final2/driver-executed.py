...
def command(name, args, cwd=None):
    ...
    input_snapshot = None
    if '--candidate-root' in args and '--cases' in args:
        original_root=pathlib.Path(args[args.index('--candidate-root')+1])
        original_cases=pathlib.Path(args[args.index('--cases')+1])
        capture=ROOT/'evaluation-inputs'/name
        before=manifest(original_root)
        shutil.copytree(original_root,capture/'candidate-root')
        shutil.copyfile(original_cases,capture/'cases-input.jsonl')
        assert before==manifest(capture/'candidate-root')==manifest(original_root)
        assert digest(original_cases)==digest(capture/'cases-input.jsonl')
        write(capture/'candidate-manifest.json',before)
        input_snapshot={'captured_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'original_candidate_root':str(original_root),'preserved_candidate_root':str(capture/'candidate-root'),'full_manifest':str(capture/'candidate-manifest.json'),'full_manifest_sha256':digest(capture/'candidate-manifest.json'),'original_cases':str(original_cases),'preserved_cases':str(capture/'cases-input.jsonl'),'cases_sha256':digest(capture/'cases-input.jsonl'),'builder_manifest_sha256':digest(BUILDER/'evals/build-manifest.json'),'outcome':'PRE_EVALUATION_COPY_AND_LIVE_READBACK_MATCH'}
    result = subprocess.run(args, cwd=cwd or ROOT, capture_output=True, text=True, encoding='utf-8')
    write(ROOT / 'commands' / (name+'.json'), {'timestamp':started,'cwd':str(cwd or ROOT),'argv':list(map(str,args)),'exit_code':result.returncode,'stdout':result.stdout,'stderr':result.stderr,'evaluation_input_snapshot':input_snapshot})
