...
for entry in index['files']:
    ...
    retained_after_candidate=retained_run/original_candidate.relative_to(original_run)
    capture=command['evaluation_input_snapshot']
    assert capture['original_candidate_root']==str(original_candidate)
    assert capture['original_cases']==str(original_cases)
    retained_candidate=pathlib.Path(capture['preserved_candidate_root'])
    retained_cases=pathlib.Path(capture['preserved_cases'])
    capture_manifest=pathlib.Path(capture['full_manifest'])
    assert trial.digest(capture_manifest)==capture['full_manifest_sha256']
    assert trial.manifest(retained_candidate)==json.loads(capture_manifest.read_text())
    assert trial.digest(retained_cases)==capture['cases_sha256']
    ...
    after_files={p.relative_to(retained_after_candidate).as_posix():trial.digest(p) for p in retained_after_candidate.rglob('*') if p.is_file()}
    added=[{'path':name,'after_sha256':after_files[name],'classification':'Present after the run but absent in the preserved pre-evaluation snapshot; not part of the recorded measured input set.'} for name in sorted(set(after_files)-set(current_files))]
    replaced=[{'path':name,'pre_evaluation_sha256':current_files[name],'after_run_sha256':after_files[name],'preserved_pre_evaluation_locator':str(retained_candidate/name)} for name in sorted(set(after_files)&set(current_files)) if after_files[name]!=current_files[name]]
    audits.append({'original_result':str(original_result),'retained_result':str(retained_result),'result_sha256':result_hash,'command_receipt':str(command_path),'command_receipt_sha256':trial.digest(command_path),'original_candidate_root':str(original_candidate),'retained_candidate_root':str(retained_candidate),'pre_evaluation_capture_receipt':capture,'full_pre_evaluation_snapshot_readback_passed':True,'retained_after_run_candidate_root':str(retained_after_candidate),'post_snapshot_added_files':added,'post_snapshot_replaced_files':replaced,'measured_union_count':len(measured_union),'current_retained_root_file_count':len(current_files),'unmeasured_current_files':extra_files,'records':row_audits,'passed':all(x['passed'] for x in row_audits)})
audit={'schema_version':'1','audited_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'scope':'Every cases_sha256 and every candidate_digests entry emitted by the 19 records in the eight indexed final-chain evaluator result files, read directly against the immutable pre-evaluation candidate/cases snapshots recorded in each actual command receipt. Measurements are not expanded to all files later present in a reused root.','result_files':audits,'summary':{'result_file_count':len(audits),'record_count':total_cases,'cases_digest_checks':total_cases,'candidate_digest_checks':total_candidates,'immutable_input_snapshot_count':len(audits),'mapped_measurement_count':len(mappings),'distinct_mapped_retained_files':len({x['retained_absolute_locator'] for x in mappings}),'unavailable_or_mismatched_count':len(missing),'passed':not missing},'explicit_locator_mappings':mappings,'unavailable_or_mismatched':missing,'mutations':'This audit reads original result/input files and writes new audit artifacts only. No measured bytes or paths are replaced.'}
