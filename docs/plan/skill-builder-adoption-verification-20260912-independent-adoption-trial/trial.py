...
def resolution_execute():
    ...
    write(ROOT/'resolution-edit-readback.json',{'instruction':resolution,'before':before,'after':manifest(target),'restored_path':'scripts/receipt_totals.py','restored_sha256':digest(edited),'timestamp':datetime.datetime.now(datetime.timezone.utc).isoformat()})
