...
def capture(label, args, cwd=PACKAGE, timeout=300, extra=None):
    ...
    receipt['environment_overrides'] = {'CARGO_TARGET_DIR': env['CARGO_TARGET_DIR'], 'WF_TEST_EVIDENCE': env['WF_TEST_EVIDENCE'], **(extra or {})}
    ...
    final_manifest = [{'path': str(p), 'bytes': p.stat().st_size, 'sha256': sha(p)}
                      for p in sorted(PACKAGE.rglob('*'))
                      if p.is_file() and 'target' not in p.relative_to(PACKAGE).parts]
    (attempt / 'candidate-readback.json').write_text(json.dumps(final_manifest, indent=2), encoding='utf-8')
    receipt['candidate_unchanged_during_command'] = manifest == final_manifest
...
if __name__ == '__main__':
    ...
    elif mode == 'owned-format':
        capture('root-owned-format',[CARGO.with_name('rustfmt.exe'),'--edition','2024',
            *[PACKAGE / p for p in ['src/main.rs','src/runner.rs','src/protocol.rs','src/journal.rs','src/process_windows.rs',
                                    'tests/preflight_cli.rs','tests/preflight_recovery.rs']]],timeout=60)
    else:
