...
fn fixed_config(config: &Value, exact: bool) -> Result<()> {
...
pub fn validate_config(
    ...
    fixed_config(root.get("config").ok_or("protocol_error")?, false)?;
    let origins = root
        .get("origins")
        .and_then(Value::as_object)
        .ok_or("protocol_error")?;
    ...
    for layer in layers {
        ...
        match source_type {
            "sessionFlags" => {
                session_flags += 1;
                fixed_config(layer.get("config").ok_or("protocol_error")?, true)?;
            }
            _ if disk_source => {
                disk_layers += 1;
            }
