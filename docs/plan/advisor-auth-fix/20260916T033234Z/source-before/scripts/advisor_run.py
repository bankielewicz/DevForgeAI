...
DEFAULTS = {"type": "approach", "model": "opus", "effort": "high", "total_budget_usd": "2.00", "timeout_seconds": 300}
...
def validate_request(value):
    ...
    for field, choices in (("type", ("approach", "stuck", "done", "reconcile")), ("model", ("opus", "sonnet")), ("effort", ("high", "max"))):
...
def execute_process(argv, cwd, timeout):
    try:
        process = subprocess.run(argv, cwd=cwd, stdin=subprocess.DEVNULL, capture_output=True, timeout=timeout, shell=False)
...
def preflight(executable, cwd, execute=execute_process):
    results = {key: execute([str(executable), "--" + key], cwd, 15) for key in ("version", "help")}
    ...
    return {"version": results["version"]["stdout"].decode("utf-8", errors="replace").strip(),
            ...
            "qualification": "HELP_ONLY; native behavior requires separate evidence"}


def build_command(request, attempt, cap):
    ...
    return [request["claude_path"], "--print", prompt, "--model", request["model"], "--effort", request["effort"],
...
def validate_prior_attempt(attempt, number, request, request_hash, cap):
    try:
        ...
        if (not isinstance(receipt, dict) or receipt.get("schema") != "advisor-execution-v1"
                ...
                or receipt.get("command") != build_command(request, attempt, cap)
...
def run_review(request, run_dir, briefing, reason="initial", execute=execute_process):
    ...
    try:
        ...
        command = build_command(request, attempt, cap)
        started = {"schema": "advisor-execution-v1", "started_utc": now(), "cwd": request["repo_root"],
        ...
        process = execute(command, request["repo_root"], request["timeout_seconds"])
...
def main(argv=None):
    ...
    try:
        result = preflight(args.claude.resolve(strict=True), Path.cwd()) if args.command == "preflight" else run_review(read_json(args.request), args.run_dir, args.briefing, args.reason)
