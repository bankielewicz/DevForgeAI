...
Supported implementation target: native Windows with Python 3.10+ standard library and a native Claude CLI executable. Linux behavior is unqualified. No MCP or GUI dependency. Claude authentication must already be configured; the runner inherits it without reading or recording credential environment values. Do not install or change authentication as a recovery step.
...
```powershell
python -B -X utf8 C:/Projects/DevForgeAI/src/agents/skills/advisor/scripts/advisor_run.py preflight --claude C:/Users/bryan/.local/bin/claude.exe
```
...
```json
{
  ...
  "contract_sha256": "af4cd7f0505c92a186c29edc17328107ac2103f6069109c5aacac04da32b18ab",
