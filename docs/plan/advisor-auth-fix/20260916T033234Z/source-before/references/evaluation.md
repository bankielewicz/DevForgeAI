...
The JSONL grader uses exact comparison against fixture expectations. Results retain one record per required fixture, including ERROR/FAIL; retries use new directories and never erase earlier attempts. No mocks or live model calls are used by the JSONL corpus. Unit tests use explicit simulated CLI results to exercise failures; subprocess tests separately execute real local Python child processes. Neither establishes live Claude confinement or reasoning quality.
