...
def origin(contract):
    ...
    if value.get('schema_version') in ('1', '2') and value.get('result') == 'COMPLETE':
        ...
        if value['schema_version'] == '2':
            ...
            historical_contract = parse(captured_files[cp])
            if digest(captured_files[cp]) != value['contract_sha256'] or historical_contract.get('target_name') != value['target_name']:
...
def begin(contract_path, run_root):
    ...
    if not required <= set(c) or c['schema_version'] != 'authoring-contract-v1':
