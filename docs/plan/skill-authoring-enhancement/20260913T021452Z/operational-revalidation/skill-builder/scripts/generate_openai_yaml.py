...
from authoring import safe, read_bytes
...
def write_openai_yaml(skill_dir, skill_name, raw_overrides):
    ...
    skill_dir = safe(skill_dir, write=True)
    output_path = safe(skill_dir / 'agents/openai.yaml', write=True)
    original = read_bytes(output_path) if output_path.exists() else None
    if original is not None:
        # Focused updates preserve unrelated YAML fields including policy and
        # dependencies. The original generator replaced the complete file.
        import yaml
        value = yaml.safe_load(original.decode('utf-8-sig'))
        if not isinstance(value, dict) or not isinstance(value.get('interface', {}), dict):
            raise ValueError('openai.yaml must contain mappings')
        value.setdefault('interface', {}).update(overrides)
        content = yaml.safe_dump(value, allow_unicode=True, sort_keys=False)
        if read_bytes(output_path) != original:
            raise ValueError('metadata changed before write')
    ...
    output_path = safe(agents_dir / "openai.yaml", write=True)
...
def main():
    ...
    parser.add_argument('--allow-implicit-invocation', choices=('true', 'false'), help='Set only when explicitly requested; otherwise preserve existing policy')
    args = parser.parse_args()

    skill_dir = safe(args.skill_dir, write=True)
    ...
    if result:
        if args.allow_implicit_invocation is not None:
            import yaml
            original = read_bytes(result)
            value = yaml.safe_load(original.decode('utf-8-sig'))
            value.setdefault('policy', {})['allow_implicit_invocation'] = args.allow_implicit_invocation == 'true'
            if read_bytes(result) != original:
                raise ValueError('metadata changed before policy write')
            result.write_text(yaml.safe_dump(value, allow_unicode=True, sort_keys=False), encoding='utf-8')
        sys.exit(0)
    sys.exit(1)
