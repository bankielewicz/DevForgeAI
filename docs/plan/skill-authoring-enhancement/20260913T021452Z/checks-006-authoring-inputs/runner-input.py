...
def retained_run(command, *args, **kwargs):
    ...
    # Scaffolder/metadata helpers use positional paths and --path rather than
    # evaluator flags. Capture these actual disposable inputs too.
    for i, token in enumerate(command[2:], start=2):
        source = Path(token)
        if source.is_absolute() and source.is_relative_to(TEMP) and source.exists():
            dest = evidence / ('positional-input-' + str(i))
            if source.is_dir():
                shutil.copytree(source, dest)
            elif source.is_file():
                shutil.copy2(source, dest)
            inputs['argument-' + str(i)] = {'path': str(source), 'snapshot': str(dest)}
...
suite = unittest.defaultTestLoader.discover(str(STAGE / 'skill-validator/tests'), pattern='test_authoring.py' if 'authoring-only' in sys.argv[2:] else 'test*.py')
