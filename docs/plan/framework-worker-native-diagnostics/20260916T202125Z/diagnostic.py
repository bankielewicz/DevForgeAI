    ...
    with (attempt / "stdout.bin").open("xb") as stdout, (attempt / "stderr.bin").open("xb") as stderr:
        try:
            ...
            write(attempt / "owned-process.json", {"pid": child.pid, "start_utc": utc_now(),
                  "argv": argv, "ownership": "live subprocess handle", "window": "inherited terminal"})
            try:
...
def prepare(version, inventory_path):
    ...
    observed_cwd = inventory["fixture_cwd"]
    if observed_cwd.startswith("\\\\?\\"):
        observed_cwd = observed_cwd[4:]
    assert observed_cwd.lower() == str(FIXTURE).lower()
    candidate_check()
