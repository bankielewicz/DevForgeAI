...
Include substantive change content in the populated specification or an assigned standalone artifact location. It is an authoring record, not a model-issued completion receipt. For managed operation, follow [managed-authoring.md](managed-authoring.md): runtime owns final identity checks and publication/readback; no manual phase or receipt-helper fallback. Never request the containing specification's own digest. Record:
...
- New or unchanged specification ID, revision, path and preserved requirement IDs. A containing document excludes its own digest; runtime binds final bytes externally.
