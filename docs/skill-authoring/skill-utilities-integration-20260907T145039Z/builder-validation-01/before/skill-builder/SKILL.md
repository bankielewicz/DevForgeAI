...
## Deliver the authoring receipt and stop

Return specification and canonical file links, creation/enhancement/reuse rationale with search limits, source-to-installation mapping, and material contract, dependency, or enforcement gaps. Record the candidate's before/after identities, scope, changed files, preserved behavior, and finding dispositions in the builder change receipt. Missing identities stay explicit.

For remediation, map F-### findings and CHG-### changes to applied, deferred, or declined, with old/new path and SHA-256 identities, preserved requirement IDs, and reasons. Record partially completed or blocked work as deferred with its remaining scope. An edit does not resolve a validation result. Return the new candidate and receipt for a separate validator run under the selected contract; do not launch it automatically.

Report "Validation status: Not performed." For proposed hooks, also report "Hook status: Design only." Finish after authoring; do not label the candidate tested, validated, production-ready, accepted, or actively enforced.
