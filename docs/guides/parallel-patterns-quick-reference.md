...
| Cleanup on skip/defer | Yes (KillShell) |

### Retrieve Results

```python
# Check if complete
output = TaskOutput(task_id="bash_1")
...
7-10 tasks:   Maximum, use with caution
...
**Symptom:** TaskOutput shows running but never completes
...
```python
TaskOutput(task_id=task_id)  # Must happen
```
...
# Better approach:
Task(subagent_type="code-generator", prompt="Generate code...")
Task(subagent_type="code-analyzer", prompt="Analyze parallel design...")
...
test_output = TaskOutput(task_id=bash_test)
lint_output = TaskOutput(task_id=bash_lint)
```
...
- `docs/guides/parallel-orchestration-guide.md` (detailed patterns with code)

**Architecture Constraints:**
- `devforgeai/specs/context/architecture-constraints.md` (Parallel Execution Rules section)

**This Document:**
- `docs/guides/parallel-patterns-quick-reference.md` (you are here)
...
See full documentation in `docs/guides/parallel-orchestration-guide.md`
