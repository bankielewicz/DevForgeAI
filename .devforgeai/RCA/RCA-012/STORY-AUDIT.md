...
```bash
Read .ai_docs/Stories/STORY-038-code-quality-metrics-validation-enhancement.story.md
```
...
**File:** `.ai_docs/Stories/STORY-038-code-quality-metrics-validation-enhancement.story.md`
...
grep "^## Approved Deferrals" .ai_docs/Stories/STORY-038*.story.md

# Verify user approval timestamp
grep "User Approval: 2025-01-21" .ai_docs/Stories/STORY-038*.story.md

# Count deferred items documented
grep -c "^\*\*Deferred Items:\*\*" .ai_docs/Stories/STORY-038*.story.md
# Expected: 1 (section header)

# Verify all 4 items listed
grep -A 50 "Deferred Items:" .ai_docs/Stories/STORY-038*.story.md | grep -c "^[0-9]\."
...
```
Read .ai_docs/Stories/{STORY-ID}-{slug}.story.md
```
...
grep "^## Approved Deferrals" .ai_docs/Stories/{STORY-ID}*.story.md

# Verify user approval present
grep "User Approval:.*UTC" .ai_docs/Stories/{STORY-ID}*.story.md

# Verify all incomplete items documented
UNCHECKED=$(grep -c "^- \[ \]" .ai_docs/Stories/{STORY-ID}*.story.md)
DEFERRED=$(grep -A 50 "Deferred Items:" .ai_docs/Stories/{STORY-ID}*.story.md | grep -c "^[0-9]\.")
