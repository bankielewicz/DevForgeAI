...
1. **`epic:` Field Prevalence (95%):**
   ```bash
   grep -c "^epic:" .ai_docs/Stories/STORY-*.story.md | grep ":1" | wc -l
   # Result: 60 out of 63 stories
   ```

2. **Valid Epic References (89%):**
   ```bash
   grep -h "^epic:" .ai_docs/Stories/STORY-*.story.md | sort | uniq -c
   # Result: 56 have EPIC-XXX, 4 have None/null/TBD
   ```

3. **Bidirectional Mapping (38%):**
   ```bash
   grep -c "## Stories" .ai_docs/Epics/EPIC-*.epic.md | grep ":1" | wc -l
...
```bash
grep "^epic:" .ai_docs/Stories/STORY-*.story.md
# Output: .ai_docs/Stories/STORY-066-npm-package-creation-structure.story.md:epic: EPIC-012
```

**Extract `## Stories` table from epics:**
```bash
# Read epic file, grep for table rows after ## Stories header
grep -A 20 "## Stories" .ai_docs/Epics/EPIC-012-npm-package-distribution.epic.md
```
...
epic_id="EPIC-012"
grep -l "^epic: $epic_id" .ai_docs/Stories/*.story.md
```
...
- **Related Documents:**
  - EPIC-015: `.ai_docs/Epics/EPIC-015-epic-coverage-validation-traceability.epic.md`
  - RESEARCH-002: `.devforgeai/research/shared/RESEARCH-002-epic-coverage-traceability.md`
  - Evidence: `grep "^epic:" .ai_docs/Stories/STORY-*.story.md` (60/63 = 95%)
