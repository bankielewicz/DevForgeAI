...
**Source:** DevForgeAI research + official docs (consolidated 2025-11-06, updated 2026-01-27)
...
| Section | Topics | Key Metrics | Jump To |
|---------|--------|-------------|---------|
| **1. Workflow Design** | Command architecture, naming, skill integration | 15K char budget | [↓](#section-1-workflow-design-best-practices) |
| **2. Prompt Engineering** | XML tags, few-shot examples, chain-of-thought | 40% error reduction | [↓](#section-2-prompt-engineering-for-claude-code) |
| **3. System Prompts** | Role definition, constraints, context hierarchy | Long-context optimization | [↓](#section-3-system-prompt-best-practices) |
| **4. Token Efficiency** | Native tools vs Bash, parallel calls | 40-73% savings | [↓](#section-4-token-efficiency--native-tools) |
| **5. Common Workflows** | Git, testing, refactoring, API patterns | Established patterns | [↓](#section-5-common-workflows) |
| **6. Plan Mode** | When to plan, exploration patterns | Reduce wasted effort | [↓](#section-6-plan-mode-usage) |
| **7. Summary** | Quick wins, anti-patterns, optimization checklist | Essential checklist | [↓](#section-7-best-practices-summary) |
| **8. Advanced Patterns** | Progressive disclosure, lean orchestration, feedback loops | 2026 DevForgeAI patterns | [↓](#section-8-devforgeai-advanced-patterns-2026-updates) |

**File Size:** 1,416 lines | **Evidence-Based:** All claims backed by measurements
</quick_index>
...
1. [Workflow Design Best Practices](#section-1-workflow-design-best-practices)
2. [Prompt Engineering for Claude Code](#section-2-prompt-engineering-for-claude-code)
3. [System Prompt Best Practices](#section-3-system-prompt-best-practices)
4. [Token Efficiency & Native Tools](#section-4-token-efficiency--native-tools)
5. [Common Workflows](#section-5-common-workflows)
6. [Plan Mode Usage](#section-6-plan-mode-usage)
7. [Best Practices Summary](#section-7-best-practices-summary)
8. [DevForgeAI Advanced Patterns (2026 Updates)](#section-8-devforgeai-advanced-patterns-2026-updates)
...
- Avoid overly generic names that WILL conflict with built-in commands (use domain-specific prefixes)
...
### Edge Case Examples: Token Efficiency

<example type="edge_case">
**Edge Case 1: File Too Large for Single Read**

**❌ PROBLEM:**
```
Read(file_path="logs/system.log")  # 50,000 lines

**Result:**
- Read tool truncates at 2,000 lines
- Missing lines 2,001-50,000
- Incomplete analysis
```

**✅ RECOVERY OPTIONS:**

**Option A: Use offset and limit**
```
# Read first 2,000 lines
Read(file_path="logs/system.log", limit=2000)

# Read next 2,000 lines (lines 2001-4000)
Read(file_path="logs/system.log", offset=2000, limit=2000)
...
```
# Instead of reading entire log, search for errors
Grep(pattern="ERROR|FATAL", path="logs/system.log", output_mode="content")

# Much more efficient - returns only relevant lines
```

**Why This Happens:** Read tool has 2,000 line limit per request to prevent context overflow. Large files require pagination or targeted search.
</example>

<example type="edge_case">
**Edge Case 2: Bash Required for Complex Pipe Chains**

**❌ PROBLEM:**
```
# Need to find Python files, count lines, sort by size, show top 10

Using only native tools:
files = Glob(pattern="**/*.py")
# Then what? Native tools don't chain like Bash pipes
```

**✅ CORRECT APPROACH:**

**Use Bash for complex shell operations that genuinely need pipe chains:**
```
Bash(command="find . -name '*.py' | xargs wc -l | sort -rn | head -10",
     description="Find top 10 largest Python files by line count")
```

**Reasoning:**
- Native tools excel at **individual operations**
- Bash excels at **complex pipe chains**
- This operation requires: find → count → sort → filter
- Native tool equivalent would require multiple calls + Python processing
- Bash pipe chain is more efficient for this specific use case

**Rule:** Use native tools for single file operations, Bash for complex multi-step shell workflows
</example>

<example type="edge_case">
**Edge Case 3: Glob Pattern Doesn't Match Expected Files**
...
**Result:**
- Returns: []
- Expected: test files in tests/ directory
- Files exist but not found
```

**✅ DIAGNOSIS & FIX:**

**Diagnosis:** Glob is recursive by default but searches from CWD
```
# Check current working directory
Bash(command="pwd", description="Check current directory")
# Result: /mnt/c/Projects/MyApp

# Files are in: /mnt/c/Projects/MyApp/tests/*.test.js
# Pattern "*.test.js" looks for files in CWD, not subdirectories
```

**Fix: Use recursive pattern**
```
Glob(pattern="**/*.test.js")
# Recursively searches all subdirectories for *.test.js files
```

**Why This Happens:** Glob patterns without `**/` only match files in the current directory. Always use `**/*.ext` for recursive search.
</example>

<example type="edge_case">
**Edge Case 4: Grep Returns "Files With Matches" But Need Content**
...
**Result:**
- Returns: ["src/auth.js", "src/login.js"]
- Need: Actual function code, not just file names
```

**✅ FIX: Use output_mode parameter**
```
Grep(pattern="function authenticate",
     output_mode="content",
     -A=10,
...
**Result:**
- Returns: Matching lines with 10 lines of context after each match
- Includes line numbers for precise location
- Full function code visible
```

**Default Behavior:**
- `output_mode="files_with_matches"` - Returns only file paths (default)
- `output_mode="content"` - Returns matching lines with context
- `output_mode="count"` - Returns match counts per file

**Why This Happens:** Grep defaults to file listing for efficiency. Specify `output_mode="content"` when you need the actual code.
</example>

<example type="edge_case">
**Edge Case 5: Context Window Approaching Limit**

**❌ PROBLEM:**
```
# Session includes:
- 50 file reads (~75,000 tokens)
- 30 Grep operations (~60,000 tokens)
- 20 Edit operations (~30,000 tokens)
- Total: ~165,000 tokens (approaching 200K limit)

Next operation requires large file:
Read(file_path="schema.sql")  # Would add 40,000 tokens

**Risk:** Context overflow, session restart required
```

**✅ PREVENTION & RECOVERY:**

**Prevention: Strategic Tool Selection**
```
# Instead of reading entire schema.sql:
Grep(pattern="CREATE TABLE users", path="schema.sql", output_mode="content", -A=20)

# Reads only relevant section (~2,000 tokens vs 40,000)
# Saves 38,000 tokens
```

**Recovery: Session Checkpoint**
```
# If approaching limit, checkpoint progress:
1. Write summary of findings to file:
   Write(file_path="analysis-checkpoint.md", content="...")

2. Restart session with checkpoint:
   Read(file_path="analysis-checkpoint.md")
   # Continue from checkpoint
```

**Monitoring:**
- Track token usage in session
- Use targeted searches (Grep) over full reads (Read) when possible
- Batch operations to stay within budget
- Checkpoint progress before approaching limits

**Why This Happens:** Even with native tools, 115+ operations can consume significant context. Strategic tool selection and checkpointing prevent overflow.
</example>
...
## Section 8: DevForgeAI Advanced Patterns (2026 Updates)

*New patterns added as of 2026-01-27*

### AC Compliance Verification (EPIC-046)

**Pattern:** Fresh-context verification of acceptance criteria at key workflow checkpoints.

**When to Use:**
- Phase 4.5 (post-refactoring) - Verify AC before integration tests
- Phase 5.5 (post-integration) - Verify AC after integration tests

**Implementation:**
```markdown
Phase 04.5: AC Compliance Verification

Invoke ac-compliance-verifier subagent with:
- Story file path
- No prior coding context (fresh start)

The subagent:
1. Reads story file (AC definitions)
2. Locates implementation files
3. Validates each AC fulfilled
4. Returns PASS/FAIL per AC

HALT if any AC fails verification.
```
...
### AI Analysis Hooks (Automatic Feedback)

**Pattern:** Event-driven capture of AI-generated improvement recommendations after workflow completion.

**Hooks:**
- `post-dev-ai-analysis` - Captures insights after /dev completes
- `post-qa-ai-analysis` - Captures insights after /qa completes

**What is Captured:**
```yaml
what_worked_well:
  - Pattern that proved effective
  - Framework feature that helped
areas_for_improvement:
  - Non-aspirational improvement
  - Specific, actionable recommendation
recommendations:
  - Implementable within Claude Code Terminal
  - References specific files/sections
```

**Output Location:** `devforgeai/feedback/ai-analysis/{STORY_ID}/`

**Manual Trigger:**
```bash
/feedback --type=ai_analysis
```

---

### Deferral Validation (RCA-006)

**Pattern:** All deferred DoD items MUST have justified technical reasons and proper documentation.

**Validation Checkpoints:**
1. **Dev Phase 06:** Challenge ALL deferrals (pre-existing + new)
2. **QA Phase 0 Step 2.5:** Validate deferral justifications

**Valid Deferral Criteria:**
- Technical blocker documented
- ADR or story reference provided
- User approval with timestamp

**Invalid Deferral Patterns (CRITICAL):**
- Circular deferrals (STORY-A → STORY-B → STORY-A)
- Missing justification
- No follow-up reference

**Enforcement:**
```markdown
IF deferral.justification IS EMPTY:
    HALT - Require justification

IF deferral.reference IS EMPTY:
    HALT - Require ADR or story reference

IF deferral.user_approval IS EMPTY:
    HALT - Require explicit user approval with timestamp
```

---

### Git Worktree Patterns (EPIC-010)

**Pattern:** Use isolated Git worktrees for parallel story development.

**When to Use:**
- Multiple stories in development simultaneously
- Stories with file conflicts need isolation
- CI/CD pipelines need story-specific branches

**Workflow:**
```bash
# Phase 0 Step 0.2 (automatic via /dev)
git-worktree-manager creates:
- worktree at {base_path}/{STORY_ID}
- branch {STORY_ID}-dev
- isolated development environment

# /worktrees command to view
/worktrees

# Output shows:
- Active worktrees with story associations
...
**Best Practices:**
- Max 5 concurrent worktrees (configurable)
- Cleanup idle worktrees after 7 days
- Use /worktrees to monitor status

---

### Feedback System Integration

**Pattern:** Capture, persist, and query feedback across sessions.
...
| `/export-feedback` | Portable ZIP export |
| `/import-feedback` | Import from ZIP |

**Feedback Types:**
- `retrospective` - Human-provided feedback
- `ai_analysis` - AI-generated insights
- `process_improvement` - Framework improvement ideas

**Search Example:**
```bash
/feedback-search --type=ai-analysis --priority=high --last 30
```

---

### Observation Extraction (STORY-318/319)

**Pattern:** Extract structured observations from subagent outputs for framework self-improvement.

**When to Use:**
- At phase exit gates
- After high-frequency subagent completion (test-automator, code-reviewer, backend-architect)

**Observation Categories:**
| Category | Description | Severity |
|----------|-------------|----------|
| `friction` | Workflow pain point | HIGH |
| `gap` | Missing coverage | MEDIUM |
| `pattern` | Recurring issue | MEDIUM |
| `success` | What worked well | LOW |
| `warning` | Non-blocking issue | MEDIUM |

**Implementation:**
```markdown
At phase exit:
1. Check for subagent outputs in context
2. Invoke observation-extractor if outputs available
3. Append observations to phase-state.json
