...
Comprehensive test suite generated for STORY-139: Skill Loading Failure Recovery. The tests validate error handling when the `/ideate` command's `devforgeai-ideation` skill fails to load, ensuring users receive clear repair instructions and the session remains stable.
...
4. **Recovery Actions**
   - FILE_MISSING: `git checkout .claude/skills/devforgeai-ideation/`
...
- **Error Handling Reference:** `.claude/skills/devforgeai-ideation/references/error-handling.md`
