...
```
current_phase_id = "07"  // Extracted from filename: phase-{NN}-*.md → NN

phase_steps = registry[current_phase_id].steps  // Filter for current phase only

IF phase_steps is empty:
    Display: "No steps defined for phase 07 in registry"
```
...
FOR each step in phase_steps:
    // Example: "Step 07.1: Update DoD items" or "Step 07.2: Validate format"
    subject = "Step 07.{step.id}: {step.check}"
    ...
    TaskCreate(
        ...
        activeForm="Executing Step 07.{step.id}"
    )
```
