...
```
current_phase_id = "08"  // Extracted from filename: phase-{NN}-*.md → NN

phase_steps = registry[current_phase_id].steps  // Filter for current phase only

IF phase_steps is empty:
    Display: "No steps defined for phase 08 in registry"
```
...
FOR each step in phase_steps:
    // Example: "Step 08.1: Stage files" or "Step 08.2: Commit changes"
    subject = "Step 08.{step.id}: {step.check}"
    ...
    TaskCreate(
        ...
        activeForm="Executing Step 08.{step.id}"
    )
```
