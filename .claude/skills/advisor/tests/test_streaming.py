...
class StreamProcessTests(unittest.TestCase):
    ...
    def test_continuously_filled_queue_still_obeys_cleanup_ceiling(self):
        ...
        try:
            ...
            # The deliberate overrun outlives execute_stream's cleanup ceiling.
            # The fixture owns those remaining resources after the call returns.
            for child in children:
                if child.poll() is None:
                    child.kill()
                child.wait(timeout=5)
            for reader in readers:
                reader.join(timeout=streaming.CLEANUP_SECONDS + 1)
            for child in children:
                child.stdout.close()
                child.stderr.close()
