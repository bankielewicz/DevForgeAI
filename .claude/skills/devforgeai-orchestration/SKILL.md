...
- `**Operation:** plan-sprint` + `**Sprint Name:** {name}` → Sprint Planning Mode
...
1. **Sprint Planning Mode** - Phase 3 workflow (capacity validation, story selection)
2. **Audit Deferrals Mode** - Phase 7 workflow (deferral audit + hook integration) - NEW (STORY-050)
...
**Mode priority:** Sprint > Audit > Story > Default
...
This skill delegates specialized tasks to **2 subagents:**
...
- `mode-detection.md` - Mode detection logic and context markers (265 lines)
...
- `sprint-command-workflow.md` - Sprint command business logic extracted from /create-sprint (STORY-458)
...
**Total reference content:** 19 files, ~9,300 lines (loaded progressively as needed)
