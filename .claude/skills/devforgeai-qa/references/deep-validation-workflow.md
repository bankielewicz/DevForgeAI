...
C++:     OpenCppCoverage --sources {source_dir} --modules {debug_build_dir} --export_type cobertura:{coverage_dir}/coverage.cobertura.xml -- {test_executable}
```
...
Pattern: grep "C#|Python|Node.js|Java|Go|Rust|C\+\+"
...
**Treelint Integration:** This subagent is Treelint-enabled for AST-aware semantic code search (40-80% token reduction). Treelint availability is detected automatically; falls back to Grep-based scanning when unavailable.
...
**Treelint Integration:** All three parallel validators (test-automator, code-reviewer, security-auditor) are Treelint-enabled for AST-aware semantic code search (40-80% token reduction). Treelint availability is detected automatically; falls back to Grep-based search when unavailable.
