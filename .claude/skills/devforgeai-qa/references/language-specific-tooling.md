...
```bash
# Build and run tests (Debug config required for coverage)
cmake --build build --config Debug
ctest --test-dir build --build-config Debug

# Run specific test
ctest --test-dir build --build-config Debug -R test_address_cache
```

### Coverage Tools

**OpenCppCoverage (Windows/MSVC)**
```bash
# Install via Chocolatey
choco install opencppcoverage

# Collect coverage (requires Debug build with PDB symbols)
OpenCppCoverage --sources {source_dir} --modules {debug_build_dir} --export_type cobertura:{coverage_dir}/coverage.cobertura.xml -- {test_executable}
```

**Key Requirements:**
- **Debug build required** — Release builds strip PDB symbols needed for line mapping. Coverage command must explicitly build or reference Debug config.
- **`--modules` flag is MANDATORY** — Without it, no modules are selected and report is empty (0 lines covered).
- **`--sources` flag** — Scopes coverage to project source files only (excludes system headers).
- **Cobertura XML output** — Identical schema to .NET coverage. Reuses existing .NET parsing logic (zero new parsing needed).

**WSL Usage:**
```bash
# Run from WSL via /mnt/c/ path (no cmd.exe wrapper needed)
"/mnt/c/Program Files/OpenCppCoverage/OpenCppCoverage.exe" \
  --sources "C:\Projects\MyApp\native\hook" \
  --modules "C:\Projects\MyApp\native\build\tests\Debug" \
  --export_type "cobertura:C:\Projects\MyApp\coverage.xml" \
  -- "C:\Projects\MyApp\native\build\tests\Debug\test_my_app.exe"
```

### Parsing Coverage Data (C++)

**Cobertura XML Format (identical to .NET):**
```xml
<coverage line-rate="1" lines-covered="45" lines-valid="45">
  <packages>
    <package name="test_app.exe" line-rate="1">
      <classes>
        <class name="my_module.cpp" filename="native\hook\src\my_module.cpp" line-rate="1">
...
| **C++** | `ctest --test-dir build` | `OpenCppCoverage --sources {src} --modules {dbg} --export_type cobertura:{cov}/coverage.cobertura.xml -- {test_exe}` | `{coverage_dir}/coverage.cobertura.xml` | N/A (MSVC built-in) |
...
| **C++** | `OpenCppCoverage --sources {source_dir} --modules {debug_build_dir} --export_type cobertura:tests/coverage/{STORY_ID}/coverage.cobertura.xml -- {test_executable}` | `tests/coverage/{STORY_ID}/coverage.cobertura.xml` |
