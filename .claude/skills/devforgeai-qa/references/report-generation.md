...
**When to generate:** When `overall_status == "FAIL"` OR `overall_status == "PASS WITH WARNINGS"` (gaps need targeted remediation or advisory tracking)

### Generate gaps.json

```
IF overall_status == "FAIL" OR overall_status == "PASS WITH WARNINGS":
    ...
    FOR EACH file in coverage_gaps:
        gap_entry = {
            ...
            "blocking": true  # boolean - Coverage gaps are always blocking
        }
    ...
    FOR EACH violation in (critical_violations + high_violations):
        violation_entry = {
            ...
            "message": violation.message,
            "remediation": violation.remediation,
            "blocking": true  # CRITICAL/HIGH are always blocking
        }
        gaps_data.anti_pattern_violations.append(violation_entry)

    # Extract anti-pattern violations (MEDIUM and LOW - advisory, non-blocking)
    FOR EACH violation in (medium_violations + low_violations):
        violation_entry = {
            "file": violation.file,
            "line": violation.line,
            "type": violation.type,
            ...
            "message": violation.message,
            "remediation": violation.remediation,
            "blocking": false  # MEDIUM/LOW are non-blocking warnings
        }
        gaps_data.anti_pattern_violations.append(violation_entry)
    ...
    # Write gaps.json for PASS WITH WARNINGS or FAIL (creates structured gap export)
...
```json
{
  ...
  "anti_pattern_violations": [
    ...
    },
    {
      ...
      "severity": "MEDIUM",
      "message": "Duplicate code block detected (15 lines)",
      "remediation": "Extract shared logic to common function",
      "blocking": false
    }
  ],
...
The `qa_result` field at the root level indicates the overall QA outcome. Valid values are: FAILED, PASS WITH WARNINGS, or PASSED.

- **FAILED**: One or more blocking gaps detected (CRITICAL/HIGH severity)
- **PASS WITH WARNINGS**: Only non-blocking gaps detected (MEDIUM/LOW severity)
- **PASSED**: No gaps detected

### Blocking Field Behavior

The `blocking: boolean` field indicates whether a gap blocks QA approval:

**Severity Rules:**
- **CRITICAL severity** → `blocking: true` (always)
- **HIGH severity** → `blocking: true` (always)
- **MEDIUM severity** → `blocking: false` (advisory only)
- **LOW severity** → `blocking: false` (advisory only)

**Default Behavior (Backward Compatibility):**
When the `blocking` field is missing from an existing gaps.json file, the default value is `true`. This ensures backward compatibility with existing gaps.json files that were created before this schema update - they will continue to be treated as blocking gaps and parse successfully without errors.

**Determining Function:**
```
FUNCTION determine_blocking(severity):
...
### implementing-stories Skill Integration
