...
| `--blocking-only` | `$BLOCKING_ONLY` | false |
...
| `$BLOCKING_ONLY` | Filter to show only blocking gaps |
...
- `blocking` (boolean) - Optional, default: true (CRITICAL/HIGH always blocking)
...
Existing gaps.json files without `blocking` field will parse successfully - when the blocking field is missing, the default blocking value is true. This ensures all existing gap files continue to work without errors.
...
### Step 3.3.5: Apply Blocking-Only Filter (if active)

**IF `$BLOCKING_ONLY` == true:**

Filter `$FILTERED_GAPS` to only include gaps where `blocking === true`.
Advisory gaps (blocking: false) are excluded from the display when this filter is active:

```
$PRE_FILTER_COUNT = count($FILTERED_GAPS)
$FILTERED_GAPS = filter($FILTERED_GAPS where blocking === true)
$ADVISORY_HIDDEN_COUNT = $PRE_FILTER_COUNT - count($FILTERED_GAPS)
# Total count after filter reflects only blocking gaps (filtered count)
```

**Combined Filter Logic (AND, not OR):**
When `--blocking-only` is used with `--min-severity`:
- Both conditions must be met (intersection, not union)
- Example: `--blocking-only --min-severity HIGH` = gaps must be `blocking: true` AND `severity >= HIGH`
- Step 3.3 filters by severity first, then Step 3.3.5 filters by blocking status

**Display Filter Notification:**
```
IF $BLOCKING_ONLY == true AND $ADVISORY_HIDDEN_COUNT > 0:
    Display: "--blocking-only filter active ({$ADVISORY_HIDDEN_COUNT} advisory gaps hidden)"
```
...
| `$ADVISORY_HIDDEN_COUNT` | Count of advisory gaps hidden by --blocking-only filter |
...
├────┼────────────────┼────────────────────────────┼──────────┼──────┼───────┼────────┤
│ 1  │ anti_pattern   │ src/service.py             │ CRITICAL │ 100  │ S-035 │ [R]    │
│ 2  │ coverage_gap   │ src/indexer.py             │ HIGH     │ 85   │ S-078 │ [R]    │
│ 3  │ code_quality   │ src/processor.py           │ MEDIUM   │ 50   │ S-082 │ [Y]    │
│ ...│ ...            │ ...                        │ ...      │ ...  │ ...   │ ...    │
└────┴────────────────┴────────────────────────────┴──────────┴──────┴───────┴────────┘

Status Column Indicators:
  [R] = Red indicator - Blocking gap (blocking: true) - requires remediation
  [Y] = yellow indicator - Advisory gap (blocking: false) - can be deferred
...
**Validation Rule:** Total Gaps = Blocking Gaps + Advisory Gaps
...
```
Dry-Run Complete:
  Would process X blocking gaps and Y advisory gaps
  Would create Z stories
  Would defer W gaps to technical debt register
...
# Advisory story fields (STORY-348)
**Is Advisory:** {gap.blocking === false}
**Source Gap ID:** {gap.id}  # e.g., GAP-001
**Source Story ID:** {gap.source_story}  # e.g., STORY-123
```

**Feature Name Generation:**

| Gap Type | Blocking | Template |
|----------|----------|----------|
| coverage_gap | true | "Improve test coverage for {layer} layer in {file}" |
| coverage_gap | false | "[ADVISORY] Improve test coverage for {layer} layer in {file}" |
| anti_pattern | true | "Fix {violation_type} violation in {file}" |
| anti_pattern | false | "[ADVISORY] Fix {violation_type} violation in {file}" |
| code_quality | true | "Reduce {metric} in {file}" |
| code_quality | false | "[ADVISORY] Reduce {metric} in {file}" |
| deferral | true | "Resolve deferred DoD item: {item_description}" |
| deferral | false | "[ADVISORY] Resolve deferred DoD item: {item_description}" |

**Advisory Filename Generation (STORY-348):**
- Blocking stories: `STORY-XXX-{slug}.story.md`
- Advisory stories: `STORY-XXX-advisory-{slug}.story.md`

**Priority Mapping:**

| Gap Severity/Layer | Blocking | Story Priority |
|--------------------|----------|----------------|
| CRITICAL | true | High |
| HIGH | true | High |
| Business Logic coverage | true | High |
| MEDIUM | true | Medium |
| Application coverage | true | Medium |
| LOW | true | Low |
| Infrastructure coverage | true | Low |
| ANY severity | **false** | **Low** |

**Note (STORY-348):** Advisory stories (blocking: false) ALWAYS default to priority `Low` regardless of gap severity. This can be overridden by explicit user input during gap selection.

**Priority Override Mechanism:** Users can override the default Low priority for advisory stories during Step 4.3 Interactive Selection by specifying custom priority when selecting individual gaps.
...
Batch Story Creation Complete:
  Blocking Stories Created: {count where is_advisory === false}
  Advisory Stories Created: {count where is_advisory === true}
  Total Stories Created: {count of $CREATED_STORIES}
  Stories Failed: {count of $FAILED_STORIES}

Created Blocking Stories:
  - STORY-XXX: {gap description}
  ...

Created Advisory Stories:
  - STORY-YYY: [ADVISORY] {gap description}
  ...
...
**Note (STORY-348):** Summary distinguishes blocking vs advisory story counts for sprint planning visibility.

### Phase 05 Output

| Variable | Value |
|----------|-------|
| `$CREATED_STORIES` | Array of {story_id, gap_id, gap_type, is_advisory} |
| `$FAILED_STORIES` | Array of {gap_id, error} |
| `$STORIES_CREATED_COUNT` | Count of successful creations |
| `$BLOCKING_STORIES_COUNT` | Count of blocking stories created (STORY-348) |
| `$ADVISORY_STORIES_COUNT` | Count of advisory stories created (STORY-348) |
...
**Filter Active Footer (when $BLOCKING_ONLY == true):**
```
