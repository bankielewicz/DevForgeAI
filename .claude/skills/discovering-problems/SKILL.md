...
FOR phase_num in range(CURRENT_PHASE, 8):  # Phases 01-07
    ...
    1. LOAD: Read(file_path=".claude/skills/discovering-problems/phases/{phase_files[phase_num]}")
    ...
    2. REFERENCE: Read the phase's reference file as specified in the phase Contract section.
       Each phase references a file in .claude/skills/brainstorming/references/.
...
- **Templates:** `.claude/skills/brainstorming/assets/templates/` (loaded via Read, not duplicated)
...
Load error recovery patterns from: `.claude/skills/brainstorming/references/error-handling.md`
