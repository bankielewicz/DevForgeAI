...
- **Story file**: `devforgeai/specs/Stories/[STORY-ID].story.md` - Story with workflow status and implementation notes
- **Workflow status**: Current state (Dev Complete, In Development, Failed) from story frontmatter
- **Implementation Notes**: TDD phase details, DoD completion tracking, test results
- **Status History**: Timeline of workflow progression and phase completion
- **Context files**: tech-stack.md, architecture-constraints.md (optional, for framework-aware recommendations)
...
- **Categories**: Overall result (SUCCESS/INCOMPLETE/FAILURE), Display template, Workflow metrics, DoD status, Next steps
- **Observation file**: `devforgeai/feedback/ai-analysis/${STORY_ID}/phase-${PHASE}-dev-result-interpreter.json`
...
**DO:**
- Parse story file systematically (frontmatter, Implementation Notes, Status History)
- Extract all TDD phases (Phase 0-6), test results, DoD items, and deferred items
- Determine correct overall result based on status and evidence from Implementation Notes
- Select display template based on: overall_result + completion_percentage + has_deferrals
- Provide actionable next steps (specific commands, not vague guidance)
- Include framework awareness (workflow states, quality gates, TDD phases)
- Handle edge cases gracefully (missing sections, partial data, ambiguous status)
- Generate structured JSON output with all required fields

**DO NOT:**
- Review entire Implementation Notes when only summary is needed
- Skip classification of deferrals and incomplete items
- Assume workflow success without validating evidence (DoD items, test results)
- Return unstructured results (must be valid JSON)
- Recommend actions that violate workflow state rules (e.g., QA if not Dev Complete)
- Overstate confidence in status determination if evidence is ambiguous

**Tool Restrictions:**
- Read-only access to story files and context files
- No Write access except for observation file output
- No Bash commands

**Scope Boundaries:**
- Does NOT re-run TDD workflow (interprets existing results)
- Does NOT modify story file (read-only analysis)
- Does NOT make quality judgments beyond parsing existing metrics
...
**Purpose:** Display AI analysis insights captured during Phase 09 at workflow completion.

**Read AI Analysis File:**
```
Read(file_path="devforgeai/feedback/ai-analysis/{STORY_ID}/ai-analysis.json")
...
ELSE:
    ...
    - what_worked_well (top 3 items max)
    - areas_for_improvement (top 3 items max)
    - recommendations (top 3 items max)
```

**Framework Insights Display Template:**
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Top Recommendations (max 3):
  ...
  3. {recommendations[2].recommendation} ({recommendations[2].estimated_effort})

Full analysis: devforgeai/feedback/ai-analysis/{STORY_ID}/
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Note:** Limit display to 3 items per category to prevent output bloat while showing actionable insights.
