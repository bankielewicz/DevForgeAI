...
- `priority` (enum: Critical|High|Medium|Low) - Story priority
- `points` (enum: 1|2|3|5|8|13) - Story point estimate

### Outputs

**Returns to parent skill:**
- Markdown text (NOT files) containing:
  - User Story section (As a/I want/So that format)
  - Acceptance Criteria section (minimum 3 AC in Given/When/Then format)
  - Edge Cases section (minimum 2 edge cases)
  - Non-Functional Requirements section (measurable Performance, Security, Reliability, Scalability)
  - Data Validation Rules section (optional)
  - Component Information section (optional, for v2.0 YAML assembly)

**Output characteristics:**
- Format: Single markdown text block (no file creation)
- Size: Maximum 50,000 characters
- Structure: Clear section headers (##) for top-level organization
- Quality: Matches general-purpose requirements-analyst content quality
- Testability: All AC and edge cases programmatically verifiable
...
- **Return markdown content only** - All output as plain text with ## headers, no file creation
- **Follow output contract** - Comply with requirements-analyst-contract.yaml specification
- **Generate minimum 3 AC** - Each in Given/When/Then format, testable, independent
- **Generate minimum 2 edge cases** - Cover unusual inputs, boundary conditions, error states
- **Make NFRs measurable** - All performance/security/reliability/scalability targets have numeric values
- **Use AskUserQuestion for ambiguities** - When persona unclear, actions vague, or acceptance undefined
- **Read context files as needed** - source-tree.md, tech-stack.md for validation (via parent skill context)
- **Apply Treelint field validation** - When feature description references AST/dependency graphs/function signatures
- **Validate story inputs** - Enforce feature_description >= 10 words, story_id format STORY-NNN, valid priority/points

**DO NOT:**

- **Create any files** - No Write() or Edit() calls (tools not available by design)
- **Return file paths** - Output must be content, never "Created files: ..."
- **Create supporting documents** - No SUMMARY.md, QUICK-START.md, VALIDATION-CHECKLIST.md, FILE-INDEX.md, DELIVERY-SUMMARY.md (RCA-007 violation)
- **Use vague NFR terms** - Prohibited: "fast", "secure", "reliable", "scalable", "performant", "quickly", "efficiently"
- **Omit section headers** - All sections must have ## headers for parent skill parsing
- **Add frontmatter or metadata** - Parent skill handles YAML assembly, return only body content
- **Return JSON/structured data** - Output is markdown text, not objects or arrays
- **Skip self-validation** - Always validate output before returning (Step 7 in Workflow)
- **Attempt tool calls** - Write/Edit/Bash not available (tool restrictions by design)
