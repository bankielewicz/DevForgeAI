...
For a VPR-2 assignment, recover the actual requested claim, owner-accepted base and capability/environment scope, policy acceptance and selected contract version. Keep the current routinely accepted candidate separate from the last fully qualified candidate/environment or explicit accepted unqualified baseline. Carry immediate and cumulative impact, compatibility gaps and a proposed Routine/Full validation scope into the design and [prepared handoff](assets/handoff.md). Use the [packaged policy](references/contracts/skill-authoring-contract.md#opt-in-vpr-2-validation-policy); generic “validate,” installation or distribution alone does not demand Full. Only consequential unknowns reopen Q&A. No selection, acceptance or qualification is created by this authoring record.
...
Carry prior report completion, observed outcomes, validation disposition, owner acceptance and qualification as separate historical facts; source edits resolve none of them. A validator's prepared T12 handoff does not require recursive Full qualification. Actual eligible target output, receiver loading and the required receiver action or expected negative disposition remain separately required before T09 when selected by the governing policy, including Full qualification.
