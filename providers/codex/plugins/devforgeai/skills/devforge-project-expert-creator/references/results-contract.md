...
10. handoff.md: preserves completed output references and the allocated next task. In managed operation runtime binds final bytes and publishes/readbacks the external receipt; the model does not drive a receipt-helper fallback. Its own digest is excluded. Prepared content is separate from transition admission and actual receiving invocation.
