...
4. Save the run manifest and case grades as separate evidence files. Each grade uses the selected devforge.skill-case-grade/v1 or v2 schema; v2 retains separate assertion_judgments. Preserve case_id, arm, overall, run_manifest with its path/sha256, and separately graded dimensions. Record observed token/time metrics only when the runtime exposes them; otherwise leave them unknown/null.
...
## VPR-2 observation selection and reuse

The preceding attempt procedure applies to selected native observations. A reviewed Routine no-native branch completes its enforced selection/reporting records without a native plan, boundary claim, schedule or launch. Legacy v1 and Full retain applicable native obligations. Routine selected B/A still requires matching intact C PASS. A selected A may omit B only under the pre-run T04-reviewed T07 NOT_SELECTED/NOT_RUN disposition. Missing/failed/stale C blocks dependents; an observed selected B failure cannot be relabeled unselected.

Freeze the entire original assertion/variant/arm/repetition inventory and required D/S/N evidence. Preserve original SV-009, SV-012 and SV-024–SV-028 native requirements and representative real nested review, meaningful return/failure and eligible receiving integration. Generic inner validation does not trigger a seventeen-case Full campaign. Synthetic transcripts/bundles prove only explicitly synthetic mechanical/error branches; they cannot establish native clean negatives, installed resources, source/history isolation, callback origin, nested execution or receiving.

Reuse one actual observation only for matching candidate/environment/inputs/prompt, forcing/implicit invocation, arm/variant/repetition, visibility, freshness and prerequisite timing. Record shared sample counts/correlation. Keep separate per-arm/assertion judgments against complete raw outputs; one independently allocated paired/batch grade may emit those records. Baseline quality FAIL can complete an intact comparison while the candidate's judgment remains separate; absent/mismatched evidence cannot.

Full receiving assertions consume actual eligible output produced by the measured target, observed receiver loading and the required completed action or expected negative disposition before T09. The evaluator's prepared T12 handoff is distinct and enqueues no recursive qualification. Keep receipt creation, publication/readback and actual invocation separate.

The v2 native allocation includes the complete typed generation graph, including T04 review, grades, controls/probes, actual receiving, meaningful parent returns and continuations; static reviewers are not installed target attempts. A separate external native grant supplies limits and original UTC/monotonic clocks; existing exhausted ledgers and engineering funding cannot fund it. Failed allocated launch stays charged, no replay relaunch/reset is permitted, and unsupported dispatch stays COULD_NOT_RUN. Supported authentication lifecycle reuse may be separate from fresh evidence state, but its actual mechanism, global-state isolation and required interactive/nested transport remain unresolved until observed. Never copy credentials or silently reuse writable history.
