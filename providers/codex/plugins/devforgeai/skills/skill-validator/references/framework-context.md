...
## Selected VPR-2 policy source

This revision derives its applicable policy and exact record matrix from G1 DevForgeAI commit `75bcba915fd1d5f88477318d4b27db6e6961ca81`; exact source/destination hashes are in [derivation.json](derivation.json). Read [authoring policy](contracts/skill-authoring-contract.md#opt-in-vpr-2-validation-policy) and [v2 records](contracts/execution-contract.md#vpr-2-record-contract). Earlier dated implementation observations remain historical. Current source/package identity is not native validation, installed runtime support or operational adoption.

Routine maintenance is within actually accepted capability/environment scope and cumulative lineage. Full covers first/explicit qualification, new scope, consequential authority/transfer, a Full-required claim, or unbounded impact. Current Routine acceptance and last qualified identity are separate; a helper cannot change either. All original assertions, structural checks and semantic invariants remain. Engineering implementation of this enforcement amendment requires matching CI-05 Full before use. Unknown authentication, effective isolation, interactive/nested transport and actual receiving support remain unresolved observations, not new authorization to implement or run them.
