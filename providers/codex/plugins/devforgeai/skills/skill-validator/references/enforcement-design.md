...
## VPR-2 implementation/adoption boundary

W1, P1–P6, T01–T12, S001–S013 and R01–R10 retain all Enforced meanings. V2 changes only externally selected policy/record semantics: independently reviewed conditional native dispositions, strict authority/lineage/coverage binding and honest separate reporting/suitability. Required native observations remain actual C/B/A under the selected dependencies; unselected ones stay NOT_RUN. The protected runtime owns admission, original clocks/charges, imported collector authenticity, transitions and custody receipts. The skill helper advises and grants no acceptance. Required target receiving evidence is before Full T09; evaluator T12 is nonrecursive.

This consequential enforcement change itself triggers CI-05 Full before adoption; CI-06 applies to changed transfer meaning. Deterministic implementation tests, schema/hash checks and source authoring cannot bootstrap operational qualification or show active denial hooks. Actual native/control/compatibility/receiving evidence and supported authentication/transport/boundaries remain separately required. Hook status: Design only; no installed or observed enforcement is claimed by this package revision.
