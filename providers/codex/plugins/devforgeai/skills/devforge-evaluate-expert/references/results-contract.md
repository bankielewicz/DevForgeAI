> Record selection: the legacy v1 sections below retain historical meanings. New manual Routine/Full evaluations use [manual v2 records](manual-records.md) and the supplied v2 assets. Do not mix record versions. VPR-2 permits independently reviewed conditional native coverage while every phase/task remains Enforced. Automated funding is not a manual prerequisite.

# Result and handoff contract

This is the paired devforge-project-expert-creator/devforge-evaluate-expert authoring contract. JSON schema identifiers below identify these local record shapes; they are not new DevForge CLI gate schemas. The bundled helper checks only the fields documented for it.
...
1. skill-design-spec.md: existing design supplied by devforge-project-expert-creator, or the actual selected equivalent specification.
...
devforge-project-expert-creator verifies selected identities, retains the old candidate, resolves consequential ambiguity and applies only authorized changes. It reports changed files and new candidate identity without running validation. devforge-evaluate-expert then assesses the new revision and closes findings only using new matching evidence. Preserve finding history; do not overwrite an earlier FAIL.
