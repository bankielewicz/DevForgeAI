> Manual promotion: [manual operation](manual-operation.md) defines the implemented adoption prerequisite and its limits. The H1–H5 mapping below is retained historical managed/hook design; it does not admit a new-name adapter or activate hooks.

# Enforced validation workflow and hook proposals

All validator phases P1-P6 and tasks T01-T12 are Enforced requirements, explicitly selected by the user on 2026-09-07. No tasks inherit an unrecorded optional default. Runtime hook deployment is outside devforge-project-expert-creator's authoring scope.
