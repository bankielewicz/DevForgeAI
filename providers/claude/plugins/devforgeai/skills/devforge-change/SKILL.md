...
| The session was interrupted and is being resumed | Preserve the phase you reached and the evidence you gathered; do not restart from the beginning and do not discard a partial impact table. Before continuing, re-read the trigger, every artifact revision you cited and the session assignment, and confirm they still match the identities you recorded. An identity that moved while you were away starts a new iteration - reconcile it as a stale upstream rather than continuing on the reading you made of bytes that have since changed. |
...
Resuming is not stopping, and neither is it starting over. A session that comes back to a half-finished assessment finishes the phase it was in, on identities it has just re-verified.
