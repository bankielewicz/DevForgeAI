...
A multi-step workflow skill may also carry `phases/`, one file per phase, linked directly from `SKILL.md`. The portable specification permits additional directories, and the compiled installer and exporter select by exclusion - `evals/`, `history/`, `__pycache__/`, `*.pyc`, `provenance.json` - so `phases/` ships with the rest of the runtime package. It is a package-local structure **proposed** for the authoring contract's per-skill structure list and not yet in it; see [phase mapping](phase-mapping.md). A phase file describes obligations and enforces nothing: phase state, transitions, gates, validators, mutation permission and acceptance belong to the compiled CLI.
