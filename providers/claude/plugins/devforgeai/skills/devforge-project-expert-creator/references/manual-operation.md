...
| Integration owner exports a runtime plugin | `devforge install export-plugin --framework <DevForgeAI> --provider claude --output <parent>/devforgeai` | Writes a new export directory only, omitting `evals/`, `history/`, caches and `provenance.json`; everything else under a skill, including `phases/`, ships. A collision or an unsupported component refuses. Export is staging for isolated resource testing; it is not adoption and not evidence of native admission. |
| Integration owner installs into a project | `devforge --project <abs-project> install framework --framework <DevForgeAI> --provider claude --runtime <abs-devforge>` | Generates the project-local installed copies from the selected provider source, under the same exclusions. A package declaring `hooks/runtime-requirements.json` needs the explicit absolute `--runtime` executable. Installation is not activation, not evaluation and not acceptance. |
...
`devforge expert bind`, `devforge expert status` and `devforge check` accept their flags at the leaf subcommand, as their own `--help` output shows; `install framework` takes `--project` as a global flag before the subcommand. The companion repository's older `scripts/install_framework.py` still exists and is legacy Python awaiting the required migration; it is not the authority for these operations. Inspect the selected executable's own `--help` rather than assuming either form.
...
The phases in `phases/` are the workflow this skill performs: five classified Enforced by the authoring contract, plus a sixth completion phase whose classification is a proposal to that contract's owner. They are not CLI subcommands, no command above intercepts them, and the markdown files enforce nothing. The commands implement exactly the predicates stated in their rows: they do not guard manual authoring, receiving, or native execution against phase evidence.
