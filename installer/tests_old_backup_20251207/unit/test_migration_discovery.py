...
class TestMigrationDiscovery:
    ...
    def test_discover_single_migration_for_patch_upgrade(self, migrations_dir, sample_migration_scripts):
        ...
        discovery = MigrationDiscovery(migrations_dir=migrations_dir, logger=Mock())
        ...
        assert migrations[0].path.name == "v1.0.0-to-v1.1.0.py"
...
class TestMigrationScriptConvention:
    ...
    def test_script_name_follows_convention_vX_Y_Z_to_vA_B_C(self, migrations_dir):
        ...
        # Act
        all_scripts = discovery.list_all_migration_scripts()

        # Assert
        assert len(all_scripts) == 2
        assert any(s.path.name == "v1.0.0-to-v1.1.0.py" for s in all_scripts)
        assert any(s.path.name == "v2.0.0-to-v2.1.0.py" for s in all_scripts)
    ...
    def test_invalid_script_names_ignored(self, migrations_dir):
        ...
        assert all_scripts[0].path.name == "v1.0.0-to-v1.1.0.py"
    ...
    def test_parse_from_version_from_script_name(self, migrations_dir):
        ...
        # Act
        scripts = discovery.list_all_migration_scripts()
...
class TestMigrationGapDetection:
    ...
    def test_detect_gap_when_intermediate_migration_missing(self, migrations_dir):
        ...
        When: MigrationDiscovery.discover() is called
        Then: Gap detected and warning logged
        """
        # Arrange
        from installer.migration_discovery import MigrationDiscovery
        ...
        mock_logger = Mock()
        discovery = MigrationDiscovery(migrations_dir=migrations_dir, logger=mock_logger)

        # Act
        migrations = discovery.discover(from_version="1.0.0", to_version="1.2.0")

        # Assert
        assert len(migrations) == 1  # Only found migration returned
        mock_logger.log_warning.assert_called()
        warning_calls = [str(call) for call in mock_logger.log_warning.call_args_list]
        assert any("gap" in call.lower() or "missing" in call.lower() for call in warning_calls)

    def test_get_missing_migrations_returns_gap_list(self, migrations_dir):
        """
        SVC-009: get_missing_migrations() returns list of missing version ranges.

        Given: Upgrade path with gaps
        When: get_missing_migrations() is called
        Then: Returns list of missing migration version ranges
        """
        # Arrange
        from installer.migration_discovery import MigrationDiscovery
        ...
        # Act
        gaps = discovery.get_missing_migrations(from_version="1.0.0", to_version="2.0.0")

        # Assert
        assert len(gaps) >= 1
        assert any(g["from"] == "1.1.0" and g["to"] == "1.2.0" for g in gaps)

    def test_no_gaps_returns_empty_list(self, migrations_dir, sample_migration_scripts):
        """
        SVC-009: No gaps detected when migration chain is complete.
    ...
    def test_missing_migration_logged_as_warning_not_error(self, migrations_dir):
        ...
        # Warning may or may not be logged depending on whether gap exists
...
class TestMigrationDiscoveryEdgeCases:
    ...
    def test_nonexistent_migrations_directory_raises_error(self, tmp_path):
        """
        Edge case: Nonexistent migrations directory raises FileNotFoundError.

        Given: Migrations directory does not exist
        When: MigrationDiscovery is instantiated
        ...
        with pytest.raises(FileNotFoundError):
            MigrationDiscovery(migrations_dir=nonexistent, logger=Mock())
...
class TestMigrationScriptModel:
    ...
    def test_migration_script_path_exists(self, migrations_dir):
        """
        DataModel: MigrationScript.path exists and is executable.
