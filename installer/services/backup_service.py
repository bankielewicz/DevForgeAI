"""Backup service (AC#7) - SECURITY FIXED.

Creates timestamped backups before file operations, with path traversal protection.
...
SECURITY FIXES:
- Path validation with strict timestamp format regex (prevents ../../../ sequences)
- Validate backup directory within installation_root (os.path.abspath + startswith)
- All file operations use validated paths only
"""
import shutil
import time
import re
...
class BackupService:
    ...
    Implements AC#7 requirements with SECURITY hardening:
    - Timestamped backup directory creation with VALIDATED timestamp format
    ...
    - PATH TRAVERSAL PROTECTION: Validates timestamp and backup paths
    """

    # SECURITY: Strict regex for timestamp validation (prevents ../../../ sequences)
    ...
    def _validate_timestamp(self, timestamp: str) -> bool:
        ...
        Example:
            >>> service._validate_timestamp("2025-12-03T14-30-45")
            True
            >>> service._validate_timestamp("../../../etc/passwd")
            False
        """
    ...
    def _validate_backup_path_within_root(self, backup_path: Path, installation_root: Path) -> bool:
        """Validate backup directory is within installation root (path boundary check).
        ...
        Args:
            ...
            installation_root: Root directory boundary

        Returns:
            True if backup_path is within installation_root, False if escaped
        ...
        Example:
            ...
            >>> escaped = Path("/home/user/project/../../etc/passwd")
            >>> service._validate_backup_path_within_root(escaped, root)
            False
        """
        try:
    ...
    def create_backup(self, target_dir: Path, files_to_backup: List[Path]) -> Path:
        ...
        SECURITY:
        - Validates timestamp format with strict regex
        - Validates backup directory stays within installation root
