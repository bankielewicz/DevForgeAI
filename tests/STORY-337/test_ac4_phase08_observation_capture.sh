#!/bin/bash
# Test: AC#4 - Phase 08 Observation Capture with git operation observations
# Story: STORY-337
# Expected: FAIL (RED phase) - Observation Capture (EPIC-051) section does not exist yet

set -e

PHASE_FILE="src/claude/skills/devforgeai-development/phases/phase-08-git-workflow.md"
TEST_NAME="AC4: Phase 08 Observation Capture"
...
# Test 2: Verify git operation observation support
echo "Test 2: Checking for git operation observation instructions..."
if grep -qi "git" "$PHASE_FILE" && grep -qi "observation" "$PHASE_FILE"; then
    echo "  PASS: Git operation observation context found"
else
    echo "  FAIL: Git operation observation instructions missing"
    exit 1
fi
...
# Test 5: Verify OBS-08 ID pattern
echo "Test 5: Checking for OBS-08-{timestamp} ID pattern..."
if grep -q "OBS-08" "$PHASE_FILE"; then
    echo "  PASS: OBS-08 ID pattern found"
else
    echo "  FAIL: OBS-08-{timestamp} ID pattern not found"
    exit 1
fi
