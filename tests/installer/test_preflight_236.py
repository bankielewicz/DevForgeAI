...
class TestWritePermissionProbe:
    ...
    def test_write_permission_passes_when_writable(self, temp_directory):
        """
        ...
        with patch("shutil.disk_usage", return_value=mock_usage):
            with patch.object(PlatformDetector, "detect", return_value=mock_platform_info):
                # Act
                validator = PreflightValidator(target_dir=temp_directory)
                result = validator.validate()
    ...
    def test_write_permission_cleans_up_test_file_after_success(self, temp_directory):
        """
    ...
    def test_write_permission_cleans_up_test_file_after_failure(self, temp_directory):
        """
...
class TestDryRunMode:
    ...
    def test_dry_run_returns_file_counts(self, temp_directory):
        """
    ...
    def test_dry_run_returns_exclusion_list(self, temp_directory):
        """
    ...
    def test_dry_run_returns_warnings(self, temp_directory):
        """
...
class TestSourceFileAudit:
    ...
    def test_source_audit_returns_total_file_count(self, temp_directory):
        """
    ...
    def test_source_audit_applies_exclusion_patterns(self, temp_directory):
        """
        ...
        with patch("shutil.disk_usage", return_value=mock_usage):
            with patch.object(PlatformDetector, "detect", return_value=mock_platform_info):
                ...
                # Assert - __pycache__ should be in exclusions
                has_exclusion_info = (
                    hasattr(result, "exclusions") or
                    hasattr(result, "audit_info") or
                    any("exclude" in c.message.lower() or "skip" in c.message.lower()
                        for c in result.checks if c.name == "source_audit")
                )
                # This test validates exclusion patterns are considered
...
class TestBusinessRules:
    ...
    def test_br_002_test_file_cleanup_in_finally_block(self, temp_directory):
        """
        BR-002: Test file cleanup must happen in finally block
    ...
    def test_br_003_passed_true_when_all_checks_pass(self, temp_directory):
        """
...
class TestServiceRequirements:
    ...
    def test_svc_002_write_permission_probe_creates_deletes_file(self, temp_directory):
        """
...
class TestNonFunctionalRequirements:
    ...
    def test_nfr_001_validation_completes_under_2_seconds(self, temp_directory):
        """
    ...
    def test_nfr_002_no_orphan_files_on_failure(self, temp_directory):
        """
        ...
        files_before = set(os.listdir(temp_directory))
        ...
        with patch("shutil.disk_usage", return_value=mock_usage):
            with patch.object(PlatformDetector, "detect", return_value=mock_platform_info):
                ...
                files_after = set(os.listdir(temp_directory))
