#!/usr/bin/env bash
# STORY-457 AC#1: Refactored /validate-epic-coverage meets lean orchestration targets
# Tests run against src/ tree per CLAUDE.md
set -euo pipefail

COMMAND_FILE="src/claude/commands/validate-epic-coverage.md"
PASS=0
FAIL=0
...
echo "=== AC#1: validate-epic-coverage Lean Orchestration ==="
echo ""
...
if [ -f "$COMMAND_FILE" ]; then
    ...
    assert_less_equal "Line count <= 120" "$LINE_COUNT" 120
else
...
if [ -f "$COMMAND_FILE" ]; then
    # Count triple-backtick code blocks that appear before the first Skill() call
    ...
    if [ -n "$SKILL_LINE" ]; then
        ...
        # Each code block has open+close = divide by 2
...
if [ -f "$COMMAND_FILE" ]; then
    assert_true "Has argument-hint with EPIC-ID" grep -q 'argument-hint:.*EPIC-ID' "$COMMAND_FILE"
    assert_true "Supports --interactive flag" grep -q '\-\-interactive' "$COMMAND_FILE"
    assert_true "Supports --quiet flag" grep -q '\-\-quiet' "$COMMAND_FILE"
    assert_true "Supports --ci flag" grep -q '\-\-ci' "$COMMAND_FILE"
