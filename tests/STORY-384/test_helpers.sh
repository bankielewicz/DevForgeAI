#!/usr/bin/env bash
# STORY-384 Shared Test Helpers
# Extracted from duplicated boilerplate across 7 test files (DRY principle).
# Each test file sources this to get: PROJECT_ROOT, DOC, pass/fail counters,
...
require_doc() {
  local label="${1:-Research document}"
  if [ -f "$DOC" ]; then
    pass "$label exists"
  else
...
# --- Assert field value appears only as line-start field labels ---
# Usage: assert_field_only "Applicability: High"
# Checks that every occurrence of the string in DOC begins with "^FieldName:"
assert_field_only() {
  local search_string="$1"
  local field_prefix="${search_string%%:*}:"
  local prose_count=0
  while IFS= read -r line; do
    if ! echo "$line" | grep -qE "^${field_prefix}" 2>/dev/null; then
      prose_count=$((prose_count + 1))
    fi
  done < <(grep "$search_string" "$DOC" 2>/dev/null)

  if [ "$prose_count" -eq 0 ]; then
    pass "'$search_string' appears only in field labels, not prose"
  else
    fail "'$search_string' found $prose_count times outside field labels"
  fi
}
