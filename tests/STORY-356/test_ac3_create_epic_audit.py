"""
STORY-356 AC#3: Audit /create-epic command for explicit Skill(command="...") syntax.

Tests verify that src/claude/commands/create-epic.md contains an explicit
Skill(command="devforgeai-orchestration") invocation.
...
# Constants
# ---------------------------------------------------------------------------
COMMAND_NAME = "create-epic"
EXPECTED_SKILL = "devforgeai-orchestration"
COMMAND_FILE = Path(
    "/mnt/c/Projects/DevForgeAI2/src/claude/commands/create-epic.md"
)


class TestAC3CreateEpicAudit:
    """AC#3: Audit /create-epic command for Skill(command="devforgeai-orchestration")."""

    # --- Happy Path ---

    def test_should_find_command_file_when_create_epic_exists(self):
        """Verify that create-epic.md exists in src/claude/commands/."""
    ...
    def test_should_detect_explicit_skill_invocation_when_create_epic_audited(self):
        """Verify explicit Skill(command="devforgeai-orchestration") syntax is found."""
        result = audit_command(COMMAND_NAME, EXPECTED_SKILL)
    ...
    def test_should_match_exact_skill_name_when_create_epic_audited(self):
        """Verify the matched skill name is exactly 'devforgeai-orchestration'."""
        result = audit_command(COMMAND_NAME, EXPECTED_SKILL)
    ...
    def test_should_not_count_summary_language_as_compliant(self):
        ...
        fake_content = "Architecture -> Skill -> Epic creation"
...
class TestAC3CreateEpicAuditEdgeCases:
    """Edge case tests for AC#3 create-epic audit."""

    def test_should_handle_missing_file_gracefully(self):
        """Missing command file should report NOT FOUND, not raise exception."""
        result = audit_command("does-not-exist", EXPECTED_SKILL)
